...
require(gridExtra); require(tidyr); require(ggplot2); require(stringr);require(RColorBrewer); require(colorRamps); require(plotrix); require(plyr); require(grid);require(gtable); require(dplyr); require(tidyr); library(gridExtra);require(reshape2);library(rbin)
...
p2 <- p1 %+% xs$`Use-inspired`
p3 <- p1 %+% xs$Applied
p1<-p1+scale_fill_manual(name = "ID",values = c('#97CCFF','#42A3FF'), labels=c("2006-2010", "2011-2015"))+labs(x="", title="Type of research by percent (LMI)")+ theme(plot.margin=unit(c(1,1,-0.45,1), "cm"), plot.title = element_text(hjust = 0.5, size = 14))+ scale_y_continuous(expand=c(0,0), limits=c(0,55.5),breaks=seq(0,55,10))
p2<-p2+scale_fill_manual(name = "ID",values =c('#C8DF95','#9BC53D'), labels=c("2006-2010", "2011-2015"))+labs(x="")+ theme(plot.margin=unit(c(-0.25,1,-0.3,1), "cm")) + scale_y_continuous(expand=c(0,0), limits=c(0,55.5),breaks=seq(0,55,10))
p3<-p3+scale_fill_manual(name = "ID",values =c('#F0A490','#E55934'), labels=c("2006-2010", "2011-2015"))+ theme(plot.margin=unit(c(-0.5,1,1,1), "cm"))+ scale_y_continuous(expand=c(0,0), limits=c(0,55.5),breaks=seq(0,55,10)) + labs(x = "Percentage of research")
...
#pdf(file="figures/LMI/4.5_ChangeSuccessRate_LMI.pdf", height=11, width =7)
...
profpast
govcur$perc <- (govcur$freq/usecur$totalcur)*100
govcur
govpast$perc <- (govpast$freq/usepast$totalpast)*100
govpast
   ...
   guides(fill=guide_legend(title=NULL)) +
    theme(legend.position=c(0.922, 0.98),
        ...
        legend.key=element_blank(),
          axis.title.x=element_text(size=12), axis.title.y=element_text(size = 12),
         legend.text = element_text(size = 14), legend.background = element_rect(color = "white")) +
  labs(title="Internal",x="% research funding", y = "Percent of responsess")+
  scale_y_continuous(expand =c(0,0), limits=c(0,101), breaks=seq(0, 100, 20)) 
...
#pdf(file="figures/LMI/4.6_DistributionFunding_LMI_percentnarrow.pdf", height=11, width =7)
#svg(file="figures/LMI/4.6_DistributionFunding_LMI.svg", width = 11, height= 7)
...
pdf(file="figures/LMI/4.8A_LevelsOfPartnership_LMI_percent.pdf", width = 10, height= 12)
...
ggplot(partlevel.perc.l, aes(x=level, y=percent, fill = year),  ylim(0,375)) + 
  ...
  theme(panel.grid.major.y = element_line(linetype = 3, color = "darkgrey", size = .3),
        ...
        panel.grid.minor = element_blank(),
        legend.key=element_blank(),legend.text=element_text(size = 22),
        axis.text.x=element_text(angle=0, size=22, color = "black"),
         axis.text.y=element_text(size = 22, color = "black"),
        axis.title=element_text(size = 28), 
...
g2<-ggplot(change.part, aes(x=Var1, y=Freq, fill=Var1)) + 
    ...
    theme(axis.text.x=element_text( size=22, color = "black"),
        ...
        panel.grid.major.x = element_blank(),
          panel.grid.minor = element_blank(), plot.title = element_text(hjust = 0.5, size = 28)) + 
  #geom_text(aes(label=Freq), vjust=-0.5, size=3)+
  labs(x="", y="Number of respondents", title="Reason for change in level of partnerships (LMI)")+
  colScale+
...
ggplot(view, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+
  ...
  theme(panel.grid.major.y = element_line(linetype = 3, color = "darkgrey", size = .3),
        ...
        axis.title.y=element_text(size=28), 
        plot.title = element_text(hjust = 0.5, size = 28)) + 
  labs(x="", y="Number of respondents", title="Opinion of change in partnership level (LMI)") + 
  scale_x_discrete(labels=c("Very\npositive", "Slightly\npositive", "Neutral", "Slightly\nnegative", "Very\nnegative")) +
  colScale+
...
#pdf(file="figures/LMI/4.9_AnticipatedChangeFunding_LMI.pdf", height=11, width =7)
...
ggplot(data=ant, aes(x=Var2, Freq, fill=Var1))+
    ...
    theme( axis.text.x=element_text(size=22, color = "black"),
        ...
        panel.grid.major.x = element_blank(),
          ...
          plot.title = element_text(hjust = 0.5, size = 28)) +
    guides(fill=FALSE)+
    labs(x="", y="Number of respondents", fill="", title="Anticipated change in research funding (LMI)")+
