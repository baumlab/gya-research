...
#pdf(file="figures/LMI/4.1_FieldofResearch_LMI.pdf", width = 10, height= 11)
...
#pdf(file="figures/LMI/4.2_CareerStage_LMI.pdf", width = 7, height= 8)
...
p2 <- p1 %+% xs$`Use-inspired`
p3 <- p1 %+% xs$Applied
p1<-p1+scale_fill_manual(name = "ID",values = c('#97CCFF','#42A3FF'), labels=c("2006-2010", "2011-2015"))+labs(x="", title="Type of research by percent (LMI)")+ theme(plot.margin=unit(c(1,1,-0.45,1), "cm"), plot.title = element_text(hjust = 0.5, size = 14))+ scale_y_continuous(expand=c(0,0), limits=c(0,20.5),breaks=seq(0,20,5))
p2<-p2+scale_fill_manual(name = "ID",values =c('#C8DF95','#9BC53D'), labels=c("2006-2010", "2011-2015"))+labs(x="")+ theme(plot.margin=unit(c(-0.25,1,-0.3,1), "cm")) + scale_y_continuous(expand=c(0,0), limits=c(0,37),breaks=seq(0,35,5))
p3<-p3+scale_fill_manual(name = "ID",values =c('#F0A490','#E55934'), labels=c("2006-2010", "2011-2015"))+ theme(plot.margin=unit(c(-0.5,1,1,1), "cm"))+ scale_y_continuous(expand=c(0,0), limits=c(0,45.5),breaks=seq(0,45,10)) + labs(x = "Percentage of research")
...
ggplot(sum.reason, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  guides(fill=FALSE) +
  colScale+
  labs(y="Number of respondents", title="Reason for change in research type (LMI)")+
...
#pdf(file="figures/LMI/4.3C_ViewFieldChange_LMI.pdf", width = 7, height= 8)
...
ggplot(partlevel.perc.l, aes(x=level, y=percent, fill = year),  ylim(0,375)) + 
  ...
  scale_x_discrete(labels=c("Exlusive\npartnership", "Strong\npartnership", 'Some\npartnership',"No\npartnership"))+
  theme(panel.grid.major.y = element_line(linetype = 3, color = "darkgrey", size = .3),
        panel.grid.major.x = element_blank(),
        panel.grid.minor = element_blank(),
        legend.key=element_blank(),legend.text=element_text(size = 12),
 ...
 labs(x="", y="Number of respondents", title="Effect of funding changes on the next generation of researchers (LMI)")+
  theme(axis.text.x=element_text(size=12, color = "black"),
       ...
       panel.grid.major.y = element_line(linetype = 3, color = "darkgrey", size = .3),
        ...
        plot.title = element_text(hjust = 0.5, size = 14))+
  #geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,75))+colScale
