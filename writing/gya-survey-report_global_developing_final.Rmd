---
title: "Developing countries online survey on fundamental research - using data from André"
author: ''
...
#load data
dev <- read.csv("data/globalreport_fromAX/gya_survey_clean_final_developing.csv")
...
#according to the table in the report already there should be 272 surveys in this develop country part
unique(dev$Use.)
dev <- dev[!is.na(dev$Use.),]
dev <- droplevels(dev)
dim(dev) #272  86 - perfect
...
require(gridExtra); require(tidyr); require(ggplot2); require(stringr);require(RColorBrewer); require(colorRamps); require(plotrix); require(plyr); require(grid);require(gtable); require(dplyr); require(tidyr); library(gridExtra);require(reshape2);library(rbin); require(Rmisc)
...
#pdf(file="figures/LMI/4.1_FieldofResearch_LMI.pdf", width = 13, height= 8)
#tiff(file="figures/LMI/4.1_FieldofResearch_LMI.tiff", width = 13, height= 8, units = "in", res = 600)
#svg(file="figures/LMI/4.1_FieldofResearch_LMI.svg", width = 11, height= 7)

f4.1.LMI <- ggplot(field, aes(x = reorder(Var1, -Freq), Freq, fill=Var1)) + 
    geom_bar(stat='identity',position = position_dodge(width=0.5)) + 
  labs(y="Number of respondents",x="", title="Responses by field of research (LMI)")  +
...
ggsave("4.1_FieldofResearch_LMI_Dec2021.svg", plot = f4.1.LMI, device = "svg", path = "figures/LMI", height = 7, width  = 13 , units = "in", dpi = 500)
```
...
#pdf(file="figures/LMI/4.2_CareerStage_LMI.pdf", width = 10, height= 12)
#pdf(file="figures/LMI/4.2_CareerStage_LMI_wide.pdf", width = 12, height= 6)
#tiff(file="figures/LMI/4.2_CareerStage_LMI.tiff", width = 8, height= 10, units = "in", res = 600)
#svg(file="figures/LMI/4.2_CareerStage_LMI.svg", width = 11, height= 7)

f4.2.lmi <- ggplot(experience, aes(x = reorder(Var1, -Freq), Freq, fill=Var1)) + 
  geom_bar(stat='identity',position = position_dodge(width=0.5)) + 
  #geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  #scale_x_discrete(labels=c("Senior\nacademic", "Early\ncareer\nacademic", "Postdoc\nor RA", "Senior\nnon-\nacademic", "Early\ncareer\nnon-\nacademic")) + #narrow
  scale_x_discrete(labels=c("Senior\nacademic", "Early career\nacademic", "Postdoc\nor RA", "Senior\nnon-academic", "Early career\nnon-academic")) + #wide
  labs(y="Number of respondents", x="", title="Responses by career stage (LMI)") +
...
ggsave("4.2_CareerStage_LMI_wide_Dec2021.svg", plot = f4.2.lmi, device = "svg", path = "figures/LMI", height = 7, width  = 13 , units = "in", dpi = 500)
...
p3 <- p1 %+% xs$Applied
p1<-p1+scale_fill_manual(name = "ID",values = c('#97CCFF','#42A3FF'), labels=c("2006-2010", "2011-2015"))+labs(x="", title="Type of research by proportion (LMI)")+ theme(plot.margin=unit(c(1,1,-0.45,1), "cm"), plot.title = element_text(hjust = 0.5, size = 14))+ scale_y_continuous(expand=c(0,0), limits=c(0,41),breaks=seq(0,40,20))
p2<-p2+scale_fill_manual(name = "ID",values =c('#C8DF95','#9BC53D'), labels=c("2006-2010", "2011-2015"))+labs(x="")+ theme(plot.margin=unit(c(-0.25,1,-0.3,1), "cm")) + scale_y_continuous(expand=c(0,0), limits=c(0,81),breaks=seq(0,80,20))
p3<-p3+scale_fill_manual(name = "ID",values =c('#F0A490','#E55934'), labels=c("2006-2010", "2011-2015"))+ theme(plot.margin=unit(c(-0.5,1,1,1), "cm"))+ scale_y_continuous(expand=c(0,0), limits=c(0,101),breaks=seq(0,100,20))


#plot.margin=unit(c(-0.25,1,-0.3,1)

#pdf(file="figures/LMI/4.3_ResearchProportion_LMI.pdf", width = 11, height= 7)
#svg(file="figures/LMI/4.3_ResearchProportion_LMI.svg", width = 11, height= 7)

#grid.arrange(p1,p2,p3, left= textGrob("(a)", gp = gpar(fontsize=14), y=0.93, hjust = -.75), heights=c(0.35,0.3,0.35))
f4.3.lmi <- grid.arrange(p1,p2,p3, heights=c(0.35,0.3,0.35))

#dev.off()

ggsave("4.3_ResearchProportion_LMI_Dec2021.svg", plot = f4.3.lmi, device = "svg", path = "figures/LMI", height = 21, width  = 13 , units = "in", dpi = 500)
...
p2 <- p1 %+% xs$`Use-inspired`
p3 <- p1 %+% xs$Applied
p1<-p1+scale_fill_manual(name = "ID",values = c('#97CCFF','#42A3FF'), labels=c("2006-2010", "2011-2015"))+labs(x="", title="Type of research by percent (LMI)", y="")+ theme(plot.margin=unit(c(1,1,-0.45,1), "cm"), plot.title = element_text(hjust = 0.5, size = 28),  strip.text.x = element_text(size=24, color='#3b92e5'))+ scale_y_continuous(expand=c(0,0), limits=c(0,55.5),breaks=seq(0,55,10))
...
#pdf(file="figures/LMI/4.3_ResearchProportion_percent_LMI.pdf", width = 11, height= 7)
#pdf(file="figures/LMI/4.3_ResearchProportion_percent.narrow_LMI.pdf", width = 7, height= 11)
...
ggsave("4.3_ResearchProportion_percent.narrow_LMI_Dec2021.svg", plot = f4.3.lmi, device = "svg", path = "figures/LMI", height = 21, width  = 13 , units = "in", dpi = 500)

```

```{r 4.3b, echo=FALSE, fig.width=10, message=FALSE, warning=FALSE}
...
#pdf(file="figures/LMI/4.3b_ReasonForFieldChange_LMI.pdf", width = 7, height= 8)
#png(file="figures/LMI/4.3b_ReasonForFieldChange_LMI.png", width = 7, height= 8, units = "in", res = 600)
#svg(file="figures/LMI/4.3b_ReasonForFieldChange_LMI.svg", width = 11, height= 7)

f4.3b.lmi <- ggplot(sum.reason, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 85), breaks = seq(0, 80, 20))#+annotate('text', 5.4,795, label="(b)", size=5)#+geom_text(aes(label=Freq), vjust=-0.5, size=3)
  

#dev.off()

ggsave("4.3b_ReasonForFieldChange_LMI_Dec2021.svg", plot = f4.3b.lmi, device = "svg", path = "figures/LMI", height = 7, width  = 13 , units = "in", dpi = 500)
...
#pdf(file="figures/LMI/4.3C_ViewFieldChange_LMI.pdf", width = 8, height= 9)
#pdf(file="figures/LMI/4.3C_ViewFieldChange_LMI_wide.pdf", width = 10, height= 7)
#svg(file="figures/LMI/4.3C_ViewFieldChange_LMI.svg", width = 11, height= 7)

f4.3c.lmi <- ggplot(view, aes(x=Var1, y=Freq, fill = Var1)) + geom_bar(stat='identity')+
  theme(axis.text.x=element_text(size=22, color = "black"), 
         axis.text.y=element_text(size = 22, color = "black"), 
  ...
  labs(x="", y="Number of respondents", title="Opinion of research type change (LMI)")+
...
ggsave("4.3C_ViewFieldChange_LMI_wide_Dec2021.svg", plot = f4.3c.lmi, device = "svg", path = "figures/LMI", height = 7, width  = 13 , units = "in", dpi = 500)
...
#pdf(file="figures/LMI/4.4A_PerceivedImportanceFundamentalGovernment_LMI.pdf", height=11, width =10)
#pdf(file="figures/LMI/4.4A_PerceivedImportanceFundamentalGovernment_LMI_wide.pdf", height=6, width =10)
#svg(file="figures/LMI/4.4A_PerceivedImportanceFundamentalGovernment_LMI.svg", width = 11, height= 7)

f4.4a.lmi <- ggplot(data=import, aes(x=Var1, y=Freq, fill=Var1)) + 
      geom_bar(stat='identity', position='dodge')+ 
      labs(x="", y="Number of respondents", fill="", title="Perceived importance of fundamental research\nto their government (LMI)")+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,110), breaks = seq(0, 100, 20)) + colScale

#dev.off()

ggsave("4.4A_PerceivedImportanceFundamentalGovernment_LMI_Dec2021.svg", plot = f4.4a.lmi, device = "svg", path = "figures/LMI", height = 7, width  = 13 , units = "in", dpi = 500)
...
#pdf(file="figures/LMI/4.4B_PerceivedChangeResearchPriorityGovernment_LMI.pdf", height=11, width =10)
#pdf(file="figures/LMI/4.4B_PerceivedChangeResearchPriorityGovernment_LMI_wide.pdf", height=6, width =10)
#svg(file="figures/LMI/4.4B_PerceivedChangeResearchPriorityGovernment_LMI.svg", width = 11, height= 7)

f4.4b.lmi <- ggplot(data=sum.priority, aes(x=Var1, Freq, fill=Var1))+
...
ggsave("4.4B_PerceivedChangeResearchPriorityGovernment_LMI_Dec2021.svg", plot = f4.4b.lmi, device = "svg", path = "figures/LMI", height = 7, width  = 13 , units = "in", dpi = 500)
...
#pdf(file="figures/LMI/4.5_ChangeSuccessRate_LMI.pdf", height=19, width =12)
#svg(file="figures/LMI/4.5_ChangeSuccessRate_LMI.svg", width = 11, height= 7)

f4.5.lmi <- ggplot(data=success.change, aes(x=level, Freq, fill=type))+ facet_wrap(~type, nrow=3)+
  geom_bar(stat="identity", position="dodge")+
  ...
  theme(axis.text.x = element_text(angle=0, vjust=0.5, size = 22, color = "black"), 
        ...
        panel.grid.major.x = element_blank(),
        panel.grid.minor = element_blank(),
         strip.text.x = element_text(size=24, color=c("#3b92e5", "#7c9d30", "#ce502e")),
          strip.background = element_rect(colour="white", fill="white"),
         plot.title = element_text(hjust = 0.5, size = 28))+
  guides(fill=FALSE)+
  ...
  labs(x="", y="Number of responses", fill="", title="Change in grant success rate (LMI)")+ colScale4+
  scale_y_continuous(expand = c(0, 0), limits = c(0,121), breaks = seq(0, 120, 30))

#dev.off()

ggsave("4.5_ChangeSuccessRate_LMI_Dec2021.svg", plot = f4.5.lmi, device = "svg", path = "figures/LMI", height = 21, width  = 13 , units = "in", dpi = 500)
...
p1<-p1+scale_fill_manual(name = "ID",values = "#42A3FF")+labs(x="", title="Change in grant success rate (LMI)", y="Number of responses")+ theme(plot.margin=unit(c(1,1,-0.45,1), "cm"), plot.title = element_text(hjust = 0.5, size = 28), strip.text.x = element_text(size=24, color='#3b92e5'))+ scale_y_continuous(expand=c(0,0), limits=c(0,121),breaks=seq(0,120,30))

p2<-p2+scale_fill_manual(name = "ID",values ="#9BC53D")+ theme(plot.margin=unit(c(-0.5,1,1,1), "cm"), strip.text.x = element_text(size=24, color='#7c9d30')) + scale_y_continuous(expand=c(0,0), limits=c(0,121),breaks=seq(0,120,30)) + labs(x="  ", y="Number of responses", title="")

p3<-p3+scale_fill_manual(name = "ID",values ="#E55934")+ theme(plot.margin=unit(c(-0.5,1,0.5,1), "cm"), strip.text.x = element_text(size=24, color='#ce502e'))+ scale_y_continuous(expand=c(0,0), limits=c(0,121),breaks=seq(0,120,30)) + labs(x = "", y = "Number of responses")

#pdf(file="figures/LMI/4.5_ChangeSuccessRate_LMI_2.pdf", height=19, width =12)
#svg(file="figures/LMI/4.5_ChangeSuccessRate_LMI.svg", width = 11, height= 7)

f4.5.2.lmi <- grid.arrange(p1,p2,p3, heights=c(0.33,0.33,0.33))

#dev.off()

ggsave("4.5_ChangeSuccessRate_LMI_2_Dec2021.svg", plot = f4.5.2.lmi, device = "svg", path = "figures/LMI", height = 21, width  = 13 , units = "in", dpi = 500)
...
p1<-p1+scale_fill_manual(name = "ID",values = "#42A3FF")+labs(x="", title="", y="Number of responses")+ theme(plot.margin=unit(c(-0.5,1,1,1), "cm"), plot.title = element_text(hjust = 0.5, size = 28), strip.text.x = element_text(size=24, color='#3b92e5'))+ scale_y_continuous(expand=c(0,0), limits=c(0,121),breaks=seq(0,120,30))

p2<-p2+scale_fill_manual(name = "ID",values ="#9BC53D")+ theme(plot.margin=unit(c(-0.5,1,1,1), "cm"), strip.text.x = element_text(size=24, color='#7c9d30')) + scale_y_continuous(expand=c(0,0), limits=c(0,121),breaks=seq(0,120,30)) + labs(x="  ", y="Number of responses", title="")

p3<-p3+scale_fill_manual(name = "ID",values ="#E55934")+ theme(plot.margin=unit(c(-0.5,1,1,1), "cm"), strip.text.x = element_text(size=24, color='#ce502e'))+ scale_y_continuous(expand=c(0,0), limits=c(0,121),breaks=seq(0,120,30)) + labs(x = "", y = "Number of responses")

#pdf(file="figures/LMI/4.5_ChangeSuccessRate_LMI_3.pdf", height=17, width =25)
#svg(file="figures/LMI/4.5_ChangeSuccessRate_LMI.svg", width = 11, height= 7)

#grid.arrange(p1,p2,p3, heights=c(0.33,0.33,0.33))


f4.5.3.lmi <- grid.arrange(arrangeGrob(p1, p2, p3, 
                         layout_matrix=rbind(c(1,1,2,2),
                                             c(NA,3,3,NA))), top=textGrob("Change in grant success rate (LMI)",gp=gpar(fontsize=28)))

#dev.off()

ggsave("4.5_ChangeSuccessRate_LMI_3_Dec2021.svg", plot = f4.5.3.lmi, device = "svg", path = "figures/LMI", height = 14, width  = 26 , units = "in", dpi = 500)
...
p1<-ggplot(fp, aes(percent, fill=year))+ geom_histogram(position='dodge', binwidth=10)  + colScale7 +
  ...
  scale_y_continuous(expand =c(0,0), limits=c(0,251), breaks=seq(0, 250, 50))+
...
#pdf(file="figures/LMI/4.6_DistributionFunding_LMI.pdf", height=11, width =7)
#svg(file="figures/LMI/4.6_DistributionFunding_LMI.svg", width = 11, height= 7)
...
grid.arrange(arrangeGrob(p1+ theme(legend.position="none"), p2,
                         ...
                         p5,
                         layout_matrix=rbind(c(1,1,2,2),c(3,3,4,4), c(NA,5,5,NA))), top=textGrob("Distribution of research funding (LMI)",gp=gpar(fontsize=17)))
...
#pdf(file="figures/LMI/4.6_DistributionFunding_LMI_percentnarrow.pdf", height=17, width =13)
#svg(file="figures/LMI/4.6_DistributionFunding_LMI.svg", width = 11, height= 7)
...
#for narrow view
f4.6.lmi <- grid.arrange(arrangeGrob(p1+ theme(legend.position="none"), p2,
                         p3+ theme(legend.position="none"), p4,
                         p5,
                         layout_matrix=rbind(c(1,1,2,2),c(3,3,4,4), c(NA,5,5,NA))), top=textGrob("Distribution of research funding (LMI)",gp=gpar(fontsize=28)))

#dev.off()

ggsave("4.6_DistributionFunding_LMI_percentnarrow_Dec2021.svg", plot = f4.6.lmi, device = "svg", path = "figures/LMI", height = 21, width  = 26 , units = "in", dpi = 500)
...
#pdf(file="figures/LMI/4.6_DistributionFunding_LMI_percentwide.pdf", height=10, width =20)
...
#for narrow view
f4.6.wide.lmi <- grid.arrange(arrangeGrob(p1+ theme(legend.position="none"), p2 + theme(legend.position="none"),
                         p3 , p4 +theme(legend.position="none"),
                         p5,
                         layout_matrix=rbind(c(1,1,2,2,3,3),
                                             c(NA,4,4,5,5,NA))), 
             top=textGrob("Distribution of research funding (LMI)",gp=gpar(fontsize=30)))

#dev.off()

ggsave("4.6_DistributionFunding_LMI_percentwide_Dec2021.svg", plot = f4.6.wide.lmi, device = "svg", path = "figures/LMI", height = 14, width  = 39 , units = "in", dpi = 500)
...
#pdf(file="figures/LMI/4.6_DistributionFunding_LMI_percent_3.pdf", height=17, width =17)
...
#for narrow view
f4.6.3.lmi <- grid.arrange(arrangeGrob(p1+ theme(legend.position="none"), p2,
                         p3 +theme(legend.position="none"), p4 ,
                         p5,
                         layout_matrix=rbind(c(1,1,2,2),
                                             c(3,3,4,4), 
                                             c(NA,5,5,NA))),
             top=textGrob("Distribution of research funding (LMI)",gp=gpar(fontsize=35)))

#dev.off()

ggsave("4.6_DistributionFunding_LMI_percent_3_Dec2021.svg", plot = f4.6.3.lmi, device = "svg", path = "figures/LMI", height = 21, width  = 26 , units = "in", dpi = 500)
...
#pdf(file="figures/LMI/4.7A_ImportancePracticalApplication_LMI.pdf", height=7, width =11)
#svg(file="figures/LMI/4.7A_ImportancePracticalApplication_LMI.svg", width = 11, height= 7)

f4.7a.lmi <- ggplot(prac, aes(Var2, Freq, fill=Var1)) + 
  geom_bar(stat="identity", position = "dodge")+
  ...
  theme(panel.grid.major.y = element_line(linetype = 3, color = "darkgrey", size = .3),
        ...
        axis.text.x=element_text(angle=-45, hjust = 0.1, size=22, color = "black"),
         axis.text.y=element_text(size = 22, color = "black"),
  ...
  labs(x="", y="Number of respondents", title="Importance of practical application of research for grant success (LMI)")+ colScale7+
  scale_y_continuous(expand = c(0, 0), limits = c(0,100))
  
#dev.off()

ggsave("4.7A_ImportancePracticalApplication_LMI_Dec2021.svg", plot = f4.7a.lmi, device = "svg", path = "figures/LMI", height = 7, width  = 13 , units = "in", dpi = 500)
...
#pdf(file="figures/LMI/4.7A_ImportancePracticalApplication_LMI_percent.pdf", height=6, width =10)
#svg(file="figures/LMI/4.7A_ImportancePracticalApplication_LMI.svg", width = 11, height= 7)

f4.7a.2.lmi <- ggplot(prac.perc.l, aes(opinion, percent, fill=year)) + 
  geom_bar(stat="identity", position = "dodge")+
  #geom_text(aes(label=Freq), vjust=-0.7, hjust=0.45, size=3, position = position_dodge(width = 0.95))+
  theme(panel.grid.major.y = element_line(linetype = 3, color = "darkgrey", size = .3),
        panel.grid.major.x = element_blank(),
  ...
  guides(fill=guide_legend(title=NULL, reverse=FALSE)) + 
  scale_x_discrete(labels=c("Mandatory", "Very\n important","Quite\n important", 'Somewhat\n important', 'Not at all\n important', "Can't\n comment"))+
  labs(x="", y="Percent of responses", title="Importance of practical application of research\nfor grant success (LMI)")+ colScale7+
  scale_y_continuous(expand = c(0, 0), limits = c(0,41))
  
#dev.off()

ggsave("4.7A_ImportancePracticalApplication_LMI_percent_Dec2021.svg", plot = f4.7a.2.lmi, device = "svg", path = "figures/LMI", height = 7, width  = 13 , units = "in", dpi = 500)
...
#pdf(file="figures/LMI/4.7B_ImportancePartnersApplication_LMI.pdf", height=8, width =7)
#svg(file="figures/LMI/4.7B_ImportancePartnersApplication_LMI.svg", width = 11, height= 7)

ggplot(part, aes(Var2, Freq, fill=Var1))+ 
  geom_bar(stat="identity", position = "dodge")+
  #geom_text(aes(label=Freq), vjust=-0.7, hjust=0.5, size=3, position = position_dodge(width = 0.8))+
    guides(fill=guide_legend(title=NULL, reverse=FALSE)) +
    labs(x="", y="Number of respondents", title="Importance of including partners (non-academia) for the success of grant applications (LMI)")+
...
part.perc.l$opinion<-factor(part.perc.l$opinion, levels(part.perc.l$opinion)[c(2,6,4,5,3,1)])
...
#pdf(file="figures/LMI/4.7B_ImportancePartnersApplication_LMI_percent.pdf", height=6, width =10)
#svg(file="figures/LMI/4.7B_ImportancePartnersApplication_LMI.svg", width = 11, height= 7)

f4.7b <- ggplot(part.perc.l, aes(opinion, percent, fill=year))+ 
  geom_bar(stat="identity", position = "dodge")+
  #geom_text(aes(label=Freq), vjust=-0.7, hjust=0.5, size=3, position = position_dodge(width = 0.8))+
    guides(fill=guide_legend(title=NULL, reverse=FALSE)) +
    labs(x="", y="Percent of responses", title="Importance of including partners (non-academia)\nfor the success of grant applications (LMI)")+
  scale_x_discrete(labels=c("Mandatory", "Quite\n important", 'Somewhat\n important',"Not very\n important", 'Not at all\n important', "Can't\n comment"))+
    theme(panel.grid.major.y = element_line(linetype = 3, color = "darkgrey", size = .3),
        panel.grid.major.x = element_blank(),
...
ggsave("4.7B_ImportancePartnersApplication_LMI_percent_Dec2021.svg", plot = f4.7b, device = "svg", path = "figures/LMI", height = 7, width  = 13 , units = "in", dpi = 500)
...
#pdf(file="figures/LMI/4.8A_LevelsOfPartnership_LMI.pdf", width = 11, height= 7)
#svg(file="figures/LMI/4.8A_LevelsOfPartnership_LMI.svg", width = 11, height= 7)
...
ggplot(part.level, aes(x=Var2, y=Freq, fill = Var1),  ylim(0,375)) + 
  ...
  labs(x="", y="Number of respondents", fill="", title="Levels of partnerships outside of academia (LMI)")+
  theme(panel.grid.major.y = element_line(linetype = 3, color = "darkgrey", size = .3),
        panel.grid.major.x = element_blank(),
        panel.grid.minor = element_blank(),
  ...
  colScale7+scale_y_continuous(expand = c(0, 0), limits = c(0,150))
...
#pdf(file="figures/LMI/4.8A_LevelsOfPartnership_LMI_percent.pdf", width = 9.5, height= 11)
#pdf(file="figures/LMI/4.8A_LevelsOfPartnership_LMI_percent_wide.pdf", width = 10, height= 6)
#svg(file="figures/LMI/4.8A_LevelsOfPartnership_LMI.svg", width = 11, height= 7)

f4.8a.lmi <- ggplot(partlevel.perc.l, aes(x=level, y=percent, fill = year),  ylim(0,375)) + 
  geom_bar( stat='identity', position="dodge") +
  #geom_text(aes(label=Freq), position=position_dodge(width=1), vjust=-0.5, size=3) +
  labs(x="", y="Percent of responses", fill="", title="Levels of partnerships outside of academia (LMI)")+
...
ggsave("4.8A_LevelsOfPartnership_LMI_percent_wide_Dec2021.svg", plot = f4.8a.lmi, device = "svg", path = "figures/LMI", height = 7, width  = 13 , units = "in", dpi = 500)
...
g1<-ggplot(data=part.change, aes(x=Var1, y=Freq, fill=Var1))+ 
    ...
    guides(fill=FALSE) + 
    labs(x="", y="Number of respondents",title="Change in level of partnerships (LMI)")+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,141), breaks = seq(0,140, 20))
...
g2<-ggplot(change.part, aes(x=Var1, y=Freq, fill=Var1)) + 
  ...
  colScale+
  scale_y_continuous(expand = c(0, 0), limits = c(0,71), breaks = seq(0,70,10))

#annotate('text', 5,420, label="(b)", size=7)+ 

#pdf(file="figures/LMI/4.8B_PartnershipChange&Reason_LMI.pdf", width = 10, height= 17)
#svg(file="figures/LMI/4.8B_PartnershipChange_LMI.svg", width = 11, height= 7)

f4.8b.1.lmi <- grid.arrange(g1, g2, ncol=1)

#dev.off()

ggsave("4.8B_PartnershipChange&Reason_LMI_Dec2021.svg", plot = f4.8b.1.lmi, device = "svg", path = "figures/LMI", height = 14, width  = 13 , units = "in", dpi = 500)


#pdf(file="figures/LMI/4.8B_PartnershipChange&Reason_LMI_wide.pdf", width = 25, height= 10)
#svg(file="figures/LMI/4.8B_PartnershipChange_LMI.svg", width = 11, height= 7)

f4.8b.2.lmi <- grid.arrange(g1, g2, nrow=1)

#dev.off()

ggsave("4.8B_PartnershipChange&Reason_LMI_wide_Dec2021.svg", plot = f4.8b.2.lmi, device = "svg", path = "figures/LMI", height = 7, width  = 26 , units = "in", dpi = 500)
...
#pdf(file="figures/LMI/4.8C_ViewChangePartnership_LMI.pdf", width = 10, height= 12)
#svg(file="figures/LMI/4.8C_ViewChangePartnership_LMI.svg", width = 11, height= 7)

f4.8c.lmi <- ggplot(view, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+
  #geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  guides(fill=FALSE)+
  theme(panel.grid.major.y = element_line(linetype = 3, color = "darkgrey", size = .3),
        panel.grid.major.x = element_blank(),
...
ggsave("4.8C_ViewChangePartnership_LMI_Dec2021.svg", plot = f4.8c.lmi, device = "svg", path = "figures/LMI", height = 7, width  = 13 , units = "in", dpi = 500)
...
p1<-p1+scale_fill_manual(name = "ID",values = "#42A3FF")+labs(x="", title="Anticipated change in research funding (LMI)", y="Number of responses")+ theme(plot.margin=unit(c(1,1,-0.45,1), "cm"), plot.title = element_text(hjust = 0.5, size = 28), strip.text.x = element_text(size=24, color='#3b92e5'))+ scale_y_continuous(expand=c(0,0), limits = c(0,101), breaks = seq(0,100,20))

p2<-p2+scale_fill_manual(name = "ID",values ="#9BC53D")+ theme(plot.margin=unit(c(-0.5,1,1,1), "cm"), strip.text.x = element_text(size=24, color='#7c9d30')) + scale_y_continuous(expand=c(0,0), limits = c(0,101), breaks = seq(0,100,20)) + labs(x="  ", y="Number of responses", title="")

p3<-p3+scale_fill_manual(name = "ID",values ="#E55934")+ theme(plot.margin=unit(c(-0.5,1,0.5,1), "cm"), strip.text.x = element_text(size=24, color='#ce502e'))+ scale_y_continuous(expand=c(0,0), limits = c(0,101), breaks = seq(0,100,20)) + labs(x = "", y = "Number of responses")





#pdf(file="figures/LMI/4.9_AnticipatedChangeFunding_LMI.pdf", height=19, width =12)
#tiff(file="figures/LMI/4.9_AnticipatedChangeFunding_LMI.tiff", height=17, width =10, units = "in", res = 600)
#svg(file="figures/LMI/4.9_AnticipatedChangeFunding_LMI.svg", width = 11, height= 7)

f4.9.lmi <- grid.arrange(p1,p2,p3, heights=c(0.33,0.33,0.33))

#dev.off()

ggsave("4.9_AnticipatedChangeFunding_LMI_Dec2021.svg", plot = f4.9.lmi, device = "svg", path = "figures/LMI", height = 21, width  = 13 , units = "in", dpi = 500)
...
#pdf(file="figures/LMI/4.10_EffectNextGeneration_LMI.pdf", height=7, width =11)
#tiff(file="figures/LMI/4.10_EffectNextGeneration_LMI.tiff", height=11, width =10, units = "in", res = 600)
#svg(file="figures/LMI/4.10_EffectNextGeneration_LMI.svg", width = 11, height= 7)

f4.10.lmi <- ggplot(data=future, aes(x=Var1, Freq, fill=Var1))+ 
    geom_bar(stat='identity', position="dodge")  +
        scale_x_discrete(labels=c("Increase\nconsiderably", "Increase\nslightly", 
                                    "Stay the\nsame", "Decrease\nslightly", "Decrease\nconsiderably", "Can't\ncomment"))+
    guides(fill=FALSE) +
 labs(x="", y="Number of respondents", title="Effect of funding changes on the next\ngeneration of researchers (LMI)")+
  theme(axis.text.x=element_text(size=22, color = "black"),
         axis.text.y=element_text(size = 22, color = "black"),
       panel.grid.major.y = element_line(linetype = 3, color = "darkgrey", size = .3),
        panel.grid.major.x = element_blank(),
        ...
        plot.title = element_text(hjust = 0.5, size = 28))+
  #geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,62))+colScale

#dev.off()

ggsave("4.10_EffectNextGeneration_LMI_Dec2021.svg", plot = f4.10.lmi, device = "svg", path = "figures/LMI", height = 7, width  = 13 , units = "in", dpi = 500)
