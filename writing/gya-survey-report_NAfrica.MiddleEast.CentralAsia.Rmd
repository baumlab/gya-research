---
title: "North Africa, Middle East, and Central Asia"
author: ''
date: "8 March 2018"
output:
  pdf_document: null
...
##selecting for north africa, middle east, and central asian countries
na_me_ca<-read.csv("../data/nafrica_middleeast_centralasia.csv")
na_me_ca <- as.data.frame(na_me_ca)
na_me_ca <- na_me_ca[ , 1]

na.me.ca<-survey[survey$nation%in%na_me_ca,]

#check
unique(na.me.ca$nation)
na.me.ca <- droplevels(na.me.ca)


#Gender:
n.na.me.ca.res.men <- na.me.ca[na.me.ca$gender=="Male",]               #Count how many responses from  male researchers
percent.men<-((dim(n.na.me.ca.res.men)[1])/(dim(na.me.ca)[1]))*100      #Calculate the % of responses from  male researchers
n.na.me.ca.res.women <- na.me.ca[na.me.ca$gender=="Female",]           #Count how many responses from  female researchers
percent.women<-((dim(n.na.me.ca.res.women)[1])/(dim(na.me.ca)[1]))*100  #Calculate the % of responses from  female researchers

#Career Stage Tabulations:
#table(na.me.ca$what_participant_group)
n.seniorac <- na.me.ca[na.me.ca$what_participant_group=="Senior academic researcher with >10 years of experience applying for research grants",]
percent.seniorac<-((dim(n.seniorac)[1])/(dim(na.me.ca)[1]))*100 #Calculate the % of responses from  senior academic researchers
n.earlyac <- na.me.ca[na.me.ca$what_participant_group=="Early career academic researcher with <10 years experience applying for research grants since completion of PhD",]
percent.earlyac<-((dim(n.earlyac)[1])/(dim(na.me.ca)[1]))*100 #Calculate the % of responses from  early academic researchers
n.pdf <- na.me.ca[na.me.ca$what_participant_group=="Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future",]
percent.pdf<-((dim(n.pdf)[1])/(dim(na.me.ca)[1]))*100 #Calculate the % of responses from  post-docs
n.nonACearly <- na.me.ca[na.me.ca$what_participant_group=="Non-academic researcher conducting or managing research in industry or government with <10 years of experience",]
percent.nonACearly<-((dim(n.nonACearly)[1])/(dim(na.me.ca)[1]))*100 #Calculate the % of responses from early career non-academics
n.nonACsenior <- na.me.ca[na.me.ca$what_participant_group=="Non-academic researcher conducting or managing research in industry or government with >10 years of experience",]
percent.nonACsenior<-((dim(n.nonACsenior)[1])/(dim(na.me.ca)[1]))*100 #Calculate the % of responses from early career non-academics
n.unk <- na.me.ca[na.me.ca$what_participant_group=="",]
percent.unk<-((dim(n.unk)[1])/(dim(na.me.ca)[1]))*100 #Calculate the % of responses from unknown career stage

#Discipline Tabulations:
n.natural <- na.me.ca[na.me.ca$field_research=="Natural Science",]
percent.natural <- ((dim(n.natural)[1])/(dim(na.me.ca)[1]))*100 #Calculate the % of responses from  natural scientists
n.physics <- na.me.ca[na.me.ca$field_research=="Physical Science (eg. math, physics, chemistry, computer science)",]
percent.physics <- ((dim(n.physics)[1])/(dim(na.me.ca)[1]))*100 #Calculate the % of responses from  physical scientists
n.med <- na.me.ca[na.me.ca$field_research=="Medicine and Life Science",]
percent.med <- ((dim(n.med)[1])/(dim(na.me.ca)[1]))*100 #Calculate the % of responses from  med/life scientists
n.eng <- na.me.ca[na.me.ca$field_research=="Engineering",]
percent.eng <- ((dim(n.eng)[1])/(dim(na.me.ca)[1]))*100 #Calculate the % of responses from  engineering scientists
n.int <- na.me.ca[na.me.ca$field_research=="Interdisciplinary Science",]
percent.int <- ((dim(n.int)[1])/(dim(na.me.ca)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists
n.ssh <- na.me.ca[na.me.ca$field_research=="Social Science / Humanities",]
percent.ssh <- ((dim(n.ssh)[1])/(dim(na.me.ca)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists
...
locations<-aggregate(gender ~ nation, na.me.ca, length)
locations<-locations[!locations$nation=="",]
locations<-droplevels(locations)

#pdf(file="figures/n.afr_mid.east_cen.asia/responses_country_12Mar18.pdf", width = 11, height= 8)
...
ggplot(locations, aes(x = reorder(nation, -gender), gender)) + geom_bar(stat='identity',position = position_dodge(width=0.5),fill='#1f78b4') + theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5)) + labs(y="Number of responses", x="", title="Responses by Country")  + 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+scale_y_continuous(expand = c(0,0), limits=c(0, 200)) 
...
#pdf(file="figures/n.afr_mid.east_cen.asia/responses_country_gapplot_12Mar18.pdf", height=8, width=11)
...
locations.more<-aggregate(gender ~ nation, na.me.ca, length)
...
#pdf(file="figures/n.afr_mid.east_cen.asia/responses_country_10+_gapplot_12Mar18.pdf", height=8, width=11)
...
locations.less<-aggregate(gender ~ nation, na.me.ca, length)
...
color1<-c('#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4')
#pdf(file="figures/n.afr_mid.east_cen.asia/responses_country_10+&10-_gapplot_12Mar18.pdf", height=8, width=15)
...
experience<-data.frame(table(na.me.ca$what_participant_group))
...
#pdf(file="figures/n.afr_mid.east_cen.asia/4.1_CareerStage_11March18.pdf", width = 11, height= 7)
...
field<-data.frame(table(na.me.ca$field_research))
...
#pdf(file="figures/n.afr_mid.east_cen.asia/4.2_FieldofResearch_11March18.pdf", width = 11, height= 7)
...
#pdf(file="figures/n.afr_mid.east_cen.asia/4.1and4.2_CareerStageAndFieldOfResearch.pdf", width = 11, height= 7)
...
#select for just north africa etc
research.na.me.ca<-research[research$nation%in%na_me_ca,]
research.na.me.ca <- droplevels(research.na.me.ca)

## make sure that Use-inspired is the middle level in the legend
res.plot.na.me.ca<-subset(research.na.me.ca, type%in%c("percent_fundemental_research_current",  "percent_Applied_Research_current" , "percent_Use_inspired_Research_current","percent_Applied_Research_past",  "percent_Fundamental_Research_past" , "percent_Use_inspired_Research_past"))

res.plot.na.me.ca$percent<-str_replace_all(res.plot.na.me.ca$percent, "%", "")
res.plot.na.me.ca$percent<-as.numeric(as.character(res.plot.na.me.ca$percent))
res.plot.na.me.ca$type<-as.character(res.plot.na.me.ca$type)

# add time variable
res.plot.na.me.ca$time<-ifelse(grepl( "current", res.plot.na.me.ca$type),"Current", "Past")
res.plot.na.me.ca$type[res.plot.na.me.ca$type=="percent_Applied_Research_past"]<-"Applied"
res.plot.na.me.ca$type[res.plot.na.me.ca$type=="percent_Applied_Research_current"]<-"Applied"
res.plot.na.me.ca$type[res.plot.na.me.ca$type=="percent_Fundamental_Research_past"]<-"Fundamental"
res.plot.na.me.ca$type[res.plot.na.me.ca$type=="percent_fundemental_research_current"]<-"Fundamental"
res.plot.na.me.ca$type[res.plot.na.me.ca$type=="percent_Use_inspired_Research_past"]<-"Use-inspired"
res.plot.na.me.ca$type[res.plot.na.me.ca$type=="percent_Use_inspired_Research_current"]<-"Use-inspired"

# change order of the levels
res.plot.na.me.ca$time<-as.factor(res.plot.na.me.ca$time)
res.plot.na.me.ca$time<-factor(res.plot.na.me.ca$time, levels(res.plot.na.me.ca$time)[c(2,1)])

res.plot.na.me.ca$type<-as.factor(res.plot.na.me.ca$type)
res.plot.na.me.ca$type<-factor(res.plot.na.me.ca$type, levels(res.plot.na.me.ca$type)[c(2,3,1)])

res.plot.na.me.ca$time<-factor(res.plot.na.me.ca$time)
res.plot.na.me.ca$time<-factor(res.plot.na.me.ca$time, levels(res.plot.na.me.ca$time)[c(1,2)])
...
res.plot.na.me.ca$ID<-paste(res.plot.na.me.ca$type, res.plot.na.me.ca$time)

#table(res.plot.na.me.ca$percent,res.plot.na.me.ca$time,res.plot.na.me.ca$type)
#write.csv(res.plot.na.me.ca, file="data/4.1a.calculations.csv", row.names = FALSE)

# change order of the levels
res.plot.na.me.ca$ID<-as.factor(res.plot.na.me.ca$ID)
res.plot.na.me.ca$ID<-factor(res.plot.na.me.ca$ID, levels(res.plot.na.me.ca$ID)[c(4,3,6,5,2,1)])

# add colours to res.plot
mycols<-c('#33a02c','#a6cee3','#fb9a99','#1f78b4', '#e31a1c','#b2df8a')
mycols<-as.data.frame(mycols)
mycols$ID<-levels(res.plot.na.me.ca$ID)
res.plot.na.me.ca$col<-mycols$mycols[match(res.plot.na.me.ca$ID, mycols$ID)]
res.plot.na.me.ca$col<-factor(res.plot.na.me.ca$col, levels(res.plot.na.me.ca$col)[c(3,1,4,2,6,5)])
mycols<-as.character(unique(res.plot.na.me.ca$col))
...
#table(res.plot.na.me.ca$type, res.plot.na.me.ca$time, res.plot.na.me.ca$col)

#pdf(file="figures/n.afr_mid.east_cen.asia/4.3.1_ResearchProportion_11March18.pdf", width = 11, height= 7)

ggplot(res.plot.na.me.ca, aes(percent, fill=ID)) +
  ...
  scale_x_continuous(breaks=seq(0,100,10))+scale_y_continuous(expand=c(0,0), limits=c(0,40),breaks=c(0,10, 20, 30, 40))

#dev.off()
```

```{r echo=FALSE, fig.cap="Figure 4.3 (different layout) Respondents type of research describe in proportal amounts of fundamental,  use-inspired and applied research. Reseachers were questioned about the percentage of funding allocated to Fundamental, Use-inspired or Applied research in the past and in their current research.", fig.height=7, fig.width=11, warning=FALSE, message=FALSE}
...
res.plot.na.me.ca$type[res.plot.na.me.ca$type=="percent_Use_inspired_Research_past"]<-"Use-inspired"
res.plot.na.me.ca$type[res.plot.na.me.ca$type=="percent_Use_inspired_Research_current"]<-"Use-inspired"
res.plot.na.me.ca$time[res.plot.na.me.ca$time=="Current"]<-"2011-2015"
res.plot.na.me.ca$time[res.plot.na.me.ca$time=="Past"]<-"2006-2010"
...
mycols$ID<-levels(res.plot.na.me.ca$ID)
res.plot.na.me.ca$col<-mycols$mycols[match(res.plot.na.me.ca$ID, mycols$ID)]
res.plot.na.me.ca$col<-factor(res.plot.na.me.ca$col, levels(res.plot.na.me.ca$col)[c(3,5,4,2,6,1)])
mycols<-as.character(unique(res.plot.na.me.ca$col))
colScale <- scale_fill_manual(name = "ID",values =mycols, labels=c("2006-2010", "2011-2015"))


#ggplot(res.plot.na.me.ca, aes(percent, fill=time)) +
#    geom_histogram(position='dodge', binwidth=10, bins=seq(0, 100, 10))  +
#    guides(fill=guide_legend(title=NULL, reverse=FALSE, nrow=2)) +
...
library(gridExtra)
#####add (a)
xs <- split(res.plot.na.me.ca,f = res.plot.na.me.ca$type)
...
#pdf(file="figures/n.afr_mid.east_cen.asia/4.3.2_ResearchProportion_11March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.4a&b Change in research type proportions and the reasons. Researchers were asked to answer yes, no, or can't comment on if their type of research had changed in the last 10 years and to select what reasons for the change applied to them.", fig.height=7, fig.width=11, warning=FALSE, message=FALSE}

#select for just north africa etc
research.change.na.me.ca<-research.change[research.change$nation%in%na_me_ca,]
research.change.na.me.ca <- droplevels(research.change.na.me.ca)

#Change yes or no
yes.no.na.me.ca<-subset(research.change.na.me.ca, select=c("Location","Country", "Country_work",  "gender", "changed_10yrs"))

mid.e.yes.no<-aggregate(Country~ changed_10yrs, yes.no.na.me.ca, length)
mid.e.yes.no<-mid.e.yes.no[!mid.e.yes.no$changed_10yrs=="",]

p1<-ggplot(data=mid.e.yes.no, aes(x=changed_10yrs, y=Country, fill=Country))+ geom_bar(stat='identity') + guides(fill=FALSE)+
  theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(), axis.title.x=element_blank(), plot.title = element_text(hjust = 0.5))+
  geom_text(aes(label=Country), vjust=-0.5)+labs(y="Number of responses", title="Change in research type")+ scale_y_continuous(expand = c(0, 0), limits = c(0,145))+ annotate('text', 0.5,1470, label="(a)", size=5)

####reason for change  
reason.na.me.ca<-subset(research.change.na.me.ca, select=c("Location","Country","Country_work",  "gender", "Main_reason_change_interest_related",
...
require(tidyr)
mid.e.long<-gather(reason.na.me.ca, reason.change, yes, -Location, -gender, -Country_work, -Country)
# using table to count cases of each category
sum.mid.e<-data.frame(table(mid.e.long$reason.change, mid.e.long$yes))
#remove non-response
sum.mid.e<-sum.mid.e[!sum.mid.e$Var1=="",]

sum.mid.e$Var1<-revalue(sum.mid.e$Var1, c("Main_reason_change_Career_related" = "Career", "Main_reason_change_Funding_related" = "Funding", "Main_reason_change_interest_related" = "Interest", "Main_reason_change_Other" = "Other", "Main_reason_change_Socially_related"  = "Social"))
# change order of the levels
sum.mid.e$Var1<-factor(sum.mid.e$Var1,levels(sum.mid.e$Var1)[c(2,3,1,5,4)])

myColors4<-(brewer.pal(9, "Blues"))
myColors4<-myColors4[c(9, 8,7,6,5,3)]

p2<-ggplot(sum.mid.e, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 60))+annotate('text', 5.4,885, label="(b)", size=5)+geom_text(aes(label=Freq), vjust=-0.5, size=3)


#pdf(file="figures/n.afr_mid.east_cen.asia/4.4_ReasonForFieldChange.pdf", width = 11, height= 7)

#print(p2)

#dev.off()

#pdf(file="figures/n.afr_mid.east_cen.asia/4.4a&b_ReasonForFieldChange_11March18.pdf", width = 11, height= 7)
...
view.propor<-na.me.ca[!na.me.ca$view_change_of_type=="",]
...
#pdf(file="figures/n.afr_mid.east_cen.asia/4.5_ViewFieldChange_11march18.pdf", width = 11, height= 7)
...
b.part<-data.frame(table(na.me.ca$partnership_outside_before))
b.part<-b.part[!b.part$Var1=="",]
b.part<-droplevels(b.part)
b.part$time<-"Past"
a.part<-data.frame(table(na.me.ca$partnership_outside))
...
#pdf(file="figures/n.afr_mid.east_cen.asia/4.6_LevelsOfPartnership_11March18.pdf", width = 11, height= 7)
...
ggplot(part, aes(x=Var1, y=Freq, fill = time),  ylim(0,375)) + 
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,150))
...
change.part.glb<-data.frame(table(na.me.ca$partnership_change_10yrs))
...
#select for just north africa etc
part2.reason.na.me.ca<-part2.reason[part2.reason$nation%in%na_me_ca,]
part2.reason.na.me.ca <- droplevels(part2.reason.na.me.ca)

reason.pt.long.mid.e<-gather(part2.reason.na.me.ca, change.reason, yes, -Country,-Country_work, -gender, -nation, -Location, -what_participant_group, -field_research)
sum.mid.e<-data.frame(table(reason.pt.long.mid.e$change.reason, reason.pt.long.mid.e$yes))

sum.mid.e$Var1<-revalue(sum.mid.e$Var1, c("reason_partnership_change_career" = "Career", "reason_partnership_change_funding" = "Funding", "reason_partnership_change_interest" = "Interest", "reason_partnership_change_other" = "Other", "reason_partnership_change_socially"  = "Social"))
sum.mid.e$Var1<-reorder(sum.mid.e$Var1, -sum.mid.e$Freq)

myColors7<-(brewer.pal(9, "Blues"))
myColors7<-myColors7[c(9, 8,7,6,5,3)]

g2<-ggplot(sum.mid.e, aes(x=Var1, y=Freq, fill=Var1)) + 
...
#pdf(file="figures/n.afr_mid.east_cen.asia/4.7a&b_PartnershipChange&Reason_11March18.pdf", width = 11, height= 7)
...
view<-data.frame(table(na.me.ca$view_change_partnership))
view<-view[!view$Var1=="",]

# change order of the levels

view$Var1<-factor(view$Var1, levels(view$Var1)[c(5, 4,2,3,1)])

myColors8<-(brewer.pal(9, "Blues"))
myColors8<-myColors8[c(4,4,4,4,4)]
colScale <- scale_fill_manual(name = "Var1",values = myColors8)

#pdf(file="figures/n.afr_mid.east_cen.asia/4.8_ViewChangePartnership_11March18.pdf", width = 11, height= 7)
...
ggplot(view, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+
  ...
  labs(x="", y="Number of responses", title="Opinion of change in partnership level") + colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,40), breaks = seq(0,40,10))

#dev.off()

```
 
```{r eval=FALSE, fig.cap="Figure ( 4.6 + 4.7 + 4.8 ) External Research Partnerships. ", message=FALSE, warning=FALSE,fig.width=11, fig.height=7, include=FALSE}
###4.6
myColors6<-(brewer.pal(9, "Blues"))
myColors6<-rev(myColors6[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors6)
...
#select for just north africa etc
part3.grants.long.mid.e<-part3.grants.long[part3.grants.long$nation%in%na_me_ca,]
part3.grants.long.mid.e <- droplevels(part3.grants.long.mid.e)

part3.grants.long.mid.e$type.grant<-as.character(part3.grants.long.mid.e$type.grant)


#add time variable
part3.grants.long.mid.e$time<-ifelse(grepl("11_15", part3.grants.long.mid.e$type.grant), "2011-2015", "2006-2010")
part3.grants.long.mid.e$type.grant[part3.grants.long.mid.e$type.grant=="external_pi_grant_11_15_fundamental"]<-"Fundamental"
part3.grants.long.mid.e$type.grant[part3.grants.long.mid.e$type.grant=="external_pi_grant_6_10_fundamental"]<-"Fundamental"
part3.grants.long.mid.e$type.grant[part3.grants.long.mid.e$type.grant=="external_pi_grant_11_15_use"]<-"Use-Inspired"
part3.grants.long.mid.e$type.grant[part3.grants.long.mid.e$type.grant=="external_pi_grant_6_10_use"]<-"Use-Inspired"
part3.grants.long.mid.e$type.grant[part3.grants.long.mid.e$type.grant=="external_pi_grant_11_15_applied"]<-"Applied"
part3.grants.long.mid.e$type.grant[part3.grants.long.mid.e$type.grant=="external_pi_grant_6_10_applied"]<-"Applied"

grants.mid.e<-data.frame(with(part3.grants.long.mid.e, table(type.grant,time,number)))
grants.mid.e$type.grant<-as.factor(grants.mid.e$type.grant)


#change order of levels
grants.mid.e$number<-factor(grants.mid.e$number, levels(grants.mid.e$number)[c(1,2,6,7,3,4,5)])
grants.mid.e$type.grant<-factor(grants.mid.e$type.grant, levels(grants.mid.e$type.grant)[c(2,3,1)])

#myColors9<-(brewer.pal(9, "Blues"))
#myColors9<-rev(myColors9[c(9, 5)])
#colScale <- scale_fill_manual(name = "time",values = myColors9)

grants.mid.e$ID<-paste(grants.mid.e$type.grant, grants.mid.e$time)

# change order of the levels
grants.mid.e$ID<-as.factor(grants.mid.e$ID)
grants.mid.e$ID<-factor(grants.mid.e$ID, levels(grants.mid.e$ID)[c(4,3,6,5,2,1)])

##colors
mycols<-c('#33a02c','#a6cee3','#fb9a99','#1f78b4', '#e31a1c','#b2df8a')
#mycols<-as.data.frame(mycols)

#pdf(file="figures/n.afr_mid.east_cen.asia/4.9_NumberGrantApps_11March18.pdf", height=7, width =11)


#####add (a)
xs <- split(grants.mid.e,f = grants.mid.e$type.grant)
...
```{r echo=FALSE, fig.cap="Fig 4.10 Research grant application success over the past 10 years. Researchers were asked to estimate the percentage of their research grant applications that were successful, in 2006-2010 and in 2011-2015. Respondents also had the choice to answer No need for applications for this research type.", fig.height=7, fig.width=11, message=FALSE}
part3.success.long<-read.csv(file="../data/gya-part3.success.long.csv")

#select for just north africa etc
part3.success.long.mid.e<-part3.success.long[part3.success.long$nation%in%na_me_ca,]
part3.success.long.mid.e <- droplevels(part3.success.long.mid.e)

part3.success.long.mid.e$type<-as.character(part3.success.long.mid.e$type)

#add time variable
part3.success.long.mid.e$time<-ifelse(grepl("11_15", part3.success.long.mid.e$type), "2011-2015", "2006-2010")
part3.success.long.mid.e$type[part3.success.long.mid.e$type=="successful_grants_11_15_fundamental"]<-"Fundamental"
part3.success.long.mid.e$type[part3.success.long.mid.e$type=="successful_grants_6_10_fundamental"]<-"Fundamental"
part3.success.long.mid.e$type[part3.success.long.mid.e$type=="successful_grants_11_15_use"]<-"Use-Inspired"
part3.success.long.mid.e$type[part3.success.long.mid.e$type=="successful_grants_6_10_use"]<-"Use-Inspired"
part3.success.long.mid.e$type[part3.success.long.mid.e$type=="successful_grants_11_15_applied"]<-"Applied"
part3.success.long.mid.e$type[part3.success.long.mid.e$type=="successful_grants_6_10_applied"]<-"Applied"

success.mid.e<-data.frame(with(part3.success.long.mid.e, table(type, percent,time)))

#change order of types
success.mid.e$type<-as.factor(success.mid.e$type)
success.mid.e$type<-factor(success.mid.e$type, levels(success.mid.e$type)[c(2,3,1)])

success.mid.e$id<-paste(success.mid.e$type, success.mid.e$time)

# change order of the levels
success.mid.e$id<-as.factor(success.mid.e$id)
success.mid.e$id<-factor(success.mid.e$id, levels(success.mid.e$id)[c(3,4,5,6,1,2)])

mycols10<-c('#1f78b4','#a6cee3','#33a02c','#b2df8a','#e31a1c', '#fb9a99')
mycols10<-as.data.frame(mycols10)
mycols10$id<-levels(success.mid.e$id)
success.mid.e$col<-mycols10$mycols10[match(success.mid.e$id, mycols10$id)]
success.mid.e$col<-factor(success.mid.e$col, levels(success.mid.e$col)[c(3,1,4,2,6,5)])

#pdf(file="figures/n.afr_mid.east_cen.asia/4.10_GrantAppSuccess_11March18.pdf", height=7, width =11)

out2<-by(data = success.mid.e, INDICES = success.mid.e$type, FUN = function(n){
  n <- droplevels(n)
...
```{r echo=FALSE, fig.cap="Fig 4.11 Change in grant success rates over the past 10 years. Researchers were asked if they thought that grant sucecss rates have changed in the past 10 years, for each research category.", message=FALSE, fig.height=7, fig.width=11}

#select for just north africa etc
part3.change.mid.e<-part3.change[part3.change$nation%in%na_me_ca,]
part3.change.mid.e <- droplevels(part3.change.mid.e)

##switch to long format
change.long.mid.e<-gather(part3.change.mid.e, type, level, -Location, -gender, -Country,-nation, -Country_work, -what_participant_group, -field_research)
success.change<-with(change.long.mid.e, data.frame(table(type, level)))
#remove no response
...
#pdf(file="figures/n.afr_mid.east_cen.asia/4.11_ChangeSuccessRate_11March18.pdf", height=7, width =11)
...
```{r echo=FALSE, fig.cap="Fig 4.12 Importance of practical application of research over the past 10 years. Researchers were asked how important it was to suggest practical applications of their research to ensure that the grant was successful, in 2006-2010 and in 2011-2015.", fig.height=7, fig.width=11, message=FALSE}

#select for just north africa etc
part3.prac.long.mid.e<-part3.prac.long[part3.prac.long$nation%in%na_me_ca,]
part3.prac.long.mid.e <- droplevels(part3.prac.long.mid.e)

part3.prac.long.mid.e$year<-as.character(part3.prac.long.mid.e$year)

part3.prac.long.mid.e$year[part3.prac.long.mid.e$year=="practical_applications_important_11_15"]<-"2011-2015"
part3.prac.long.mid.e$year[part3.prac.long.mid.e$year=="practical_applications_important_6_10"]<-"2006-2010"


prac.mid.e<-data.frame(table(part3.prac.long.mid.e$year, part3.prac.long.mid.e$level))

#change order of levels
prac.mid.e$Var2<-factor(prac.mid.e$Var2, levels(prac.mid.e$Var2)[c(2,6,4,5,3,1)])

myColors12<-(brewer.pal(9, "Blues"))
myColors12<-rev(myColors12[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors12)

#pdf(file="figures/n.afr_mid.east_cen.asia/4.12_ImportancePracticalApplication_11March18.pdf", height=7, width =11)

ggplot(prac.mid.e, aes(Var2, Freq, fill=Var1)) + 
...
#select for just north africa etc
part3.part.long.mid.e<-part3.part.long[part3.part.long$nation%in%na_me_ca,]
part3.part.long.mid.e <- droplevels(part3.part.long.mid.e)

part3.part.long.mid.e$year<-as.character(part3.part.long.mid.e$year)

part3.part.long.mid.e$year[part3.part.long.mid.e$year=="include_nonacademia_partners_success_11_15"]<-"2011-2015"
part3.part.long.mid.e$year[part3.part.long.mid.e$year=="include_nonacademia_partners_success_6_10"]<-"2006-2010"

part.mid.e<-data.frame(table(part3.part.long.mid.e$year, part3.part.long.mid.e$level))
#change order of levels
part.mid.e$Var2<-factor(part.mid.e$Var2, levels(part.mid.e$Var2)[c(2,5,6,4,3,1)])
...
#pdf(file="figures/n.afr_mid.east_cen.asia/4.13_ImportancePartnersApplication_11March18.pdf", height=7, width =11)

ggplot(part.mid.e, aes(Var2, Freq, fill=Var1))+ 
...
#select for just north africa etc
p3_master.long.mid.e<-p3_master.long[p3_master.long$nation%in%na_me_ca,]
p3_master.long.mid.e <- droplevels(p3_master.long.mid.e)

##revalue the year values
p3_master.long.mid.e$year<- revalue(p3_master.long.mid.e$year, c("11_15"="2011-2015", "6_10"="2006-2010"))
#change order of levels
p3_master.long.mid.e$year<-factor(p3_master.long.mid.e$year, levels(p3_master.long.mid.e$year)[c(2,1)])

##For-Profit
fp<-p3_master.long.mid.e[p3_master.long.mid.e$type=="For-Profit",]
...
p1<-ggplot(fp, aes(percent, fill=year))+ geom_histogram(position='dodge', binwidth=10)  + colScale +
  ...
  scale_y_continuous(expand =c(0,0),limits=c(0,240), breaks=seq(0, 240, 50))+
  scale_x_continuous( breaks = seq(0, 100, 10))


##Government
gov<-p3_master.long.mid.e[p3_master.long.mid.e$type=="Government",]
...
int<-p3_master.long.mid.e[p3_master.long.mid.e$type=="Internal",]
...
ngov<-p3_master.long.mid.e[p3_master.long.mid.e$type=="Non-governmental",]
...
other<-p3_master.long.mid.e[p3_master.long.mid.e$type=="Other",]
...
#pdf(file="figures/n.afr_mid.east_cen.asia/4.14_DistributionFunding_11March18.pdf", height=7, width =11)
...
#select for just north africa etc
part4.mid.e<-part4[part4$nation%in%na_me_ca,]
part4.mid.e <- droplevels(part4.mid.e)

## order bars from very important (left) to can't comment (right)
important<-subset(part4.mid.e, select=c("Location","Country",'Country_work', "gender","opinion_fundamental_important"))
...
#pdf(file="figures/n.afr_mid.east_cen.asia/4.15_PerceivedImportanceFundamentalGovernment_11march18.pdf", height=7, width =11)
...
ggplot(data=perceive, aes(x=Var1, y=Freq, fill=Var1)) + 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,100)) + colScale
...
priority<-subset(part4.mid.e, select=c("Location","Country",'Country_work', "gender", "high_priority_fundamental", "high_priority_use_inspired", "high_priority_applied", "high_priority_no_change"))
...
#pdf(file="figures/n.afr_mid.east_cen.asia/4.16_PerceivedChangeResearchPriorityGovernment.pdf", height=7, width =11)
...
ggplot(data=per.change, aes(x=what.type, higher.priority, fill=what.type))+
  ...
  geom_text(aes(label=higher.priority, vjust=-0.5), size=3) +scale_y_continuous(expand = c(0, 0), limits = c(0,160))+ colScale
...
availability.change<-subset(part4.mid.e, select=c("Location", "Country",'Country_work', "gender", "available_funding_fundamental",  
                                             "available_funding_use_inspired", "available_funding_applied"))
## switch to long format
availability.change.long.mid.e<-gather(availability.change, what.type, level, -gender, -Location, -Country, -Country_work)
availability.mid.e<-with(availability.change.long.mid.e, data.frame(table(what.type, level)))

#remove non response
availability.mid.e<-availability.mid.e[!availability.mid.e$level=="",]

## change type to factor with levels
availability.mid.e$level<-str_replace_all(availability.mid.e$level, "Will ", "")
availability.mid.e$what.type<-as.factor(availability.mid.e$what.type)
availability.mid.e$level<-as.factor(availability.mid.e$level)

## change order of levels
availability.mid.e$level<-factor(availability.mid.e$level, levels(availability.mid.e$level)[c(4,5,6,3,2,1 )])
availability.mid.e$what.type<-factor(availability.mid.e$what.type, levels(availability.mid.e$what.type)[c(2,3,1)])

availability.mid.e$what.type<-revalue(availability.mid.e$what.type, c("available_funding_fundamental"="Fundamental",
...
#pdf(file="figures/n.afr_mid.east_cen.asia/4.17_AnticipatedChangeFunding_11March18.pdf", height=7, width =11)

ggplot(data=availability.mid.e, aes(x=level, Freq, fill=what.type))+
    geom_bar(stat="identity", position="dodge")+
...
## order bars into increase considerably (left) to decrease considerably (right), then no comment at the far right
next.generation<-subset(part4.mid.e, select=c("Location", "Country","Country_work", "gender","next_generation"))
# remove non-response
...
#pdf(file="figures/n.afr_mid.east_cen.asia/4.18_EffectNextGeneration_11March18.pdf", height=7, width =11)
...
ggplot(data=impact, aes(x=Var1, Freq, fill=Var1))+ 
 ...
 labs(x="", y="Number of responses", title="Effect of funding changes on the next generation of researchers")+
  ...
  geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,70))+colScale
