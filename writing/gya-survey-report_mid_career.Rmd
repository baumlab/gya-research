...
##selecting for mid career stage respondents
...
gbl.mid<-survey[(survey$what_participant_group%in%c("Early career academic researcher" ,"Non-academic researcher <10 yrs experience")),]

#check
unique(gbl.mid$what_participant_group)
gbl.mid <- droplevels(gbl.mid)


#Gender:
n.gbl.mid.res.men <- gbl.mid[gbl.mid$gender=="Male",]               #Count how many responses from  male researchers
percent.men<-((dim(n.gbl.mid.res.men)[1])/(dim(gbl.mid)[1]))*100      #Calculate the % of responses from  male researchers
n.gbl.mid.res.women <- gbl.mid[gbl.mid$gender=="Female",]           #Count how many responses from  female researchers
percent.women<-((dim(n.gbl.mid.res.women)[1])/(dim(gbl.mid)[1]))*100  #Calculate the % of responses from  female researchers
# 
# #Career Stage Tabulations:

n.earlyac <- gbl.mid[gbl.mid$what_participant_group=="Early career academic researcher",]
percent.earlyac<-((dim(n.earlyac)[1])/(dim(gbl.mid)[1]))*100 #Calculate the % of responses from  early academic researchers

n.nonACearly <- gbl.mid[gbl.mid$what_participant_group=="Non-academic researcher <10 yrs experience",]
percent.nonACearly<-((dim(n.nonACearly)[1])/(dim(gbl.mid)[1]))*100 #Calculate the % of responses from early career non-academics


#Discipline Tabulations:
n.natural <- gbl.mid[gbl.mid$field_research=="Natural Science",]
percent.natural <- ((dim(n.natural)[1])/(dim(gbl.mid)[1]))*100 #Calculate the % of responses from  natural scientists
n.physics <- gbl.mid[gbl.mid$field_research=="Physical Science (eg. math, physics, chemistry, computer science)",]
percent.physics <- ((dim(n.physics)[1])/(dim(gbl.mid)[1]))*100 #Calculate the % of responses from  physical scientists
n.med <- gbl.mid[gbl.mid$field_research=="Medicine and Life Science",]
percent.med <- ((dim(n.med)[1])/(dim(gbl.mid)[1]))*100 #Calculate the % of responses from  med/life scientists
n.eng <- gbl.mid[gbl.mid$field_research=="Engineering",]
percent.eng <- ((dim(n.eng)[1])/(dim(gbl.mid)[1]))*100 #Calculate the % of responses from  engineering scientists
n.int <- gbl.mid[gbl.mid$field_research=="Interdisciplinary Science",]
percent.int <- ((dim(n.int)[1])/(dim(gbl.mid)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists
n.ssh <- gbl.mid[gbl.mid$field_research=="Social Science / Humanities",]
percent.ssh <- ((dim(n.ssh)[1])/(dim(gbl.mid)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists


```

**Results**  
In total, `r dim(gbl.mid)[1]`  researchers completed the online survey. Of these, almost xxxxx were male (`r ceiling(percent.men)`%) and xxxxx were female (`r round(percent.women)`%); xxxx proportion either did not input their gender or selected other. `r round(percent.earlyac)` were academics with less than 10 years of experience since completing their PhD and `r round(percent.nonACearly)` were non academics with less than 10 years of experience since completing their PhD.
...
locations<-aggregate(gender ~ nation, gbl.mid, length)
locations<-locations[!locations$nation=="",]
locations<-droplevels(locations)

#pdf(file="figures/mid_career/responses_country_13Mar18.pdf", width = 11, height= 8)
...
ggplot(locations, aes(x = reorder(nation, -gender), gender)) + geom_bar(stat='identity',position = position_dodge(width=0.5),fill='#1f78b4') + theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5)) + labs(y="Number of responses", x="", title="Responses by Country")  + 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+scale_y_continuous(expand = c(0,0), limits=c(0, 410)) 
...
#pdf(file="figures/mid_career/responses_country_gapplot_13Mar18.pdf", height=8, width=11)

par(mar=c(11,4,4,2))
gap.barplot(sort(locations$gender,decreasing=TRUE), gap=c(76, 380),horiz=F, xaxlab=barlabs, xlab="     ", 
            ylab="Number of responses", ytics=c(0,25,50,75,384), main = "Responses by country",las=2, col=c(blue2red(29)))
...
locations.more<-aggregate(gender ~ nation, gbl.mid, length)
...
#pdf(file="figures/mid_career/responses_country_10+_gapplot_13Mar18.pdf", height=8, width=11)

par(mar=c(7,4,4,2))
gap.barplot(sort(locations.more$gender,decreasing=TRUE), gap=c(76, 380),horiz=F, xaxlab=barlabs, xlab="     ", 
            ylab="Number of responses", ytics=c(0,25,50,75,384), main = "Countries with greater than 10 responses",las=2, col=color1)
...
locations.less<-aggregate(gender ~ nation, gbl.mid, length)
...
color1<-c('#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4')
#pdf(file="figures/mid_career/responses_country_10+&10-_gapplot_13Mar18.pdf", height=8, width=15)
layout(matrix(c(1,2),1,2, byrow = TRUE), widths=c(0.7,2))
par(mar=c(11,4,4,2))
gap.barplot(sort(locations.more$gender,decreasing=TRUE), gap=c(76, 380),horiz=F, xaxlab=barlabs, xlab="     ", 
            ylab="Number of responses", ytics=c(0,25,50,75,384), main = "Countries with greater\nthan 10 responses",las=2, col=color1)
par(mar=c(11,2,4,1))
barplot(sort(locations.less$gender,decreasing=TRUE), names.arg = locations.less$nation[order(locations.less$gender, decreasing=TRUE)] ,horiz=F, xlab="", 
...
experience<-data.frame(table(gbl.mid$what_participant_group))
...
#pdf(file="figures/mid_career/4.1_CareerStage_13March18.pdf", width = 11, height= 7)
...
ggplot(experience, aes(x = reorder(Var1, -Freq), Freq, fill=Var1)) + 
  ...
  scale_y_continuous(expand = c(0,0), limits = c(0,890))
#dev.off()
...
field<-data.frame(table(gbl.mid$field_research))
...
#pdf(file="figures/mid_career/4.2_FieldofResearch_13March18.pdf", width = 11, height= 7)
...
#pdf(file="figures/mid_career/4.1and4.2_CareerStageAndFieldOfResearch.pdf", width = 11, height= 7)
...
research.gbl.mid<-research[(research$what_participant_group%in%c("Early career academic researcher" ,"Non-academic researcher <10 yrs experience")),]
research.gbl.mid <- droplevels(research.gbl.mid)


## make sure that Use-inspired is the middle level in the legend
res.plot<-subset(research.gbl.mid, type%in%c("percent_fundemental_research_current",  "percent_Applied_Research_current" , "percent_Use_inspired_Research_current","percent_Applied_Research_past",  "percent_Fundamental_Research_past" , "percent_Use_inspired_Research_past"))
...
#pdf(file="figures/mid_career/4.3.1_ResearchProportion_13March18.pdf", width = 11, height= 7)
...
ggplot(res.plot, aes(percent, fill=ID)) +
  ...
  scale_x_continuous(breaks=seq(0,100,10))+scale_y_continuous(expand=c(0,0), limits=c(0,180),breaks=seq(0,180, 20))
...
#pdf(file="figures/mid_career/4.3.2_ResearchProportion.pdf", width = 11, height= 7)
...
research.change.gbl.mid<-research.change[(research.change$what_participant_group%in%c("Early career academic researcher" ,"Non-academic researcher <10 yrs experience")),]
research.change.gbl.mid <- droplevels(research.change.gbl.mid)

#Change yes or no
yes.no<-subset(research.change.gbl.mid, select=c("Location","Country", "Country_work",  "gender", "changed_10yrs"))

mid.yes.no<-aggregate(Country~ changed_10yrs, yes.no, length)
mid.yes.no<-mid.yes.no[!mid.yes.no$changed_10yrs=="",]

p1<-ggplot(data=mid.yes.no, aes(x=changed_10yrs, y=Country, fill=Country))+ geom_bar(stat='identity') + guides(fill=FALSE)+
  theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(), axis.title.x=element_blank(), plot.title = element_text(hjust = 0.5))+
  geom_text(aes(label=Country), vjust=-0.5)+labs(y="Number of responses", title="Change in research type")+ scale_y_continuous(expand = c(0, 0), limits = c(0,390))+ annotate('text', 0.5,385, label="(a)", size=5)
...
reason<-subset(research.change.gbl.mid, select=c("Location","Country","Country_work",  "gender", "Main_reason_change_interest_related",
...
require(tidyr)
mid.long<-gather(reason, reason.change.mid, yes.mid, -Location, -gender, -Country_work, -Country)
# using table to count cases of each category
sum.mid<-data.frame(table(mid.long$reason.change.mid, mid.long$yes.mid))
#remove non-response
sum.mid<-sum.mid[!sum.mid$Var1=="",]

sum.mid$Var1<-revalue(sum.mid$Var1, c("Main_reason_change_Career_related" = "Career", "Main_reason_change_Funding_related" = "Funding", "Main_reason_change_interest_related" = "Interest", "Main_reason_change_Other" = "Other", "Main_reason_change_Socially_related"  = "Social"))
# change order of the levels
sum.mid$Var1<-factor(sum.mid$Var1,levels(sum.mid$Var1)[c(2,3,1,5,4)])

myColors4<-(brewer.pal(9, "Blues"))
myColors4<-myColors4[c(9, 8,7,6,5,3)]

p2<-ggplot(sum.mid, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 225))+annotate('text', 5.4,220, label="(b)", size=5)+geom_text(aes(label=Freq), vjust=-0.5, size=3)


#pdf(file="figures/mid_career/4.4_ReasonForFieldChange.pdf", width = 11, height= 7)

#print(p2)

#dev.off()

#pdf(file="figures/mid_career/4.4a&b_ReasonForFieldChange_13March18.pdf", width = 11, height= 7)
...
view.propor<-gbl.mid[!gbl.mid$view_change_of_type=="",]
view.propor<-droplevels(view.propor)
change.view.mid<-aggregate(gender~ view_change_of_type, view.propor, length)
# change order of the levels
change.view.mid$view_change_of_type<-factor(change.view.mid$view_change_of_type, 
                                        levels(change.view.mid$view_change_of_type)[c(5,3,1,2,4)])

myColors5<-(brewer.pal(9, "Blues"))
myColors5<-myColors5[c(4,4,4,4,4,4)]

#pdf(file="figures/mid_career/4.5_ViewFieldChange_13March18.pdf", width = 11, height= 7)

ggplot(change.view.mid, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
...
b.part<-data.frame(table(gbl.mid$partnership_outside_before))
b.part<-b.part[!b.part$Var1=="",]
b.part<-droplevels(b.part)
b.part$time<-"Past"
a.part<-data.frame(table(gbl.mid$partnership_outside))
...
#pdf(file="figures/mid_career/4.6_LevelsOfPartnership_13March18.pdf", width = 11, height= 7)
...
ggplot(part, aes(x=Var1, y=Freq, fill = time),  ylim(0,375)) + 
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,390))
...
change.part.mid<-data.frame(table(gbl.mid$partnership_change_10yrs))
change.part.mid<-change.part.mid[!change.part.mid$Var1=="",]
change.part.mid$Var1<-revalue(change.part.mid$Var1, c("Can't comment (new researcher)"="Can't comment"))
change.part.mid$Var1<-factor(change.part.mid$Var1, levels=c("Yes", "No", "Can't comment"))
myColors<-(brewer.pal(9, "Blues"))
myColors<-myColors[c(9, 7,5)]
g1<-ggplot(data=change.part.mid, aes(x=Var1, y=Freq, fill=Var1))+ 
...
part2.reason.gbl.mid<-part2.reason[(part2.reason$what_participant_group%in%c("Early career academic researcher" ,"Non-academic researcher <10 yrs experience")),]
part2.reason.gbl.mid <- droplevels(part2.reason.gbl.mid)

reason.pt.long<-gather(part2.reason.gbl.mid, change.reason, yes, -Country,-Country_work, -gender, -nation, -Location, -what_participant_group, -field_research)
sum.mid<-data.frame(table(reason.pt.long$change.reason, reason.pt.long$yes))

sum.mid$Var1<-revalue(sum.mid$Var1, c("reason_partnership_change_career" = "Career", "reason_partnership_change_funding" = "Funding", "reason_partnership_change_interest" = "Interest", "reason_partnership_change_other" = "Other", "reason_partnership_change_socially"  = "Social"))
sum.mid$Var1<-reorder(sum.mid$Var1, -sum.mid$Freq)

myColors7<-(brewer.pal(9, "Blues"))
myColors7<-myColors7[c(9, 8,7,6,5,3)]

g2<-ggplot(sum.mid, aes(x=Var1, y=Freq, fill=Var1)) + 
...
#pdf(file="figures/mid_career/4.7a&b_PartnershipChange&Reason_13March18.pdf", width = 11, height= 7)
...
view<-data.frame(table(gbl.mid$view_change_partnership))
view<-view[!view$Var1=="",]
...
#pdf(file="figures/mid_career/4.8_ViewChangePartnership_13march18.pdf", width = 11, height= 7)
...
ggplot(view, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+
  ...
  labs(x="", y="Number of responses", title="Opinion of change in partnership level") + colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,100), breaks = seq(0,100,20))
...
part3.grants.long.gbl.mid<-part3.grants.long[(part3.grants.long$what_participant_group%in%c("Early career academic researcher" ,"Non-academic researcher <10 yrs experience")),]
part3.grants.long.gbl.mid <- droplevels(part3.grants.long.gbl.mid)

part3.grants.long.gbl.mid$type.grant<-as.character(part3.grants.long.gbl.mid$type.grant)


#add time variable
part3.grants.long.gbl.mid$time<-ifelse(grepl("11_15", part3.grants.long.gbl.mid$type.grant), "2011-2015", "2006-2010")
part3.grants.long.gbl.mid$type.grant[part3.grants.long.gbl.mid$type.grant=="external_pi_grant_11_15_fundamental"]<-"Fundamental"
part3.grants.long.gbl.mid$type.grant[part3.grants.long.gbl.mid$type.grant=="external_pi_grant_6_10_fundamental"]<-"Fundamental"
part3.grants.long.gbl.mid$type.grant[part3.grants.long.gbl.mid$type.grant=="external_pi_grant_11_15_use"]<-"Use-Inspired"
part3.grants.long.gbl.mid$type.grant[part3.grants.long.gbl.mid$type.grant=="external_pi_grant_6_10_use"]<-"Use-Inspired"
part3.grants.long.gbl.mid$type.grant[part3.grants.long.gbl.mid$type.grant=="external_pi_grant_11_15_applied"]<-"Applied"
part3.grants.long.gbl.mid$type.grant[part3.grants.long.gbl.mid$type.grant=="external_pi_grant_6_10_applied"]<-"Applied"

grants.mid<-data.frame(with(part3.grants.long.gbl.mid, table(type.grant,time,number)))
grants.mid$type.grant<-as.factor(grants.mid$type.grant)


#change order of levels
grants.mid$number<-factor(grants.mid$number, levels(grants.mid$number)[c(1,2,6,7,3,4,5)])
grants.mid$type.grant<-factor(grants.mid$type.grant, levels(grants.mid$type.grant)[c(2,3,1)])

#myColors9<-(brewer.pal(9, "Blues"))
#myColors9<-rev(myColors9[c(9, 5)])
#colScale <- scale_fill_manual(name = "time",values = myColors9)

grants.mid$ID<-paste(grants.mid$type.grant, grants.mid$time)

# change order of the levels
grants.mid$ID<-as.factor(grants.mid$ID)
grants.mid$ID<-factor(grants.mid$ID, levels(grants.mid$ID)[c(4,3,6,5,2,1)])

##colors
mycols<-c('#33a02c','#a6cee3','#fb9a99','#1f78b4', '#e31a1c','#b2df8a')
#mycols<-as.data.frame(mycols)

#pdf(file="figures/mid_career/4.9_NumberGrantApps_13March18.pdf", height=7, width =11)


#####add (a)
xs <- split(grants.mid,f = grants.mid$type.grant)
p1 <- ggplot(xs$Fundamental, aes(number, Freq, fill=time), width=0.7) + 
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,800), breaks = c(0,200,400,600,800))
...
part3.success.long.gbl.mid<-part3.success.long[(part3.success.long$what_participant_group%in%c("Early career academic researcher" ,"Non-academic researcher <10 yrs experience")),]
part3.success.long.gbl.mid <- droplevels(part3.success.long.gbl.mid)

part3.success.long.gbl.mid$type<-as.character(part3.success.long.gbl.mid$type)

#add time variable
part3.success.long.gbl.mid$time<-ifelse(grepl("11_15", part3.success.long.gbl.mid$type), "2011-2015", "2006-2010")
part3.success.long.gbl.mid$type[part3.success.long.gbl.mid$type=="successful_grants_11_15_fundamental"]<-"Fundamental"
part3.success.long.gbl.mid$type[part3.success.long.gbl.mid$type=="successful_grants_6_10_fundamental"]<-"Fundamental"
part3.success.long.gbl.mid$type[part3.success.long.gbl.mid$type=="successful_grants_11_15_use"]<-"Use-Inspired"
part3.success.long.gbl.mid$type[part3.success.long.gbl.mid$type=="successful_grants_6_10_use"]<-"Use-Inspired"
part3.success.long.gbl.mid$type[part3.success.long.gbl.mid$type=="successful_grants_11_15_applied"]<-"Applied"
part3.success.long.gbl.mid$type[part3.success.long.gbl.mid$type=="successful_grants_6_10_applied"]<-"Applied"

success.mid<-data.frame(with(part3.success.long.gbl.mid, table(type, percent,time)))



#change order of types
success.mid$type<-as.factor(success.mid$type)
success.mid$type<-factor(success.mid$type, levels(success.mid$type)[c(2,3,1)])

success.mid$id<-paste(success.mid$type, success.mid$time)

# change order of the levels
success.mid$id<-as.factor(success.mid$id)
success.mid$id<-factor(success.mid$id, levels(success.mid$id)[c(3,4,5,6,1,2)])

mycols10<-c('#1f78b4','#a6cee3','#33a02c','#b2df8a','#e31a1c', '#fb9a99')
mycols10<-as.data.frame(mycols10)
mycols10$id<-levels(success.mid$id)
success.mid$col<-mycols10$mycols10[match(success.mid$id, mycols10$id)]
success.mid$col<-factor(success.mid$col, levels(success.mid$col)[c(3,1,4,2,6,5)])

#pdf(file="figures/mid_career/4.10_GrantAppSuccess_13March18.pdf", height=7, width =11)

out2<-by(data = success.mid, INDICES = success.mid$type, FUN = function(n){
  n <- droplevels(n)
...
part3.change.gbl.mid<-part3.change[(part3.change$what_participant_group%in%c("Early career academic researcher" ,"Non-academic researcher <10 yrs experience")),]
part3.change.gbl.mid <- droplevels(part3.change.gbl.mid)

##switch to long format
change.long.mid<-gather(part3.change.gbl.mid, type, level, -Location, -gender, -Country,-nation, -Country_work, -what_participant_group, -field_research)
success.change<-with(change.long.mid, data.frame(table(type, level)))
#remove no response
...
#pdf(file="figures/mid_career/4.11_ChangeSuccessRate_13March18.pdf", height=7, width =11)
...
ggplot(data=success.change, aes(x=level, Freq, fill=type))+ facet_wrap(~type, nrow=3)+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,355))
...
part3.prac.long.gbl.mid<-part3.prac.long[(part3.prac.long$what_participant_group%in%c("Early career academic researcher" ,"Non-academic researcher <10 yrs experience")),]
part3.prac.long.gbl.mid <- droplevels(part3.prac.long.gbl.mid)

part3.prac.long.gbl.mid$year<-as.character(part3.prac.long.gbl.mid$year)

part3.prac.long.gbl.mid$year[part3.prac.long.gbl.mid$year=="practical_applications_important_11_15"]<-"2011-2015"
part3.prac.long.gbl.mid$year[part3.prac.long.gbl.mid$year=="practical_applications_important_6_10"]<-"2006-2010"


prac.mid<-data.frame(table(part3.prac.long.gbl.mid$year, part3.prac.long.gbl.mid$level))

#change order of levels
prac.mid$Var2<-factor(prac.mid$Var2, levels(prac.mid$Var2)[c(2,6,4,5,3,1)])

myColors12<-(brewer.pal(9, "Blues"))
myColors12<-rev(myColors12[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors12)

#pdf(file="figures/mid_career/4.12_ImportancePracticalApplication_13March18.pdf", height=7, width =11)

ggplot(prac.mid, aes(Var2, Freq, fill=Var1)) + 
...
part3.part.long.gbl.mid<-part3.part.long[(part3.part.long$what_participant_group%in%c("Early career academic researcher" ,"Non-academic researcher <10 yrs experience")),]
part3.part.long.gbl.mid <- droplevels(part3.part.long.gbl.mid)

part3.part.long.gbl.mid$year<-as.character(part3.part.long.gbl.mid$year)

part3.part.long.gbl.mid$year[part3.part.long.gbl.mid$year=="include_nonacademia_partners_success_11_15"]<-"2011-2015"
part3.part.long.gbl.mid$year[part3.part.long.gbl.mid$year=="include_nonacademia_partners_success_6_10"]<-"2006-2010"

part.mid<-data.frame(table(part3.part.long.gbl.mid$year, part3.part.long.gbl.mid$level))
#change order of levels
part.mid$Var2<-factor(part.mid$Var2, levels(part.mid$Var2)[c(2,5,6,4,3,1)])
...
#pdf(file="figures/mid_career/4.13_ImportancePartnersApplication_13March18.pdf", height=7, width =11)

ggplot(part.mid, aes(Var2, Freq, fill=Var1))+ 
...
p3_master.long.gbl.mid<-p3_master.long[(p3_master.long$what_participant_group%in%c("Early career academic researcher" ,"Non-academic researcher <10 yrs experience")),]
p3_master.long.gbl.mid <- droplevels(p3_master.long.gbl.mid)

##revalue the year values
p3_master.long.gbl.mid$year<- revalue(p3_master.long.gbl.mid$year, c("11_15"="2011-2015", "6_10"="2006-2010"))
#change order of levels
p3_master.long.gbl.mid$year<-factor(p3_master.long.gbl.mid$year, levels(p3_master.long.gbl.mid$year)[c(2,1)])

##For-Profit
fp<-p3_master.long.gbl.mid[p3_master.long.gbl.mid$type=="For-Profit",]
...
p1<-ggplot(fp, aes(percent, fill=year))+ geom_histogram(position='dodge', binwidth=10)  + colScale +
  ...
  scale_y_continuous(expand =c(0,0),limits=c(0,850), breaks=seq(0, 850, 200))+
  scale_x_continuous( breaks = seq(0, 100, 10))


##Government
gov<-p3_master.long.gbl.mid[p3_master.long.gbl.mid$type=="Government",]
...
int<-p3_master.long.gbl.mid[p3_master.long.gbl.mid$type=="Internal",]
...
ngov<-p3_master.long.gbl.mid[p3_master.long.gbl.mid$type=="Non-governmental",]
...
other<-p3_master.long.gbl.mid[p3_master.long.gbl.mid$type=="Other",]
...
#pdf(file="figures/mid_career/4.14_DistributionFunding.pdf", height=7, width =11)
...
part4.gbl.mid<-part4[(part4$what_participant_group%in%c("Early career academic researcher" ,"Non-academic researcher <10 yrs experience")),]
part4.gbl.mid <- droplevels(part4.gbl.mid)

## order bars from very important (left) to can't comment (right)
important<-subset(part4.gbl.mid, select=c("Location","Country",'Country_work', "gender","opinion_fundamental_important"))
...
#pdf(file="figures/mid_career/4.15_PerceivedImportanceFundamentalGovernment_13March18.pdf", height=7, width =11)
...
ggplot(data=perceive, aes(x=Var1, y=Freq, fill=Var1)) + 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,330)) + colScale
...
priority<-subset(part4.gbl.mid, select=c("Location","Country",'Country_work', "gender", "high_priority_fundamental", "high_priority_use_inspired", "high_priority_applied", "high_priority_no_change"))
...
#pdf(file="figures/mid_career/4.16_PerceivedChangeResearchPriorityGovernment_13March18.pdf", height=7, width =11)
...
ggplot(data=per.change, aes(x=what.type, higher.priority, fill=what.type))+
  ...
  geom_text(aes(label=higher.priority, vjust=-0.5), size=3) +scale_y_continuous(expand = c(0, 0), limits = c(0,750))+ colScale
...
availability.change<-subset(part4.gbl.mid, select=c("Location", "Country",'Country_work', "gender", "available_funding_fundamental",  
                                             "available_funding_use_inspired", "available_funding_applied"))
## switch to long format
availability.change.long.mid<-gather(availability.change, what.type, level, -gender, -Location, -Country, -Country_work)
availability.mid<-with(availability.change.long.mid, data.frame(table(what.type, level)))

#remove non response
availability.mid<-availability.mid[!availability.mid$level=="",]

## change type to factor with levels
availability.mid$level<-str_replace_all(availability.mid$level, "Will ", "")
availability.mid$what.type<-as.factor(availability.mid$what.type)
availability.mid$level<-as.factor(availability.mid$level)

## change order of levels
availability.mid$level<-factor(availability.mid$level, levels(availability.mid$level)[c(4,5,6,3,2,1 )])
availability.mid$what.type<-factor(availability.mid$what.type, levels(availability.mid$what.type)[c(2,3,1)])

availability.mid$what.type<-revalue(availability.mid$what.type, c("available_funding_fundamental"="Fundamental",
...
#pdf(file="figures/mid_career/4.17_AnticipatedChangeFunding_13March18.pdf", height=7, width =11)

ggplot(data=availability.mid, aes(x=level, Freq, fill=what.type))+
    geom_bar(stat="identity", position="dodge")+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,275), breaks = seq(0,275,25))
...
## order bars into increase considerably (left) to decrease considerably (right), then no comment at the far right
next.generation<-subset(part4.gbl.mid, select=c("Location", "Country","Country_work", "gender","next_generation"))
# remove non-response
...
#pdf(file="figures/mid_career/4.18_EffectNextGeneration_13March18.pdf", height=7, width =11)
...
ggplot(data=impact, aes(x=Var1, Freq, fill=Var1))+ 
 ...
 labs(x="", y="Number of responses", title="Effect of funding changes on the next generation of researchers")+
  ...
  geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,250))+colScale
