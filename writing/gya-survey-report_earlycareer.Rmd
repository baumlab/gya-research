...
##selecting for early career stage respondents
...
gbl.early<-survey[(survey$what_participant_group%in%"Postdoc or RA"),]

#check
unique(gbl.early$what_participant_group)
gbl.early <- droplevels(gbl.early)


#Gender:
n.gbl.early.res.men <- gbl.early[gbl.early$gender=="Male",]               #Count how many responses from  male researchers
percent.men<-((dim(n.gbl.early.res.men)[1])/(dim(gbl.early)[1]))*100      #Calculate the % of responses from  male researchers
n.gbl.early.res.women <- gbl.early[gbl.early$gender=="Female",]           #Count how many responses from  female researchers
percent.women<-((dim(n.gbl.early.res.women)[1])/(dim(gbl.early)[1]))*100  #Calculate the % of responses from  female researchers
# 
# #Career Stage Tabulations:

n.pdf <- gbl.early[gbl.early$what_participant_group=="Postdoc or RA",]
percent.pdf<-((dim(n.pdf)[1])/(dim(gbl.early)[1]))*100 #Calculate the % of responses from  post-docs

#Discipline Tabulations:
n.natural <- gbl.early[gbl.early$field_research=="Natural Science",]
percent.natural <- ((dim(n.natural)[1])/(dim(gbl.early)[1]))*100 #Calculate the % of responses from  natural scientists
n.physics <- gbl.early[gbl.early$field_research=="Physical Science (eg. math, physics, chemistry, computer science)",]
percent.physics <- ((dim(n.physics)[1])/(dim(gbl.early)[1]))*100 #Calculate the % of responses from  physical scientists
n.med <- gbl.early[gbl.early$field_research=="Medicine and Life Science",]
percent.med <- ((dim(n.med)[1])/(dim(gbl.early)[1]))*100 #Calculate the % of responses from  med/life scientists
n.eng <- gbl.early[gbl.early$field_research=="Engineering",]
percent.eng <- ((dim(n.eng)[1])/(dim(gbl.early)[1]))*100 #Calculate the % of responses from  engineering scientists
n.int <- gbl.early[gbl.early$field_research=="Interdisciplinary Science",]
percent.int <- ((dim(n.int)[1])/(dim(gbl.early)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists
n.ssh <- gbl.early[gbl.early$field_research=="Social Science / Humanities",]
percent.ssh <- ((dim(n.ssh)[1])/(dim(gbl.early)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists


```

**Results**  
In total, `r dim(gbl.early)[1]`  researchers completed the online survey. Of these, almost xxxxx were male (`r ceiling(percent.men)`%) and xxxxx were female (`r round(percent.women)`%); xxxx proportion either did not input their gender or selected other. This category only includes the responses from researchers who are currently in their PhD or are RAs.
...
locations<-aggregate(gender ~ nation, gbl.early, length)
locations<-locations[!locations$nation=="",]
locations<-droplevels(locations)

#pdf(file="figures/early_career/responses_country_13Mar18.pdf", width = 11, height= 8)
...
ggplot(locations, aes(x = reorder(nation, -gender), gender)) + geom_bar(stat='identity',position = position_dodge(width=0.5),fill='#1f78b4') + theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5)) + labs(y="Number of responses", x="", title="Responses by Country")  + 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+scale_y_continuous(expand = c(0,0), limits=c(0, 53)) 
...
#pdf(file="figures/early_career/responses_country_gapplot_13Mar18.pdf", height=8, width=11)

par(mar=c(11,4,4,2))
gap.barplot(sort(locations$gender,decreasing=TRUE), gap=c(26, 46),horiz=F, xaxlab=barlabs, xlab="     ", 
            ylab="Number of responses", ytics=c(0,10, 20, 50), main = "Responses by country",las=2, col=c(blue2red(29)))
...
locations.more<-aggregate(gender ~ nation, gbl.early, length)
...
#pdf(file="figures/early_career/responses_country_10+_gapplot_13Mar18.pdf", height=8, width=11)

par(mar=c(7,4,4,2))
gap.barplot(sort(locations.more$gender,decreasing=TRUE), gap=c(26, 46),horiz=F, xaxlab=barlabs, xlab="     ", 
            ylab="Number of responses", ytics=c(0,10, 20, 50), main = "Countries with greater than 10 responses",las=2, col=color1)
...
locations.less<-aggregate(gender ~ nation, gbl.early, length)
...
color1<-c('#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4')
#pdf(file="figures/early_career/responses_country_10+&10-_gapplot_13Mar18.pdf", height=8, width=15)
layout(matrix(c(1,2),1,2, byrow = TRUE), widths=c(0.7,2))
par(mar=c(11,4,4,2))
gap.barplot(sort(locations.more$gender,decreasing=TRUE), gap=c(26, 46),horiz=F, xaxlab=barlabs, xlab="     ", 
            ylab="Number of responses", ytics=c(0,10, 20, 50), main = "Countries with greater\nthan 10 responses",las=2, col=color1)
par(mar=c(11,2,4,1))
barplot(sort(locations.less$gender,decreasing=TRUE), names.arg = locations.less$nation[order(locations.less$gender, decreasing=TRUE)] ,horiz=F, xlab="", 
...
experience<-data.frame(table(gbl.early$what_participant_group))
...
#pdf(file="figures/early_career/4.1_CareerStage_13March18.pdf", width = 11, height= 7)

ggplot(experience, aes(x = reorder(Var1, -Freq), Freq, fill=Var1)) + 
  geom_bar(stat='identity',position = position_dodge(width=0.5)) + 
  geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  scale_x_discrete(labels=c("Postdoc or RA")) +
...
field<-data.frame(table(gbl.early$field_research))
...
#pdf(file="figures/early_career/4.2_FieldofResearch_13March18.pdf", width = 11, height= 7)
...
#pdf(file="figures/early_career/4.1and4.2_CareerStageAndFieldOfResearch.pdf", width = 11, height= 7)
...
research.gbl.early<-research[(research$what_participant_group%in%"Postdoc or RA"),]
research.gbl.early <- droplevels(research.gbl.early)

## make sure that Use-inspired is the middle level in the legend
res.plot<-subset(research.gbl.early, type%in%c("percent_fundemental_research_current",  "percent_Applied_Research_current" , "percent_Use_inspired_Research_current","percent_Applied_Research_past",  "percent_Fundamental_Research_past" , "percent_Use_inspired_Research_past"))
...
#pdf(file="figures/early_career/4.3.1_ResearchProportion_13March18.pdf", width = 11, height= 7)
...
ggplot(res.plot, aes(percent, fill=ID)) +
  ...
  scale_x_continuous(breaks=seq(0,100,10))+scale_y_continuous(expand=c(0,0), limits=c(0,30),breaks=seq(0,30,10))
...
#pdf(file="figures/early_career/4.3.2_ResearchProportion_13March18.pdf", width = 11, height= 7)
...
research.change.gbl.early<-research.change[(research.change$what_participant_group%in%"Postdoc or RA"),]
research.change.gbl.early <- droplevels(research.change.gbl.early)

#Change yes or no
yes.no<-subset(research.change.gbl.early, select=c("Location","Country", "Country_work",  "gender", "changed_10yrs"))

early.yes.no<-aggregate(Country~ changed_10yrs, yes.no, length)
early.yes.no<-early.yes.no[!early.yes.no$changed_10yrs=="",]

p1<-ggplot(data=early.yes.no, aes(x=changed_10yrs, y=Country, fill=Country))+ geom_bar(stat='identity') + guides(fill=FALSE)+
  theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(), axis.title.x=element_blank(), plot.title = element_text(hjust = 0.5))+
  geom_text(aes(label=Country), vjust=-0.5)+labs(y="Number of responses", title="Change in research type")+ scale_y_continuous(expand = c(0, 0), limits = c(0,67))+ annotate('text', 0.5,65, label="(a)", size=5)
...
reason<-subset(research.change.gbl.early, select=c("Location","Country","Country_work",  "gender", "Main_reason_change_interest_related",
...
require(tidyr)
early.long<-gather(reason, reason.change.early, yes.early, -Location, -gender, -Country_work, -Country)
# using table to count cases of each category
sum.early<-data.frame(table(early.long$reason.change.early, early.long$yes.early))
#remove non-response
sum.early<-sum.early[!sum.early$Var1=="",]

sum.early$Var1<-revalue(sum.early$Var1, c("Main_reason_change_Career_related" = "Career", "Main_reason_change_Funding_related" = "Funding", "Main_reason_change_interest_related" = "Interest", "Main_reason_change_Other" = "Other", "Main_reason_change_Socially_related"  = "Social"))
# change order of the levels
sum.early$Var1<-factor(sum.early$Var1,levels(sum.early$Var1)[c(2,3,1,5,4)])

myColors4<-(brewer.pal(9, "Blues"))
myColors4<-myColors4[c(9, 8,7,6,5,3)]

p2<-ggplot(sum.early, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 40))+annotate('text', 5.4,37, label="(b)", size=5)+geom_text(aes(label=Freq), vjust=-0.5, size=3)


#pdf(file="figures/early_career/4.4_ReasonForFieldChange.pdf", width = 11, height= 7)

#print(p2)

#dev.off()

#pdf(file="figures/early_career/4.4a&b_ReasonForFieldChange_13March18.pdf", width = 11, height= 7)
...
view.propor<-gbl.early[!gbl.early$view_change_of_type=="",]
view.propor<-droplevels(view.propor)
change.view.early<-aggregate(gender~ view_change_of_type, view.propor, length)
# change order of the levels
change.view.early$view_change_of_type<-factor(change.view.early$view_change_of_type, 
                                        levels(change.view.early$view_change_of_type)[c(5,3,1,2,4)])

myColors5<-(brewer.pal(9, "Blues"))
myColors5<-myColors5[c(4,4,4,4,4,4)]

#pdf(file="figures/early_career/4.5_ViewFieldChange_13March18.pdf", width = 11, height= 7)

ggplot(change.view.early, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
...
b.part<-data.frame(table(gbl.early$partnership_outside_before))
b.part<-b.part[!b.part$Var1=="",]
b.part<-droplevels(b.part)
b.part$time<-"Past"
a.part<-data.frame(table(gbl.early$partnership_outside))
...
#pdf(file="figures/early_career/4.6_LevelsOfPartnership_13March18.pdf", width = 11, height= 7)
...
ggplot(part, aes(x=Var1, y=Freq, fill = time),  ylim(0,375)) + 
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,95))
...
change.part.early<-data.frame(table(gbl.early$partnership_change_10yrs))
change.part.early<-change.part.early[!change.part.early$Var1=="",]
change.part.early$Var1<-revalue(change.part.early$Var1, c("Can't comment (new researcher)"="Can't comment"))
change.part.early$Var1<-factor(change.part.early$Var1, levels=c("Yes", "No", "Can't comment"))
myColors<-(brewer.pal(9, "Blues"))
myColors<-myColors[c(9, 7,5)]
g1<-ggplot(data=change.part.early, aes(x=Var1, y=Freq, fill=Var1))+ 
...
part2.reason.gbl.early<-part2.reason[(part2.reason$what_participant_group%in%"Postdoc or RA"),]
part2.reason.gbl.early <- droplevels(part2.reason.gbl.early)

reason.pt.long<-gather(part2.reason.gbl.early, change.reason, yes, -Country,-Country_work, -gender, -nation, -Location, -what_participant_group, -field_research)
sum.early<-data.frame(table(reason.pt.long$change.reason, reason.pt.long$yes))

sum.early$Var1<-revalue(sum.early$Var1, c("reason_partnership_change_career" = "Career", "reason_partnership_change_funding" = "Funding", "reason_partnership_change_interest" = "Interest", "reason_partnership_change_other" = "Other", "reason_partnership_change_socially"  = "Social"))
sum.early$Var1<-reorder(sum.early$Var1, -sum.early$Freq)

myColors7<-(brewer.pal(9, "Blues"))
myColors7<-myColors7[c(9, 8,7,6,5,3)]

g2<-ggplot(sum.early, aes(x=Var1, y=Freq, fill=Var1)) + 
...
#pdf(file="figures/early_career/4.7a&b_PartnershipChange&Reason_13March18.pdf", width = 11, height= 7)
...
view<-data.frame(table(gbl.early$view_change_partnership))
view<-view[!view$Var1=="",]
...
#pdf(file="figures/early_career/4.8_ViewChangePartnership_13March18.pdf", width = 11, height= 7)
...
ggplot(view, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+
  ...
  labs(x="", y="Number of responses", title="Opinion of change in partnership level") + colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,16), breaks = seq(0,16,2))
...
part3.grants.long.gbl.early<-part3.grants.long[(part3.grants.long$what_participant_group%in%"Postdoc or RA"),]
part3.grants.long.gbl.early <- droplevels(part3.grants.long.gbl.early)

part3.grants.long.gbl.early$type.grant<-as.character(part3.grants.long.gbl.early$type.grant)


#add time variable
part3.grants.long.gbl.early$time<-ifelse(grepl("11_15", part3.grants.long.gbl.early$type.grant), "2011-2015", "2006-2010")
part3.grants.long.gbl.early$type.grant[part3.grants.long.gbl.early$type.grant=="external_pi_grant_11_15_fundamental"]<-"Fundamental"
part3.grants.long.gbl.early$type.grant[part3.grants.long.gbl.early$type.grant=="external_pi_grant_6_10_fundamental"]<-"Fundamental"
part3.grants.long.gbl.early$type.grant[part3.grants.long.gbl.early$type.grant=="external_pi_grant_11_15_use"]<-"Use-Inspired"
part3.grants.long.gbl.early$type.grant[part3.grants.long.gbl.early$type.grant=="external_pi_grant_6_10_use"]<-"Use-Inspired"
part3.grants.long.gbl.early$type.grant[part3.grants.long.gbl.early$type.grant=="external_pi_grant_11_15_applied"]<-"Applied"
part3.grants.long.gbl.early$type.grant[part3.grants.long.gbl.early$type.grant=="external_pi_grant_6_10_applied"]<-"Applied"

grants.early<-data.frame(with(part3.grants.long.gbl.early, table(type.grant,time,number)))
grants.early$type.grant<-as.factor(grants.early$type.grant)


#change order of levels
grants.early$number<-factor(grants.early$number, levels(grants.early$number)[c(1,2,6,7,3,4,5)])
grants.early$type.grant<-factor(grants.early$type.grant, levels(grants.early$type.grant)[c(2,3,1)])

#myColors9<-(brewer.pal(9, "Blues"))
#myColors9<-rev(myColors9[c(9, 5)])
#colScale <- scale_fill_manual(name = "time",values = myColors9)

grants.early$ID<-paste(grants.early$type.grant, grants.early$time)

# change order of the levels
grants.early$ID<-as.factor(grants.early$ID)
grants.early$ID<-factor(grants.early$ID, levels(grants.early$ID)[c(4,3,6,5,2,1)])

##colors
mycols<-c('#33a02c','#a6cee3','#fb9a99','#1f78b4', '#e31a1c','#b2df8a')
#mycols<-as.data.frame(mycols)

#pdf(file="figures/early_career/4.9_NumberGrantApps_13March18.pdf", height=7, width =11)


#####add (a)
xs <- split(grants.early,f = grants.early$type.grant)
p1 <- ggplot(xs$Fundamental, aes(number, Freq, fill=time), width=0.7) + 
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,150), breaks = seq(0,150,10))
...
part3.success.long.gbl.early<-part3.success.long[(part3.success.long$what_participant_group%in%"Postdoc or RA"),]
part3.success.long.gbl.early <- droplevels(part3.success.long.gbl.early)

part3.success.long.gbl.early$type<-as.character(part3.success.long.gbl.early$type)

#add time variable
part3.success.long.gbl.early$time<-ifelse(grepl("11_15", part3.success.long.gbl.early$type), "2011-2015", "2006-2010")
part3.success.long.gbl.early$type[part3.success.long.gbl.early$type=="successful_grants_11_15_fundamental"]<-"Fundamental"
part3.success.long.gbl.early$type[part3.success.long.gbl.early$type=="successful_grants_6_10_fundamental"]<-"Fundamental"
part3.success.long.gbl.early$type[part3.success.long.gbl.early$type=="successful_grants_11_15_use"]<-"Use-Inspired"
part3.success.long.gbl.early$type[part3.success.long.gbl.early$type=="successful_grants_6_10_use"]<-"Use-Inspired"
part3.success.long.gbl.early$type[part3.success.long.gbl.early$type=="successful_grants_11_15_applied"]<-"Applied"
part3.success.long.gbl.early$type[part3.success.long.gbl.early$type=="successful_grants_6_10_applied"]<-"Applied"

success.early<-data.frame(with(part3.success.long.gbl.early, table(type, percent,time)))



#change order of types
success.early$type<-as.factor(success.early$type)
success.early$type<-factor(success.early$type, levels(success.early$type)[c(2,3,1)])

success.early$id<-paste(success.early$type, success.early$time)

# change order of the levels
success.early$id<-as.factor(success.early$id)
success.early$id<-factor(success.early$id, levels(success.early$id)[c(3,4,5,6,1,2)])

mycols10<-c('#1f78b4','#a6cee3','#33a02c','#b2df8a','#e31a1c', '#fb9a99')
mycols10<-as.data.frame(mycols10)
mycols10$id<-levels(success.early$id)
success.early$col<-mycols10$mycols10[match(success.early$id, mycols10$id)]
success.early$col<-factor(success.early$col, levels(success.early$col)[c(3,1,4,2,6,5)])

#pdf(file="figures/early_career/4.10_GrantAppSuccess_13March18.pdf", height=7, width =11)

out2<-by(data = success.early, INDICES = success.early$type, FUN = function(n){
  n <- droplevels(n)
...
part3.change.gbl.early<-part3.change[(part3.change$what_participant_group%in%"Postdoc or RA"),]
part3.change.gbl.early <- droplevels(part3.change.gbl.early)

##switch to long format
change.long.early<-gather(part3.change.gbl.early, type, level, -Location, -gender, -Country,-nation, -Country_work, -what_participant_group, -field_research)
success.change<-with(change.long.early, data.frame(table(type, level)))
#remove no response
...
#pdf(file="figures/early_career/4.11_ChangeSuccessRate_13March18.pdf", height=7, width =11)
...
part3.prac.long.gbl.early<-part3.prac.long[(part3.prac.long$what_participant_group%in%"Postdoc or RA"),]
part3.prac.long.gbl.early <- droplevels(part3.prac.long.gbl.early)

part3.prac.long.gbl.early$year<-as.character(part3.prac.long.gbl.early$year)

part3.prac.long.gbl.early$year[part3.prac.long.gbl.early$year=="practical_applications_important_11_15"]<-"2011-2015"
part3.prac.long.gbl.early$year[part3.prac.long.gbl.early$year=="practical_applications_important_6_10"]<-"2006-2010"


prac.early<-data.frame(table(part3.prac.long.gbl.early$year, part3.prac.long.gbl.early$level))

#change order of levels
prac.early$Var2<-factor(prac.early$Var2, levels(prac.early$Var2)[c(2,6,4,5,3,1)])

myColors12<-(brewer.pal(9, "Blues"))
myColors12<-rev(myColors12[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors12)

#pdf(file="figures/early_career/4.12_ImportancePracticalApplication_13March18.pdf", height=7, width =11)

ggplot(prac.early, aes(Var2, Freq, fill=Var1)) + 
...
part3.part.long.gbl.early<-part3.part.long[(part3.part.long$what_participant_group%in%"Postdoc or RA"),]
part3.part.long.gbl.early <- droplevels(part3.part.long.gbl.early)

part3.part.long.gbl.early$year<-as.character(part3.part.long.gbl.early$year)

part3.part.long.gbl.early$year[part3.part.long.gbl.early$year=="include_nonacademia_partners_success_11_15"]<-"2011-2015"
part3.part.long.gbl.early$year[part3.part.long.gbl.early$year=="include_nonacademia_partners_success_6_10"]<-"2006-2010"

part.early<-data.frame(table(part3.part.long.gbl.early$year, part3.part.long.gbl.early$level))
#change order of levels
part.early$Var2<-factor(part.early$Var2, levels(part.early$Var2)[c(2,5,6,4,3,1)])
...
#pdf(file="figures/early_career/4.13_ImportancePartnersApplication_13March18.pdf", height=7, width =11)

ggplot(part.early, aes(Var2, Freq, fill=Var1))+ 
...
p3_master.long.gbl.early<-p3_master.long[(p3_master.long$what_participant_group%in%"Postdoc or RA"),]
p3_master.long.gbl.early <- droplevels(p3_master.long.gbl.early)

##revalue the year values
p3_master.long.gbl.early$year<- revalue(p3_master.long.gbl.early$year, c("11_15"="2011-2015", "6_10"="2006-2010"))
#change order of levels
p3_master.long.gbl.early$year<-factor(p3_master.long.gbl.early$year, levels(p3_master.long.gbl.early$year)[c(2,1)])

##For-Profit
fp<-p3_master.long.gbl.early[p3_master.long.gbl.early$type=="For-Profit",]
...
##Government
gov<-p3_master.long.gbl.early[p3_master.long.gbl.early$type=="Government",]
...
int<-p3_master.long.gbl.early[p3_master.long.gbl.early$type=="Internal",]
...
ngov<-p3_master.long.gbl.early[p3_master.long.gbl.early$type=="Non-governmental",]
...
other<-p3_master.long.gbl.early[p3_master.long.gbl.early$type=="Other",]
...
#pdf(file="figures/early_career/4.14_DistributionFunding_13March18.pdf", height=7, width =11)
...
part4.gbl.early<-part4[(part4$what_participant_group%in%"Postdoc or RA"),]
part4.gbl.early <- droplevels(part4.gbl.early)

## order bars from very important (left) to can't comment (right)
important<-subset(part4.gbl.early, select=c("Location","Country",'Country_work', "gender","opinion_fundamental_important"))
...
#pdf(file="figures/early_career/4.15_PerceivedImportanceFundamentalGovernment_13March18.pdf", height=7, width =11)
...
ggplot(data=perceive, aes(x=Var1, y=Freq, fill=Var1)) + 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,55)) + colScale
...
priority<-subset(part4.gbl.early, select=c("Location","Country",'Country_work', "gender", "high_priority_fundamental", "high_priority_use_inspired", "high_priority_applied", "high_priority_no_change"))
...
#pdf(file="figures/early_career/4.16_PerceivedChangeResearchPriorityGovernment_13March18.pdf", height=7, width =11)
...
ggplot(data=per.change, aes(x=what.type, higher.priority, fill=what.type))+
  ...
  geom_text(aes(label=higher.priority, vjust=-0.5), size=3) +scale_y_continuous(expand = c(0, 0), limits = c(0,130))+ colScale
...
availability.change<-subset(part4.gbl.early, select=c("Location", "Country",'Country_work', "gender", "available_funding_fundamental",  
                                             "available_funding_use_inspired", "available_funding_applied"))
## switch to long format
availability.change.long.early<-gather(availability.change, what.type, level, -gender, -Location, -Country, -Country_work)
availability.early<-with(availability.change.long.early, data.frame(table(what.type, level)))

#remove non response
availability.early<-availability.early[!availability.early$level=="",]

## change type to factor with levels
availability.early$level<-str_replace_all(availability.early$level, "Will ", "")
availability.early$what.type<-as.factor(availability.early$what.type)
availability.early$level<-as.factor(availability.early$level)

## change order of levels
availability.early$level<-factor(availability.early$level, levels(availability.early$level)[c(4,5,6,3,2,1 )])
availability.early$what.type<-factor(availability.early$what.type, levels(availability.early$what.type)[c(2,3,1)])

availability.early$what.type<-revalue(availability.early$what.type, c("available_funding_fundamental"="Fundamental",
...
#pdf(file="figures/early_career/4.17_AnticipatedChangeFunding_13March18.pdf", height=7, width =11)

ggplot(data=availability.early, aes(x=level, Freq, fill=what.type))+
    geom_bar(stat="identity", position="dodge")+
...
## order bars into increase considerably (left) to decrease considerably (right), then no comment at the far right
next.generation<-subset(part4.gbl.early, select=c("Location", "Country","Country_work", "gender","next_generation"))
# remove non-response
...
#pdf(file="figures/early_career/4.18_EffectNextGeneration_13March18.pdf", height=7, width =11)
...
ggplot(data=impact, aes(x=Var1, Freq, fill=Var1))+ 
 ...
 labs(x="", y="Number of responses", title="Effect of funding changes on the next generation of researchers")+
  ...
  geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,50))+colScale
