...
#n.Cdn.res<-survey[survey$nation=="Canada",]
...
pdf(file="figures/canadian/4.1_CareerStage.pdf", width = 11, height= 7)
...
#pdf(file="figures/canadian/4.2_FieldofResearch.pdf", width = 11, height= 7)
...
g4.1<-ggplot(experience_can, aes(x = reorder(Var1, -Freq), Freq, fill=Var1)) + 
  geom_bar(stat='identity',position = position_dodge(width=0.5)) + 
  geom_text(aes(label=Freq, vjust=-0.40), size=3)+
...
g4.2<-ggplot(field.can, aes(x = reorder(field_research, -Freq), Freq, fill=field_research)) + 
    geom_bar(stat='identity',position = position_dodge(width=0.5)) + 
  labs(y="Number of responses",x="", title="")  +
  scale_x_discrete(labels=c("Natural\n Science", "Physical\n Science", "Medicine &\n Life Science", "Engineering", "Inter-\ndisciplinary\n Science", "Social Science/\n Humanities", "Other")) +
  geom_text(aes(label=Freq), vjust=-0.40, size=3) + 
  theme( aspect.ratio=1/2, axis.text.y=element_text(size=9),
         axis.text.x=element_text(angle=0, size=9),
         axis.title=element_text(size=11),
...
pdf(file="figures/canadian/4.1and4.2(old)_CareerStageAndFieldOfResearch_28may.pdf", width = 11, height= 7)
...
ggplot(res.plot, aes(percent, fill=ID)) +
    ...
    labs(x="Per cent", y="Number of responses")+
...
ggplot(res.plot, aes(percent, fill=ID)) +
    geom_histogram(position='dodge', binwidth=20, bins=seq(20, 100, 20))  +
    guides(fill=guide_legend(title=NULL, reverse=FALSE, nrow=2)) +
    labs(x="Per cent", y="Number of responses")+
    theme(aspect.ratio=3/4, panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),legend.text=element_text(size = 9),
...
```{r, echo=FALSE, fig.cap="Figure 4.3 OLD (now 4.1) Respondents type of research describe in proportal amounts of fundamental, use-inspired and applied research. Reseachers were questioned about the percentage of funding allocated to Fundamental, Use-inspired or Applied research in the past and in their current research.", message=FALSE, warning=FALSE, fig.width=10}
...
p1 <- ggplot(xs$Fundamental,aes(percent, fill=time)) +
    geom_histogram(position='dodge', binwidth=10, bins=seq(0, 100, 10))  +
    guides(fill=guide_legend(title=NULL, reverse=FALSE, nrow=2)) +
    labs(x="Per cent", y="Number of responses", title="")+
    theme(panel.grid.major = element_blank(),
...
p3 <- p1 %+% xs$Applied
p1<-p1+scale_fill_manual(name = "ID",values =mycols[4:5], labels=c("2006-2010", "2011-2015"))+labs(x="")+ theme(plot.margin=unit(c(1,1,-0.45,1), "cm"), plot.title = element_text(face="bold"))
...
pdf(file = "Figures/canadian/4.1a_1Jun.pdf", width = 11, height = 8)
...
# change order of the levels
sum.canada$Var1<-factor(sum.canada$Var1,levels(sum.canada$Var1)[c(2,3,1,5,4)])
...
p2<-ggplot(sum.canada, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  labs(y="")+
  scale_y_continuous(expand = c(0, 0), limits = c(0,435))+annotate('text', 5.4,420, label="(b)", size=5)+geom_text(aes(label=Freq), vjust=-0.5)
...
```{r, echo=FALSE, fig.cap="Figure 4.1 Type of research (4.3, 4.4, 4.5 but now just 4.4. 4.5 with a blank spot for 4.1a)", message=FALSE, warning=FALSE, fig.width=10}
# Setup
...
g2<-ggplot(sum.canada, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  theme( axis.text.x = element_text(angle=0, vjust=0.5, size=9), axis.text.y=element_text(size = 9), axis.title=element_text(size = 11),panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),legend.text=element_text(size = 9),axis.title.x=element_blank()) +guides(fill=FALSE) +scale_fill_manual(name = "Var1",values = myColors4)+
  labs(y="Number of responses", x="")+
  scale_y_continuous(expand = c(0, 0), limits = c(0,435))+annotate('text', 5.35,423, label="(b)", size=5)+
...
pdf(file="figures/canadian/4.1b.c._ResearchFoci_13apr_blank.a.pdf", width = 8, height = 11)
multiplot(g4, g2, g3, layout = matrix(c(1,1,2,3),nrow=2, byrow=TRUE))
...
```{r, echo=FALSE, fig.cap="Figure 4.2 Reason for change in research and opinion of change (4.4, 4.5)", message=FALSE, warning=FALSE, fig.width=10}
...
g2<-ggplot(sum.canada, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  theme( axis.text.x = element_text(angle=0, vjust=0.5, size=9), axis.text.y=element_text(size = 9), axis.title=element_text(size = 11),panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),legend.text=element_text(size = 9),axis.title.x=element_blank()) +guides(fill=FALSE) +scale_fill_manual(name = "Var1",values = myColors4)+
  labs(y="Number of responses", x="")+
  scale_y_continuous(expand = c(0, 0), limits = c(0,435))+annotate('text', 5.35,423, label="(a)", size=5)+
  scale_x_discrete(labels=c("Funding\n  ", "Interest\n  ", 
...
g3<-ggplot(change.view.ca, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
  theme(axis.text.x = element_text(angle=0, vjust=0.5, size=9), axis.text.y = element_text(size = 9), axis.title.y = element_text(size = 11), axis.title.x = element_text(size = 11)) +guides(fill=FALSE)+
  theme(panel.grid.major = element_blank(),legend.text=element_text(size = 9), panel.grid.minor = element_blank())+ labs(x=NULL, y="")+geom_text(aes(label=gender), vjust=-0.5, size=3.5)+scale_y_continuous(expand = c(0, 0), limits = c(0,137))+scale_fill_manual(name='view_change_of_type', values=myColors5)+annotate('text', 5.35,133, label="(b)", size=5)+scale_x_discrete(labels=c("Very\npositive", "Slightly\npositive", 
                                    "Neutral", "Slightly\nnegative", "Very\nnegative"),limits = rev(levels(change.view.ca$gender)))



#pdf(file="figures/4.2_ResearchFoci_29apr.pdf", width = 6.4, height = 2.25)
#grid.arrange(g2, g3, ncol=2)
g2.3<-arrangeGrob(g2, g3, ncol=2)
g2.3

ggsave("figures/canadian/4.2_FieldofResearch_28May17.pdf",g2.3, width = 6.4, height = 2.25, units = "in", scale=2)
...
p4<-ggplot(part, aes(x=Var1, y=Freq, fill = time),  ylim(0,375)) + 
  ...
  theme(panel.grid.major = element_blank(),
        ...
        legend.title=element_blank()) +
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,350))+scale_x_discrete(limits = rev(levels(part$Var1)))+annotate('text', 0.5,337, label="(a)", size=5)+labs(y="Number of responses")
p4
...
p5<-ggplot(sum.canada, aes(x=Var1, y=Freq, fill=Var1)) + 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,450))+annotate('text', 5.3,431, label="(b)", size=5)+scale_x_discrete(labels=c("Funding\n ", "Interest\n", 
                                    "Career\n", "Social\n", "Other\n "))
p5
...
p6<-ggplot(view, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+
  ...
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        axis.text.x = element_text(angle=0, vjust=0, size=9), axis.text.y=element_text(size = 9), axis.title.y=element_text(size=11)) + 
  labs(x="", y="") + colScale + scale_y_continuous(expand = c(0, 0), limits = c(0,165 ), breaks = seq(0,160,40))+scale_x_discrete(labels=c("Very\npositive", "Slightly\npositive", "Neutral\n", "Slightly\nnegative", "Very\nnegative"))+annotate('text', 0.65,158, label="(c)", size=5)
p6
...
pdf(file="figures/canadian/4.2_1June.pdf")
...
grants.ca$ID<-paste(grants.ca$type.grant, grants.ca$time)

# change order of the levels
grants.ca$ID<-as.factor(grants.ca$ID)
grants.ca$ID<-factor(grants.ca$ID, levels(grants.ca$ID)[c(4,3,6,5,2,1)])

pdf(file="figures/canadian/4.9(appendix)_12Apr.pdf", height=7, width =11)

library(gridExtra)
#####add (a)
xs <- split(grants.ca,f = grants.ca$type.grant)
...
pdf(file="figures/canadian/4.10(appendix)_13Apr.pdf", height=7, width =11)
...
```{r, echo=FALSE, fig.cap="Fig 4.11 (now 4.3) Change in grant success rates over the past 10 years. Researchers were asked if they thought that grant sucecss rates have changed in the past 10 years, for each research category.", warning=FALSE}
...
##switch to long format
change.long.ca<-gather(canada, type, level, -Location, -gender, -Country, -Country_work, -nation, -what_participant_group, -field_research)
...
pdf(file="figures/canadian/4.4_29May_2.pdf", height=7, width =11)
...
p10<-ggplot(prac.ca, aes(Var2, Freq, fill=Var1)) + 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,440))+annotate('text', 0.65,423, label="(a)", size=5)
p10
...
p11<-ggplot(part.ca, aes(Var2, Freq, fill=Var1))+ 
  ...
  scale_x_discrete(labels=c("Mandatory", "Quite\n important", 'Somewhat\n important',"Not very\n important", 'Not at all\n important', "Can't\n comment"))+
    theme(aspect.ratio=3/4, panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
          legend.key=element_blank(),legend.text=element_text(size = 9),
          legend.position=c(0.88, 0.898), axis.text.x=element_text(size = 9), axis.text.y=element_text(size=9),
          axis.title.x=element_text(size = 11), axis.title.y=element_text(size = 11)) + colScale+
  scale_y_continuous(expand = c(0, 0), limits = c(0,480))+annotate('text', 0.65,460, label="(b)", size=5)
p11

pdf(file="figures/canadian/4.3_13Apr.pdf", width = 11, height= 7)
...
pdf("Figures/canadian/test.pdf", width=40)
grid.draw(g)
dev.off()
...
p3<-ggplot(int, aes(percent, fill=year))+ geom_histogram(position='dodge', binwidth=10)  + colScale +
   guides(fill=guide_legend(title=NULL)) +
    theme(legend.position=c(0.922, 0.98),
          ...
          panel.grid.minor = element_blank(), axis.text.x=element_text(size = 9), axis.text.y=element_text(size = 9), legend.key=element_blank(),
          axis.title.x=element_text(size=11), axis.title.y=element_text(size = 11)) +
  labs(title="Internal",x="Per cent research funding", y = "Number of responses")+
  scale_y_continuous(expand =c(0,0), limits=c(0,1300), breaks=c(0, 250, 500, 750, 1000, 1250))+
  scale_x_continuous( breaks = seq(0, 100, 10))
p3
...
p4<-ggplot(ngov, aes(percent, fill=year))+ geom_histogram(position='dodge', binwidth=10)  + colScale +
   guides(fill=guide_legend(title=NULL)) +
    theme(legend.position=c(0.922, 0.98),
          ...
          panel.grid.minor = element_blank(), axis.text.x=element_text(size = 9), axis.text.y=element_text(size = 9), legend.key=element_blank(),
          axis.title.x=element_text(size=11), axis.title.y=element_text(size = 11)) +
  labs(title="Non-governmental", x="Per cent research funding", y = "")+
  scale_y_continuous(expand =c(0,0), limits=c(0,1300), breaks=c(0, 250, 500, 750, 1000, 1250))+
  scale_x_continuous( breaks = seq(0, 100, 10))
p4
...
p5<-ggplot(other, aes(percent, fill=year))+ geom_histogram(position='dodge', binwidth=10)  + colScale +
   guides(fill=guide_legend(title=NULL)) +
    theme(legend.position=c(0.922, 0.98),
          ...
          panel.grid.minor = element_blank(), axis.text.x=element_text(size = 9), axis.text.y=element_text(size = 9), legend.key=element_blank(),
          axis.title.x=element_text(size=11), axis.title.y=element_text(size = 11)) +
  labs(title="Other", x="Per cent research funding", y = "")+
  scale_y_continuous(expand =c(0,0), limits=c(0,1300), breaks=c(0, 250, 500, 750, 1000, 1250))+
  scale_x_continuous( breaks = seq(0, 100, 10))
p5
...
pdf(file="figures/canadian/4.14(appendix)_31May.pdf", height=7, width =11)
...
```{r, echo=TRUE, fig.cap="Fig 4.5 (now 4.6) Anticipated future of research funding."  - 4.15 + 4.16}
...
p13<-ggplot(data=perceive, aes(x=Var1, y=Freq, fill=Var1)) + 
      ...
      labs(x="", y="Number of responses", fill="")+
      scale_x_discrete(labels=c("Very\n important", "Somewhat\n important", 'Not very\n important', 'Not at all\n important', "Can't\n comment"))+
      geom_text(aes(label=Freq, vjust=-0.5), size=3.5) + guides(fill=FALSE)+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,550)) + colScale+annotate('text', 0.6,525, label="(a)", size=5)+ theme(plot.margin=unit(c(1,1,1,1), "cm"))
p13
...
p14<-ggplot(data=per.change, aes(x=what.type, higher.priority, fill=what.type))+
  ...
  geom_text(aes(label=higher.priority, vjust=-0.5), size=3.5) +scale_y_continuous(expand = c(0, 0), limits = c(0,1260))+ colScale+annotate('text', 0.6,1200, label="(b)", size=5)+ theme(plot.margin=unit(c(1,1,1,-1), "cm"))
p14

#pdf(file="figures/4.5_13Apr.pdf", width = 11, height = 7)

#grid.arrange(p13, p14, nrow=1,widths=c(0.535,0.465))
g13.14<-arrangeGrob(p13,p14,nrow=1,widths=c(0.53 ,0.47) )


ggsave("figures/canadian/4.5_1Jun17.pdf",g13.14, width = 6.4, height = 2.25, units = "in", scale=2)
...
```{r, echo=FALSE, fig.cap="Fig 4.17 (now 4.8) Anticipated change in research funding in next five years in Canada. Researchers were asked whether the availability of research funding would change in the next five years, for each research category. Responses were/were not significantly different between genders. "}
...
pdf(file="figures/canadian/4.17_1Jun.pdf", height=7, width =11)
...
ggsave("figures/canadian/4.18_25May17.pdf", width = 6.4, height = 2.4, units = "in", scale=2)
```
