...
##selecting for east asia and pacific countries
easia.pacific<-read.csv("../data/eastasia_pacific.csv")
easia.pacific <- as.data.frame(easia.pacific)
easia.pacific <- easia.pacific[ , 1]

ea.pac<-survey[survey$nation%in%easia.pacific,]

#check
unique(ea.pac$nation)
ea.pac <- droplevels(ea.pac)


#Gender:
n.ea.pac.res.men <- ea.pac[ea.pac$gender=="Male",]               #Count how many responses from  male researchers
percent.men<-((dim(n.ea.pac.res.men)[1])/(dim(ea.pac)[1]))*100      #Calculate the % of responses from  male researchers
n.ea.pac.res.women <- ea.pac[ea.pac$gender=="Female",]           #Count how many responses from  female researchers
percent.women<-((dim(n.ea.pac.res.women)[1])/(dim(ea.pac)[1]))*100  #Calculate the % of responses from  female researchers

#Career Stage Tabulations:
#table(ea.pac$what_participant_group)
n.seniorac <- ea.pac[ea.pac$what_participant_group=="Senior academic researcher with >10 years of experience applying for research grants",]
percent.seniorac<-((dim(n.seniorac)[1])/(dim(ea.pac)[1]))*100 #Calculate the % of responses from  senior academic researchers
n.earlyac <- ea.pac[ea.pac$what_participant_group=="Early career academic researcher with <10 years experience applying for research grants since completion of PhD",]
percent.earlyac<-((dim(n.earlyac)[1])/(dim(ea.pac)[1]))*100 #Calculate the % of responses from  early academic researchers
n.pdf <- ea.pac[ea.pac$what_participant_group=="Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future",]
percent.pdf<-((dim(n.pdf)[1])/(dim(ea.pac)[1]))*100 #Calculate the % of responses from  post-docs
n.nonACearly <- ea.pac[ea.pac$what_participant_group=="Non-academic researcher conducting or managing research in industry or government with <10 years of experience",]
percent.nonACearly<-((dim(n.nonACearly)[1])/(dim(ea.pac)[1]))*100 #Calculate the % of responses from early career non-academics
n.nonACsenior <- ea.pac[ea.pac$what_participant_group=="Non-academic researcher conducting or managing research in industry or government with >10 years of experience",]
percent.nonACsenior<-((dim(n.nonACsenior)[1])/(dim(ea.pac)[1]))*100 #Calculate the % of responses from early career non-academics
n.unk <- ea.pac[ea.pac$what_participant_group=="",]
percent.unk<-((dim(n.unk)[1])/(dim(ea.pac)[1]))*100 #Calculate the % of responses from unknown career stage

#Discipline Tabulations:
n.natural <- ea.pac[ea.pac$field_research=="Natural Science",]
percent.natural <- ((dim(n.natural)[1])/(dim(ea.pac)[1]))*100 #Calculate the % of responses from  natural scientists
n.physics <- ea.pac[ea.pac$field_research=="Physical Science (eg. math, physics, chemistry, computer science)",]
percent.physics <- ((dim(n.physics)[1])/(dim(ea.pac)[1]))*100 #Calculate the % of responses from  physical scientists
n.med <- ea.pac[ea.pac$field_research=="Medicine and Life Science",]
percent.med <- ((dim(n.med)[1])/(dim(ea.pac)[1]))*100 #Calculate the % of responses from  med/life scientists
n.eng <- ea.pac[ea.pac$field_research=="Engineering",]
percent.eng <- ((dim(n.eng)[1])/(dim(ea.pac)[1]))*100 #Calculate the % of responses from  engineering scientists
n.int <- ea.pac[ea.pac$field_research=="Interdisciplinary Science",]
percent.int <- ((dim(n.int)[1])/(dim(ea.pac)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists
n.ssh <- ea.pac[ea.pac$field_research=="Social Science / Humanities",]
percent.ssh <- ((dim(n.ssh)[1])/(dim(ea.pac)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists
...
locations<-aggregate(gender ~ nation, ea.pac, length)
locations<-locations[!locations$nation=="",]
locations<-droplevels(locations)

#pdf(file="figures/e.asia_pacific/responses_country_12Mar18.pdf", width = 11, height= 8)
...
ggplot(locations, aes(x = reorder(nation, -gender), gender)) + geom_bar(stat='identity',position = position_dodge(width=0.5),fill='#1f78b4') + theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5)) + labs(y="Number of responses", x="", title="Responses by Country")  + 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+scale_y_continuous(expand = c(0,0), limits=c(0, 7)) 
...
# 
# #pdf(file="figures/e.asia_pacific/responses_country_gapplot_1Mar18.pdf", height=8, width=11)
# 
# par(mar=c(11,4,4,2))
...
## >10 responses
# 
# locations.more<-aggregate(gender ~ nation, ea.pac, length)
...
# 
# 
# #pdf(file="figures/e.asia_pacific/responses_country_10+_gapplot_1Mar18.pdf", height=8, width=11)
# 
# par(mar=c(7,4,4,2))
...
# ## <=10 responses
# 
# locations.less<-aggregate(gender ~ nation, ea.pac, length)
...
# 
# color1<-c('#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4')
# #pdf(file="figures/e.asia_pacific/responses_country_10+&10-_gapplot_1Mar18.pdf", height=8, width=15)
...
```{r echo=FALSE, fig.cap="Figure 4.1 Number of  survey respondents by career stage", fig.width=7, message=FALSE, warning=FALSE}
...
experience<-data.frame(table(ea.pac$what_participant_group))
...
#pdf(file="figures/e.asia_pacific/4.1_CareerStage_13March18.pdf", width = 11, height= 7)

ggplot(experience, aes(x = reorder(Var1, -Freq), Freq, fill=Var1)) + 
  geom_bar(stat='identity',position = position_dodge(width=0.5)) + 
  geom_text(aes(label=Freq, vjust=-0.5), size=3)+
 scale_x_discrete(labels=c("Early career\nacademic",   "Senior academic",  "Postdoc or RA")) +
...
```{r echo=FALSE, fig.cap="Figure 4.2 Survey responses by field of research", fig.width=7, warning=FALSE, , message=FALSE}
field<-data.frame(table(ea.pac$field_research))
...
#pdf(file="figures/e.asia_pacific/4.2_FieldofResearch_13March18.pdf", width = 11, height= 7)

ggplot(field, aes(x = reorder(Var1, -Freq), Freq, fill=Var1)) + 
    geom_bar(stat='identity',position = position_dodge(width=0.5)) + 
  labs(y="Number of responses",x="", title=" Responses by field of research")  +
  scale_x_discrete(labels=c("Medicine &\nLife Science", "Interdisciplinary\nScience","Natural\n Science","Engineering",
...
```{r eval=FALSE, fig.cap="Figure 4.1 + 4.2 Career Stage and Field of research", message=FALSE,fig.width=11, fig.width=7, warning=FALSE, include=FALSE}
##plot1
...
g4.2<-ggplot(field, aes(x = reorder(Var1, -Freq), Freq, fill=Var1)) + 
  ...
  theme( aspect.ratio=1/2, axis.text.y=element_text(size=9),
         ...
         panel.grid.minor = element_blank(), plot.title = element_text(hjust = 0.5)) + 
  guides(fill=FALSE) + colScale+
  scale_y_continuous(expand = c(0,0), limits = c(0,830))

#pdf(file="figures/e.asia_pacific/4.1and4.2_CareerStageAndFieldOfResearch.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.3 Respondents type of research describe in proportal amounts of fundamental use-inspired and applied research. Reseachers were questioned about the percentage of funding allocated to Fundamental Use-inspired or Applied research in the past and in their current research.", fig.height=11, fig.width=7, message=FALSE, warning=FALSE}

# select for just east asia and pacific
research.ea.pac<-research[research$nation%in%easia.pacific,]
research.ea.pac <- droplevels(research.ea.pac)


## make sure that Use-inspired is the middle level in the legend
res.plot.ea.pac<-subset(research.ea.pac, type%in%c("percent_fundemental_research_current",  "percent_Applied_Research_current" , "percent_Use_inspired_Research_current","percent_Applied_Research_past",  "percent_Fundamental_Research_past" , "percent_Use_inspired_Research_past"))

res.plot.ea.pac$percent<-str_replace_all(res.plot.ea.pac$percent, "%", "")
res.plot.ea.pac$percent<-as.numeric(as.character(res.plot.ea.pac$percent))
res.plot.ea.pac$type<-as.character(res.plot.ea.pac$type)

# add time variable
res.plot.ea.pac$time<-ifelse(grepl( "current", res.plot.ea.pac$type),"Current", "Past")
res.plot.ea.pac$type[res.plot.ea.pac$type=="percent_Applied_Research_past"]<-"Applied"
res.plot.ea.pac$type[res.plot.ea.pac$type=="percent_Applied_Research_current"]<-"Applied"
res.plot.ea.pac$type[res.plot.ea.pac$type=="percent_Fundamental_Research_past"]<-"Fundamental"
res.plot.ea.pac$type[res.plot.ea.pac$type=="percent_fundemental_research_current"]<-"Fundamental"
res.plot.ea.pac$type[res.plot.ea.pac$type=="percent_Use_inspired_Research_past"]<-"Use-inspired"
res.plot.ea.pac$type[res.plot.ea.pac$type=="percent_Use_inspired_Research_current"]<-"Use-inspired"

# change order of the levels
res.plot.ea.pac$time<-as.factor(res.plot.ea.pac$time)
res.plot.ea.pac$time<-factor(res.plot.ea.pac$time, levels(res.plot.ea.pac$time)[c(2,1)])

res.plot.ea.pac$type<-as.factor(res.plot.ea.pac$type)
res.plot.ea.pac$type<-factor(res.plot.ea.pac$type, levels(res.plot.ea.pac$type)[c(2,3,1)])

res.plot.ea.pac$time<-factor(res.plot.ea.pac$time)
res.plot.ea.pac$time<-factor(res.plot.ea.pac$time, levels(res.plot.ea.pac$time)[c(1,2)])
...
res.plot.ea.pac$ID<-paste(res.plot.ea.pac$type, res.plot.ea.pac$time)

#table(res.plot.ea.pac$percent,res.plot.ea.pac$time,res.plot.ea.pac$type)
#write.csv(res.plot.ea.pac, file="data/4.1a.calculations.csv", row.names = FALSE)

# change order of the levels
res.plot.ea.pac$ID<-as.factor(res.plot.ea.pac$ID)
res.plot.ea.pac$ID<-factor(res.plot.ea.pac$ID, levels(res.plot.ea.pac$ID)[c(4,3,6,5,2,1)])

# add colours to res.plot
mycols<-c('#33a02c','#a6cee3','#fb9a99','#1f78b4', '#e31a1c','#b2df8a')
mycols<-as.data.frame(mycols)
mycols$ID<-levels(res.plot.ea.pac$ID)
res.plot.ea.pac$col<-mycols$mycols[match(res.plot.ea.pac$ID, mycols$ID)]
res.plot.ea.pac$col<-factor(res.plot.ea.pac$col, levels(res.plot.ea.pac$col)[c(3,1,4,2,6,5)])
mycols<-as.character(unique(res.plot.ea.pac$col))
...
#table(res.plot.ea.pac$type, res.plot.ea.pac$time, res.plot.ea.pac$col)

#pdf(file="figures/e.asia_pacific/4.3.1_ResearchProportion_11March18.pdf", width = 11, height= 7)

ggplot(res.plot.ea.pac, aes(percent, fill=ID)) +
  ...
  scale_x_continuous(breaks=seq(0,100,10))+scale_y_continuous(expand=c(0,0), limits=c(0,10),breaks=c(0,5,10))
...
```{r eval=FALSE, fig.cap="Figure 4.3 (different layout) Respondents type of research describe in proportal amounts of fundamental, use-inspired and applied research. Reseachers were questioned about the percentage of funding allocated to Fundamental, Use-inspired or Applied research in the past and in their current research.", message=FALSE,fig.width=11, fig.width=7, warning=FALSE, include=FALSE }
...
res.plot.ea.pac$type[res.plot.ea.pac$type=="percent_Use_inspired_Research_past"]<-"Use-inspired"
res.plot.ea.pac$type[res.plot.ea.pac$type=="percent_Use_inspired_Research_current"]<-"Use-inspired"
res.plot.ea.pac$time[res.plot.ea.pac$time=="Current"]<-"2011-2015"
res.plot.ea.pac$time[res.plot.ea.pac$time=="Past"]<-"2006-2010"
...
mycols$ID<-levels(res.plot.ea.pac$ID)
res.plot.ea.pac$col<-mycols$mycols[match(res.plot.ea.pac$ID, mycols$ID)]
res.plot.ea.pac$col<-factor(res.plot.ea.pac$col, levels(res.plot.ea.pac$col)[c(3,5,4,2,6,1)])
mycols<-as.character(unique(res.plot.ea.pac$col))
colScale <- scale_fill_manual(name = "ID",values =mycols, labels=c("2006-2010", "2011-2015"))

library(gridExtra)
#####add (a)
xs <- split(res.plot.ea.pac,f = res.plot.ea.pac$type)
...
#pdf(file="figures/e.asia_pacific/4.3.2_ResearchProportion_11March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.4a&b Change in research type proportions and the reasons. Researchers were asked to answer yes, no, or can't comment on if their type of research had changed in the last 10 years and to select what reasons for the change applied to them.", fig.width=7, fig.width=11,warning=FALSE, message=FALSE}
# select for just east asia and pacific
research.change.ea.pac<-research.change[research.change$nation%in%easia.pacific,]
research.change.ea.pac <- droplevels(research.change.ea.pac)
#Change yes or no
yes.no<-subset(research.change.ea.pac, select=c("Location","Country", "Country_work",  "gender", "changed_10yrs"))

ea.pac.yes.no<-aggregate(Country~ changed_10yrs, yes.no, length)
ea.pac.yes.no<-ea.pac.yes.no[!ea.pac.yes.no$changed_10yrs=="",]

p1<-ggplot(data=ea.pac.yes.no, aes(x=changed_10yrs, y=Country, fill=Country))+ geom_bar(stat='identity') + guides(fill=FALSE)+
  theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(), axis.title.x=element_blank(), plot.title = element_text(hjust = 0.5))+
  geom_text(aes(label=Country), vjust=-0.5)+labs(y="Number of responses", title="Change in research type")+ scale_y_continuous(expand = c(0, 0), limits = c(0,17))+ annotate('text', 0.5,1470, label="(a)", size=5)

####reason for change 
# select for just east asia and pacific
research.change.ea.pac<-research.change[research.change$nation%in%easia.pacific,]
research.change.ea.pac <- droplevels(research.change.ea.pac)
reason<-subset(research.change.ea.pac, select=c("Location","Country","Country_work",  "gender", "Main_reason_change_interest_related",
...
require(tidyr)
ea.pac.long<-gather(reason, reason.change.ea.pac, yes.ea.pac, -Location, -gender, -Country_work, -Country)
# using table to count cases of each category
sum.ea.pac<-data.frame(table(ea.pac.long$reason.change.ea.pac, ea.pac.long$yes.ea.pac))
#remove non-response
sum.ea.pac<-sum.ea.pac[!sum.ea.pac$Var1=="",]

sum.ea.pac$Var1<-revalue(sum.ea.pac$Var1, c("Main_reason_change_Career_related" = "Career", "Main_reason_change_Funding_related" = "Funding", "Main_reason_change_interest_related" = "Interest", "Main_reason_change_Other" = "Other", "Main_reason_change_Socially_related"  = "Social"))
# change order of the levels
sum.ea.pac$Var1<-factor(sum.ea.pac$Var1,levels(sum.ea.pac$Var1)[c(2,3,1,5,4)])

myColors4<-(brewer.pal(9, "Blues"))
myColors4<-myColors4[c(9, 8,7,6,5,3)]

p2<-ggplot(sum.ea.pac, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 15))+annotate('text', 5.4,885, label="(b)", size=5)+geom_text(aes(label=Freq), vjust=-0.5, size=3)


#pdf(file="figures/e.asia_pacific/4.4_ReasonForFieldChange_11March18.pdf", width = 11, height= 7)

#print(p2)

#dev.off()

#pdf(file="figures/e.asia_pacific/4.4a&b_ReasonForFieldChange_11March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.5 View of change in proportion of research. Researchers were asked how they viewed the change in the type of research they conduct/supervise.", fig.width=7, message=FALSE, warning=FALSE}

view.propor<-ea.pac[!ea.pac$view_change_of_type=="",]
view.propor<-droplevels(view.propor)
change.view.ea.pac<-aggregate(gender~ view_change_of_type, view.propor, length)
# change order of the levels
change.view.ea.pac$view_change_of_type<-factor(change.view.ea.pac$view_change_of_type, 
                                        levels(change.view.ea.pac$view_change_of_type)[c(5,3,1,2,4)])

myColors5<-(brewer.pal(9, "Blues"))
myColors5<-myColors5[c(4,4,4,4,4,4)]

#pdf(file="figures/e.asia_pacific/4.5_ViewFieldChange_11March18.pdf", width = 11, height= 7)

ggplot(change.view.ea.pac, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
  ...
  theme(panel.grid.major = element_blank(),legend.text=element_text(size = 9), panel.grid.minor = element_blank())+ labs(x="", y="Number of responses", title="Opinion of research type change")+geom_text(aes(label=gender), vjust=-0.5, size=3)+scale_y_continuous(expand = c(0, 0), limits = c(0,10))+scale_fill_manual(name='view_change_of_type', values=myColors5)#+annotate('text', 5.4,133, label="(c)", size=5)

#dev.off()
```

```{r eval=FALSE, fig.cap="Figure Type of research (4.3, 4.4, 4.5 )", fig.width=11, fig.width=7,, message=FALSE, warning=FALSE, include=FALSE}
...
```{r eval=FALSE, fig.cap="Figure 4.6 Current vs past level of partnership outside of academia. Researchers indicated the level of partnership that their current and past (10 years ago) research program had outside of academia).", message=FALSE, warning=FALSE, fig.width=11, fig.width=7,include=FALSE}

b.part<-data.frame(table(ea.pac$partnership_outside_before))
b.part<-b.part[!b.part$Var1=="",]
b.part<-droplevels(b.part)
b.part$time<-"Past"
a.part<-data.frame(table(ea.pac$partnership_outside))
...
#pdf(file="figures/e.asia_pacific/4.6_LevelsOfPartnership_11March18.pdf", width = 11, height= 7)
...
ggplot(part, aes(x=Var1, y=Freq, fill = time),  ylim(0,375)) + 
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,17))

#dev.off()
```

```{r echo=FALSE, fig.cap="Figure 4.7a&b Did it change and reasons for change in level of external research partnerships over the past decade.", fig.width=7, fig.height=11,  message=FALSE, warning=FALSE}

### was there change

change.part.ea.pac<-data.frame(table(ea.pac$partnership_change_10yrs))
change.part.ea.pac<-change.part.ea.pac[!change.part.ea.pac$Var1=="",]
change.part.ea.pac$Var1<-revalue(change.part.ea.pac$Var1, c("Can't comment (new researcher)"="Can't comment"))
change.part.ea.pac$Var1<-factor(change.part.ea.pac$Var1, levels=c("Yes", "No", "Can't comment"))
myColors<-(brewer.pal(9, "Blues"))
myColors<-myColors[c(9, 7,5)]
g1<-ggplot(data=change.part.ea.pac, aes(x=Var1, y=Freq, fill=Var1))+ 
...
### reason
# select for just east asia and pacific
part2.reason.ea.pac<-part2.reason[part2.reason$nation%in%easia.pacific,]
part2.reason.ea.pac <- droplevels(part2.reason.ea.pac)

reason.pt.long<-gather(part2.reason.ea.pac, change.reason, yes, -Country,-Country_work, -gender, -nation, -Location, -what_participant_group, -field_research)
sum.ea.pac<-data.frame(table(reason.pt.long$change.reason, reason.pt.long$yes))

sum.ea.pac$Var1<-revalue(sum.ea.pac$Var1, c("reason_partnership_change_career" = "Career", "reason_partnership_change_funding" = "Funding", "reason_partnership_change_interest" = "Interest", "reason_partnership_change_other" = "Other", "reason_partnership_change_socially"  = "Social"))
sum.ea.pac$Var1<-reorder(sum.ea.pac$Var1, -sum.ea.pac$Freq)

myColors7<-(brewer.pal(9, "Blues"))
myColors7<-myColors7[c(9, 8,7,6,5,3)]

g2<-ggplot(sum.ea.pac, aes(x=Var1, y=Freq, fill=Var1)) + 
...
#pdf(file="figures/e.asia_pacific/4.7a&b_PartnershipChange&Reason_11March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.8 View of change in external partnerships. Researchers were asked how they viewed the change in the level of partnership with external groups.", fig.width=7, fig.height=11, message=FALSE, warning=FALSE}

view<-data.frame(table(ea.pac$view_change_partnership))
view<-view[!view$Var1=="",]
...
#pdf(file="figures/e.asia_pacific/4.8_ViewChangePartnership_11March18.pdf", width = 11, height= 7)
...
ggplot(view, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+
  ...
  labs(x="", y="Number of responses", title="Opinion of change in partnership level") + colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,6), breaks = seq(0,6,2))
...
```{r echo=FALSE, fig.cap="Fig. 4.9 Number of research grant applications by research category in 2006-2010 and 2011-2015.", fig.height=11, fig.width=7, message=FALSE, warning=FALSE}
part3.grants.long<-read.csv(file="../data/gya-part3.grants.long.csv")

# select for just east asia and pacific
part3.grants.long.ea.pac<-part3.grants.long[part3.grants.long$nation%in%easia.pacific,]
part3.grants.long.ea.pac <- droplevels(part3.grants.long.ea.pac)

part3.grants.long.ea.pac$type.grant<-as.character(part3.grants.long.ea.pac$type.grant)


#add time variable
part3.grants.long.ea.pac$time<-ifelse(grepl("11_15", part3.grants.long.ea.pac$type.grant), "2011-2015", "2006-2010")
part3.grants.long.ea.pac$type.grant[part3.grants.long.ea.pac$type.grant=="external_pi_grant_11_15_fundamental"]<-"Fundamental"
part3.grants.long.ea.pac$type.grant[part3.grants.long.ea.pac$type.grant=="external_pi_grant_6_10_fundamental"]<-"Fundamental"
part3.grants.long.ea.pac$type.grant[part3.grants.long.ea.pac$type.grant=="external_pi_grant_11_15_use"]<-"Use-Inspired"
part3.grants.long.ea.pac$type.grant[part3.grants.long.ea.pac$type.grant=="external_pi_grant_6_10_use"]<-"Use-Inspired"
part3.grants.long.ea.pac$type.grant[part3.grants.long.ea.pac$type.grant=="external_pi_grant_11_15_applied"]<-"Applied"
part3.grants.long.ea.pac$type.grant[part3.grants.long.ea.pac$type.grant=="external_pi_grant_6_10_applied"]<-"Applied"

grants.ea.pac<-data.frame(with(part3.grants.long.ea.pac, table(type.grant,time,number)))
grants.ea.pac$type.grant<-as.factor(grants.ea.pac$type.grant)


#change order of levels
grants.ea.pac$number<-factor(grants.ea.pac$number, levels(grants.ea.pac$number)[c(1,2,6,7,3,4,5)])
grants.ea.pac$type.grant<-factor(grants.ea.pac$type.grant, levels(grants.ea.pac$type.grant)[c(2,3,1)])

#myColors9<-(brewer.pal(9, "Blues"))
#myColors9<-rev(myColors9[c(9, 5)])
#colScale <- scale_fill_manual(name = "time",values = myColors9)

grants.ea.pac$ID<-paste(grants.ea.pac$type.grant, grants.ea.pac$time)

# change order of the levels
grants.ea.pac$ID<-as.factor(grants.ea.pac$ID)
grants.ea.pac$ID<-factor(grants.ea.pac$ID, levels(grants.ea.pac$ID)[c(4,3,6,5,2,1)])

##colors
mycols<-c('#33a02c','#a6cee3','#fb9a99','#1f78b4', '#e31a1c','#b2df8a')
#mycols<-as.data.frame(mycols)

#pdf(file="figures/e.asia_pacific/4.9_NumberGrantApps_11March18.pdf", height=7, width =11)


#####add (a)
xs <- split(grants.ea.pac,f = grants.ea.pac$type.grant)
...
```{r echo=FALSE, fig.cap="Fig 4.10 Research grant application success over the past 10 years. Researchers were asked to estimate the percentage of their research grant applications that were successful, in 2006-2010 and in 2011-2015. Respondents also had the choice to answer No need for applications for this research type.", message=FALSE, fig.height=11, fig.width=7}
part3.success.long<-read.csv(file="../data/gya-part3.success.long.csv")

# select for just east asia and pacific
part3.success.long.ea.pac<-part3.success.long[part3.success.long$nation%in%easia.pacific,]
part3.success.long.ea.pac <- droplevels(part3.success.long.ea.pac)

part3.success.long.ea.pac$type<-as.character(part3.success.long.ea.pac$type)

#add time variable
part3.success.long.ea.pac$time<-ifelse(grepl("11_15", part3.success.long.ea.pac$type), "2011-2015", "2006-2010")
part3.success.long.ea.pac$type[part3.success.long.ea.pac$type=="successful_grants_11_15_fundamental"]<-"Fundamental"
part3.success.long.ea.pac$type[part3.success.long.ea.pac$type=="successful_grants_6_10_fundamental"]<-"Fundamental"
part3.success.long.ea.pac$type[part3.success.long.ea.pac$type=="successful_grants_11_15_use"]<-"Use-Inspired"
part3.success.long.ea.pac$type[part3.success.long.ea.pac$type=="successful_grants_6_10_use"]<-"Use-Inspired"
part3.success.long.ea.pac$type[part3.success.long.ea.pac$type=="successful_grants_11_15_applied"]<-"Applied"
part3.success.long.ea.pac$type[part3.success.long.ea.pac$type=="successful_grants_6_10_applied"]<-"Applied"

success.ea.pac<-data.frame(with(part3.success.long.ea.pac, table(type, percent,time)))



#change order of types
success.ea.pac$type<-as.factor(success.ea.pac$type)
success.ea.pac$type<-factor(success.ea.pac$type, levels(success.ea.pac$type)[c(2,3,1)])

success.ea.pac$id<-paste(success.ea.pac$type, success.ea.pac$time)

# change order of the levels
success.ea.pac$id<-as.factor(success.ea.pac$id)
success.ea.pac$id<-factor(success.ea.pac$id, levels(success.ea.pac$id)[c(3,4,5,6,1,2)])

mycols10<-c('#1f78b4','#a6cee3','#33a02c','#b2df8a','#e31a1c', '#fb9a99')
mycols10<-as.data.frame(mycols10)
mycols10$id<-levels(success.ea.pac$id)
success.ea.pac$col<-mycols10$mycols10[match(success.ea.pac$id, mycols10$id)]
success.ea.pac$col<-factor(success.ea.pac$col, levels(success.ea.pac$col)[c(3,1,4,2,6,5)])

#pdf(file="figures/e.asia_pacific/4.10_GrantAppSuccess_11March18.pdf", height=7, width =11)

out2<-by(data = success.ea.pac, INDICES = success.ea.pac$type, FUN = function(n){
  n <- droplevels(n)
...
```{r echo=FALSE, fig.cap="Fig 4.11 Change in grant success rates over the past 10 years. Researchers were asked if they thought that grant sucecss rates have changed in the past 10 years, for each research category.", message=FALSE, fig.height=11, fig.width=7, message=FALSE, warning=FALSE}

# select for just east asia and pacific
part3.change.ea.pac<-part3.change[part3.change$nation%in%easia.pacific,]
part3.change.ea.pac <- droplevels(part3.change.ea.pac)

##switch to long format
change.long.ea.pac<-gather(part3.change.ea.pac, type, level, -Location, -gender, -Country,-nation, -Country_work, -what_participant_group, -field_research)
success.change<-with(change.long.ea.pac, data.frame(table(type, level)))
#remove no response
...
#pdf(file="figures/e.asia_pacific/4.11_ChangeSuccessRate_11March18.pdf", height=7, width =11)
...
```{r echo=FALSE, fig.cap="Fig 4.12 Importance of practical application of research over the past 10 years. Researchers were asked how important it was to suggest practical applications of their research to ensure that the grant was successful, in 2006-2010 and in 2011-2015.", fig.height=11, fig.width=7, message=FALSE, warning=FALSE}

# select for just east asia and pacific
part3.prac.long.ea.pac<-part3.prac.long[part3.prac.long$nation%in%easia.pacific,]
part3.prac.long.ea.pac <- droplevels(part3.prac.long.ea.pac)

part3.prac.long.ea.pac$year<-as.character(part3.prac.long.ea.pac$year)

part3.prac.long.ea.pac$year[part3.prac.long.ea.pac$year=="practical_applications_important_11_15"]<-"2011-2015"
part3.prac.long.ea.pac$year[part3.prac.long.ea.pac$year=="practical_applications_important_6_10"]<-"2006-2010"


prac.ea.pac<-data.frame(table(part3.prac.long.ea.pac$year, part3.prac.long.ea.pac$level))

#change order of levels
prac.ea.pac$Var2<-factor(prac.ea.pac$Var2, levels(prac.ea.pac$Var2)[c(2,6,4,5,3,1)])

myColors12<-(brewer.pal(9, "Blues"))
myColors12<-rev(myColors12[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors12)

#pdf(file="figures/e.asia_pacific/4.12_ImportancePracticalApplication_11March18.pdf", height=7, width =11)

ggplot(prac.ea.pac, aes(Var2, Freq, fill=Var1)) + 
...
```{r echo=FALSE, fig.cap="Fig 4.13 Importance of including partners from for-profit or non-governmental sectors in grant success. Researchers were asked how important it was to include external partnerships in their research to ensure that the grant was successful, in 2006-2010 and in 2011-2015.", fig.height=11, fig.width=7, warning=FALSE,  message=FALSE}

# select for just east asia and pacific
part3.part.long.ea.pac<-part3.part.long[part3.part.long$nation%in%easia.pacific,]
part3.part.long.ea.pac <- droplevels(part3.part.long.ea.pac)

part3.part.long.ea.pac$year<-as.character(part3.part.long.ea.pac$year)

part3.part.long.ea.pac$year[part3.part.long.ea.pac$year=="include_nonacademia_partners_success_11_15"]<-"2011-2015"
part3.part.long.ea.pac$year[part3.part.long.ea.pac$year=="include_nonacademia_partners_success_6_10"]<-"2006-2010"

part.ea.pac<-data.frame(table(part3.part.long.ea.pac$year, part3.part.long.ea.pac$level))
#change order of levels
part.ea.pac$Var2<-factor(part.ea.pac$Var2, levels(part.ea.pac$Var2)[c(2,5,6,4,3,1)])
...
#pdf(file="figures/e.asia_pacific/4.13_ImportancePartnersApplication_11March18.pdf", height=7, width =11)

ggplot(part.ea.pac, aes(Var2, Freq, fill=Var1))+ 
...
```{r eval=FALSE, fig.cap="Fig (4.12+4.13) Importance of practical applications and external partnerships.", message=FALSE, warning=FALSE, fig.width=11, fig.width=7,include=FALSE}
###4.12
myColors12<-(brewer.pal(9, "Blues"))
myColors12<-rev(myColors12[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors12)
...
```{r echo=FALSE, fig.cap="Fig 4.14 Distribution of research funding over the past 10 years. Researchers were asked to estimate the distribution of their research funding sources in 2006-2010 and 2011-2015.", fig.height=11, fig.width=7, message=FALSE, warning=FALSE}
p3_master.long<-read.csv(file="../data/gya-p3_master.long.csv")
# select for just east asia and pacific
p3_master.long.ea.pac<-p3_master.long[p3_master.long$nation%in%easia.pacific,]
p3_master.long.ea.pac <- droplevels(p3_master.long.ea.pac)

##revalue the year values
p3_master.long.ea.pac$year<- revalue(p3_master.long.ea.pac$year, c("11_15"="2011-2015", "6_10"="2006-2010"))
#change order of levels
p3_master.long.ea.pac$year<-factor(p3_master.long.ea.pac$year, levels(p3_master.long.ea.pac$year)[c(2,1)])

##For-Profit
fp<-p3_master.long.ea.pac[p3_master.long.ea.pac$type=="For-Profit",]
...
##Government
gov<-p3_master.long.ea.pac[p3_master.long.ea.pac$type=="Government",]
...
int<-p3_master.long.ea.pac[p3_master.long.ea.pac$type=="Internal",]
...
ngov<-p3_master.long.ea.pac[p3_master.long.ea.pac$type=="Non-governmental",]
...
other<-p3_master.long.ea.pac[p3_master.long.ea.pac$type=="Other",]
...
#pdf(file="figures/e.asia_pacific/4.14_DistributionFunding_11March18.pdf", height=7, width =11)
...
```{r echo=FALSE, fig.cap="Fig 4.15 Perceived importance of fundamental research to their government. Researchers were asked how important they thought fundamental research was to the their government. Responses were/were not significantly different between genders.", fig.height=11, fig.width=7, message=FALSE, warning=FALSE}
# select for just east asia and pacific
part4.ea.pac<-part4[part4$nation%in%easia.pacific,]
part4.ea.pac <- droplevels(part4.ea.pac)

## order bars from very important (left) to can't comment (right)
important<-subset(part4.ea.pac, select=c("Location","Country",'Country_work', "gender","opinion_fundamental_important"))
...
#pdf(file="figures/e.asia_pacific/4.15_PerceivedImportanceFundamentalGovernment_11March18.pdf", height=7, width =11)
...
ggplot(data=perceive, aes(x=Var1, y=Freq, fill=Var1)) + 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,8)) + colScale

#dev.off()
```

```{r echo=FALSE, fig.cap="Fig 4.16 Perceived change in research priority by their government. Researchers were asked whether any types of research had become higher priority for the their government. Responses were/were not significantly different between genders.", fig.height=11, fig.width=7, message=FALSE, warning=FALSE}
# select for just east asia and pacific
part4.ea.pac<-part4[part4$nation%in%easia.pacific,]
part4.ea.pac <- droplevels(part4.ea.pac)
## order bars into applied > use > fundamental > no change
priority<-subset(part4.ea.pac, select=c("Location","Country",'Country_work', "gender", "high_priority_fundamental", "high_priority_use_inspired", "high_priority_applied", "high_priority_no_change"))

## switch to long format
priority.long<-gather(priority, what.type, higher.priority, -Location, -gender, -Country, -Country_work)
...
#pdf(file="figures/e.asia_pacific/4.16_PerceivedChangeResearchPriorityGovernment_11March18.pdf", height=7, width =11)
...
ggplot(data=per.change, aes(x=what.type, higher.priority, fill=what.type))+
  ...
  geom_text(aes(label=higher.priority, vjust=-0.5), size=3) +scale_y_continuous(expand = c(0, 0), limits = c(0,20))+ colScale

#dev.off()
```


```{r eval=FALSE, fig.cap="Fig 4.17 Anticipated change in research funding in next five years. Researchers were asked whether the availability of research funding would change in the next five years.", message=FALSE, warning=FALSE, fig.width=11, fig.width=7,include=FALSE}
# select for just east asia and pacific
part4.ea.pac<-part4[part4$nation%in%easia.pacific,]
part4.ea.pac <- droplevels(part4.ea.pac)
...
availability.change<-subset(part4.ea.pac, select=c("Location", "Country",'Country_work', "gender", "available_funding_fundamental",  
                                             "available_funding_use_inspired", "available_funding_applied"))
## switch to long format
availability.change.long.ea.pac<-gather(availability.change, what.type, level, -gender, -Location, -Country, -Country_work)
availability.ea.pac<-with(availability.change.long.ea.pac, data.frame(table(what.type, level)))

#remove non response
availability.ea.pac<-availability.ea.pac[!availability.ea.pac$level=="",]

## change type to factor with levels
availability.ea.pac$level<-str_replace_all(availability.ea.pac$level, "Will ", "")
availability.ea.pac$what.type<-as.factor(availability.ea.pac$what.type)
availability.ea.pac$level<-as.factor(availability.ea.pac$level)

## change order of levels
availability.ea.pac$level<-factor(availability.ea.pac$level, levels(availability.ea.pac$level)[c(4,5,6,3,2,1 )])
availability.ea.pac$what.type<-factor(availability.ea.pac$what.type, levels(availability.ea.pac$what.type)[c(2,3,1)])

availability.ea.pac$what.type<-revalue(availability.ea.pac$what.type, c("available_funding_fundamental"="Fundamental",
...
#pdf(file="figures/e.asia_pacific/4.17_AnticipatedChangeFunding_11March18.pdf", height=7, width =11)

ggplot(data=availability.ea.pac, aes(x=level, Freq, fill=what.type))+
    geom_bar(stat="identity", position="dodge")+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,11), breaks = seq(0,10,2))
...
#pdf(file="figures/e.asia_pacific/4.18_EffectNextGeneration_11March18.pdf", height=7, width =11)
...
ggplot(data=impact, aes(x=Var1, Freq, fill=Var1))+ 
 ...
 labs(x="", y="Number of responses", title="Effect of funding changes on the next generation of researchers")+
  ...
  geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,6))+colScale
