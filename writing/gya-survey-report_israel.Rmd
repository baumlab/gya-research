---
title: "Israeli online survey on fundamental research"
author: ''
...
**Introduction**  
In addition to the official data presented in Chapters 2 and 3, we also developed and ran a quantitative online survey to query researchers about their perceptions of, and experiences with, funding for fundamental research. An important aim of the survey was to provide an understanding researcher's personal experiences and outlook on the research funding landscape in Israel. We had an excellent response to the survey, with over xxxx Israeli researchers completing it, suggesting that fundamental research funding is a high priority topic for Canadian researchers. Herein, we detail the survey questions and results. 
...
To extract survey responses for Israeli researchers, we selected all respondents who reported 'Israel' as their country of work, as well as those respondents who did not report a country of work (i.e. field was blank) but whose location was within Israel. We conducted all statistical analyses in the open source software R (Version 3.3.1).
**Note that numbers not all the same because respondents did not always answer every question**
```{r, echo=FALSE, message=FALSE, loading data, warning=FALSE}
...
n.is.res <- survey[survey$Country_work=="Israel" | (!(survey$Country_work=="Israel") & 
             survey$Country_work=="" & survey$Country=="Israel"),]  #Count how many responses from Israel researchers
...
#Gender:
n.is.res.men <- n.is.res[n.is.res$gender=="Male",]               #Count how many responses from Israel male researchers
percent.men<-((dim(n.is.res.men)[1])/(dim(n.is.res)[1]))*100      #Calculate the % of responses from Israel male researchers
n.is.res.women <- n.is.res[n.is.res$gender=="Female",]           #Count how many responses from Israel female researchers
percent.women<-((dim(n.is.res.women)[1])/(dim(n.is.res)[1]))*100  #Calculate the % of responses from Israel female researchers

#Career Stage Tabulations:
#table(n.is.res$what_participant_group)
n.is.seniorac <- n.is.res[n.is.res$what_participant_group=="Senior academic researcher with >10 years of experience applying for research grants",]
percent.seniorac<-((dim(n.is.seniorac)[1])/(dim(n.is.res)[1]))*100 #Calculate the % of responses from Israeli senior academic researchers
n.is.earlyac <- n.is.res[n.is.res$what_participant_group=="Early career academic researcher with <10 years experience applying for research grants since completion of PhD",]
percent.earlyac<-((dim(n.is.earlyac)[1])/(dim(n.is.res)[1]))*100 #Calculate the % of responses from Israeli early academic researchers
n.is.pdf <- n.is.res[n.is.res$what_participant_group=="Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future",]
percent.pdf<-((dim(n.is.pdf)[1])/(dim(n.is.res)[1]))*100 #Calculate the % of responses from Israeli post-docs
n.is.nonACearly <- n.is.res[n.is.res$what_participant_group=="Non-academic researcher conducting or managing research in industry or government with <10 years of experience",]
percent.nonACearly<-((dim(n.is.nonACearly)[1])/(dim(n.is.res)[1]))*100 #Calculate the % of responses from early career non-academics
n.is.nonACsenior <- n.is.res[n.is.res$what_participant_group=="Non-academic researcher conducting or managing research in industry or government with >10 years of experience",]
percent.nonACsenior<-((dim(n.is.nonACsenior)[1])/(dim(n.is.res)[1]))*100 #Calculate the % of responses from early career non-academics
n.is.unk <- n.is.res[n.is.res$what_participant_group=="",]
percent.unk<-((dim(n.is.unk)[1])/(dim(n.is.res)[1]))*100 #Calculate the % of responses from unknown career stage

#Discipline Tabulations:
n.natural <- n.is.res[n.is.res$field_research=="Natural Science",]
percent.natural <- ((dim(n.natural)[1])/(dim(n.is.res)[1]))*100 #Calculate the % of responses from Israeli natural scientists
n.physics <- n.is.res[n.is.res$field_research=="Physical Science (eg. math, physics, chemistry, computer science)",]
percent.physics <- ((dim(n.physics)[1])/(dim(n.is.res)[1]))*100 #Calculate the % of responses from Israeli physical scientists
n.med <- n.is.res[n.is.res$field_research=="Medicine and Life Science",]
percent.med <- ((dim(n.med)[1])/(dim(n.is.res)[1]))*100 #Calculate the % of responses from Israeli med/life scientists
n.eng <- n.is.res[n.is.res$field_research=="Engineering",]
percent.eng <- ((dim(n.eng)[1])/(dim(n.is.res)[1]))*100 #Calculate the % of responses from Israeli engineering scientists
n.int <- n.is.res[n.is.res$field_research=="Interdisciplinary Science",]
percent.int <- ((dim(n.int)[1])/(dim(n.is.res)[1]))*100 #Calculate the % of responses from Israeli interdisciplinary scientists
n.ssh <- n.is.res[n.is.res$field_research=="Social Science / Humanities",]
percent.ssh <- ((dim(n.ssh)[1])/(dim(n.is.res)[1]))*100 #Calculate the % of responses from Israeli interdisciplinary scientists
...
<!-- Generate Israeli career stage figure -->
```{r, echo=FALSE, message=FALSE, fig.cap="Figure 4.1 Number of Israeli survey respondents by career stage", warning=FALSE, fig.width=14}
...
is.field<-survey.what[survey.what$Country_work=="Israel" | (!(survey.what$Country_work=="Israel") & survey.what$Country_work=="" & survey.what$Country=="Israel"),]

experience_is<-data.frame(table(is.field$what_participant_group))
experience_is<-experience_is[!experience_is$Var1=="",]
experience_is<-droplevels(experience_is)

experience_is$Var1<-revalue(experience_is$Var1, c("Early career academic researcher with <10 years experience applying for research grants since completion of PhD" = "Early career academic researcher", "Non-academic researcher conducting or managing research in industry or government with >10 years of experience" = "Non-academic research > 10 yrs experience", "Non-academic researcher conducting or managing research in industry or government with <10 years of experience" = "Non-academic researcher < 10 yrs experience", "Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future" = "Postdoc or RA", "Senior academic researcher with >10 years of experience applying for research grants" = "Senior academic researcher"))
experience_is$Var1<-reorder(experience_is$Var1, experience_is$Freq)
...
#pdf(file="figures/4.1_CareerStage.pdf", width = 11, height= 7)

ggplot(experience_is, aes(x = reorder(Var1, -Freq), Freq, fill=Var1)) + 
...
<!-- Generate Israeli research discipline figure -->
```{r, echo=FALSE, fig.cap="Figure 4.2 Survey responses by field of research", warning=FALSE, fig.width=14}
field.is<-with(survey.what[survey.what$Country_work=="Israel" | (!(survey.what$Country_work=="Israel") & survey.what$Country_work=="" & survey.what$Country=="Israel"),], data.frame(table(field_research)))
field.is<-field.is[!field.is$field_research=="",]
field.is$field_research<-reorder(field.is$field_research, field.is$Freq)
field.is$field_research<-revalue(field.is$field_research, c("Physical Science (eg. math, physics, chemistry, computer science)" = "Physical Science"))
...
#pdf(file="figures/4.2_FieldofResearch.pdf", width = 11, height= 7)

ggplot(field.is, aes(x = reorder(field_research, -Freq), Freq, fill=field_research)) + 
    geom_bar(stat='identity',position = position_dodge(width=0.5)) + 
...
```{r, echo=FALSE, fig.cap="Figure 4.1 + 4.2 Career Stage and Field of research", warning=FALSE, fig.width=14}
##plot1
...
res.plot<-res.plot[res.plot$Country_work=="Israel" | (!(res.plot$Country_work=="Israel") & res.plot$Country_work=="" & res.plot$Country=="Israel"),]
...
ggplot(res.plot, aes(percent, fill=ID)) +
  ...
  scale_x_continuous(breaks=seq(0,100,10))+scale_y_continuous(expand=c(0,0), limits=c(0,35),breaks=c(0,10,20,30)) +annotate('text',5.4 ,294, label="(a)", size=5)
...
#pdf(file = "Figures/4.1a_29Apr.pdf", width = 11, height = 8)
...
israel<-yes.no[yes.no$Country_work=="Israel" | (!(yes.no$Country_work=="Israel") & 
                                                  yes.no$Country_work=="" & yes.no$Country=="Israel"),]
israel.yesno<-aggregate(Country~ changed_10yrs, israel, length)
israel.yesno<-israel.yesno[!israel.yesno$changed_10yrs=="",]

p1<-ggplot(data=israel.yesno, aes(x=changed_10yrs, y=Country, fill=Country))+ geom_bar(stat='identity') + guides(fill=FALSE)+
  theme(panel.grid.major = element_blank(),
...
israel<-reason[reason$Country_work=="Israel" | (!(reason$Country_work=="Israel") & 
                                                  reason$Country_work=="" & reason$Country=="Israel"),]
# switch to long format
require(tidyr)
israel.long<-gather(israel, reason.change.is, yes.is, -Location, -gender, -Country_work, -Country)
# using table to count cases of each category
sum.israel<-data.frame(table(israel.long$reason.change.is, israel.long$yes.is))
#remove non-response
sum.israel<-sum.israel[!sum.israel$Var1=="",]

sum.israel$Var1<-revalue(sum.israel$Var1, c("Main_reason_change_Career_related" = "Career", "Main_reason_change_Funding_related" = "Funding", "Main_reason_change_interest_related" = "Interest", "Main_reason_change_Other" = "Other", "Main_reason_change_Socially_related"  = "Social"))
# change order of the levels
sum.israel$Var1<-factor(sum.israel$Var1,levels(sum.israel$Var1)[c(2,3,1,5,4)])

myColors4<-(brewer.pal(9, "Blues"))
myColors4<-myColors4[c(9, 8,7,6,5,3)]

p2<-ggplot(sum.israel, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  labs(y="")+
  scale_y_continuous(expand = c(0, 0), limits = c(0,42))+annotate('text', 5.4,420, label="(b)", size=5)+geom_text(aes(label=Freq), vjust=-0.5)
...
israel<-part1.view[part1.view$Country_work=="Israel" | (!(part1.view$Country_work=="Israel") & 
                                                          part1.view$Country_work=="" & part1.view$Country=="Israel"),]
israel<-israel[!israel$view_change_of_type=="",]
israel<-droplevels(israel)
change.view.is<-aggregate(gender~ view_change_of_type, israel, length)
# change order of the levels
change.view.is$view_change_of_type<-factor(change.view.is$view_change_of_type, 
                                        levels(change.view.is$view_change_of_type)[c(5,3,1,2,4)])

myColors5<-(brewer.pal(9, "Blues"))
myColors5<-myColors5[c(4,4,4,4,4,4)]

ggplot(change.view.is, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
  ...
  theme(panel.grid.major = element_blank(),legend.text=element_text(size = 9), panel.grid.minor = element_blank())+ labs(x="", y="Number of responses")+geom_text(aes(label=gender), vjust=-0.5, size=3)+scale_y_continuous(expand = c(0, 0), limits = c(0,20))+scale_fill_manual(name='view_change_of_type', values=myColors5)+annotate('text', 5.4,133, label="(c)", size=5)

```

```{r, echo=FALSE, fig.cap="Figure Type of research (4.3, 4.4, 4.5 )", message=FALSE, warning=FALSE, fig.width=10}
...
## newer version - update not using this version anymore
# add colours to res.plot
mycols<-c('#33a02c','#a6cee3','#fb9a99','#1f78b4', '#e31a1c','#b2df8a')
mycols<-as.data.frame(mycols)
mycols$ID<-levels(res.plot$ID)
...
```{r, echo=FALSE, fig.cap="Figure Reason for change in research and opinion of change (4.4, 4.5)", message=FALSE, warning=FALSE, fig.width=10}
...
g2<-ggplot(sum.israel, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  theme( axis.text.x = element_text(angle=0, vjust=0.5, size=9), axis.text.y=element_text(size = 9), axis.title=element_text(size = 11),panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),legend.text=element_text(size = 9),axis.title.x=element_blank()) +guides(fill=FALSE) +scale_fill_manual(name = "Var1",values = myColors4)+
  labs(y="Number of responses", x="")+
  scale_y_continuous(expand = c(0, 0), limits = c(0,43))+annotate('text', 5.35,423, label="(a)", size=5)+
  scale_x_discrete(labels=c("Funding\n  ", "Interest\n  ", 
                                    "Career\n  ", "Social\n  ", "Other\n  "))+geom_text(aes(label=Freq), vjust=-0.5, size=3)
...
g3<-ggplot(change.view.is, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
  theme(axis.text.x = element_text(angle=0, vjust=0.5, size=9), axis.text.y = element_text(size = 9), axis.title.y = element_text(size = 11), axis.title.x = element_text(size = 11)) +guides(fill=FALSE)+
  theme(panel.grid.major = element_blank(),legend.text=element_text(size = 9), panel.grid.minor = element_blank())+ labs(x=NULL, y="")+geom_text(aes(label=gender), vjust=-0.5, size=3)+scale_y_continuous(expand = c(0, 0), limits = c(0,20))+scale_fill_manual(name='view_change_of_type', values=myColors5)+annotate('text', 5.35,133, label="(b)", size=5)+scale_x_discrete(labels=c("Very\npositive", "Slightly\npositive", 
                                    "Neutral", "Slightly\nnegative", "Very\nnegative"),limits = rev(levels(change.view.ca$gender)))



#pdf(file="figures/4.2_ResearchFoci_29apr.pdf", width = 11, height = 8)
...
part2.b.a<-part2.b.a[part2.b.a$Country_work=="Israel" | (!(part2.b.a$Country_work=="Israel") & part2.b.a$Country_work=="" & part2.b.a$Country=="Israel"),]
...
ggplot(part, aes(x=Var1, y=Freq, fill = time),  ylim(0,375)) + 
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,35))
```
...
### was there change
israel<-subset(part2.change, Country=="Israel")
israel<-part2.change[part2.change$Country_work=="Israel" | (!(part2.change$Country_work=="Israel") & part2.change$Country_work=="" & part2.change$Country=="Israel"),]
change.part.is<-data.frame(table(israel$partnership_change_10yrs))
change.part.is$Var1<-revalue(change.part.is$Var1, c("Can't comment (new researcher)"="Can't comment"))
change.part.is$Var1<-factor(change.part.is$Var1, levels=c("Yes", "No", "Can't comment"))
myColors<-(brewer.pal(9, "Blues"))
myColors<-myColors[c(9, 7,5)]
g1<-ggplot(data=change.part.is, aes(x=Var1, y=Freq, fill=Var1))+ 
  ...
  annotate('text', 3.25,606, label="(a)", size=7)+scale_y_continuous(expand = c(0, 0), limits = c(0,120))

### reason
part2.reason<-part2.reason[part2.reason$Country_work=="Israel" | (!(part2.reason$Country_work=="Israel") & part2.reason$Country_work=="" & part2.reason$Country=="Israel"),]
...
sum.israel<-data.frame(table(reason.pt.long$change.reason, reason.pt.long$yes))

sum.israel$Var1<-revalue(sum.israel$Var1, c("reason_partnership_change_career" = "Career", "reason_partnership_change_funding" = "Funding", "reason_partnership_change_interest" = "Interest", "reason_partnership_change_other" = "Other", "reason_partnership_change_socially"  = "Social"))
sum.israel$Var1<-reorder(sum.israel$Var1, -sum.israel$Freq)

myColors7<-(brewer.pal(9, "Blues"))
myColors7<-myColors7[c(9, 8,7,6,5,3)]

g2<-ggplot(sum.israel, aes(x=Var1, y=Freq, fill=Var1)) + 
...
```{r, echo=FALSE, fig.cap="Figure 4.5 (previously 4.2) External Research Partnerships. ( 4.6 + 4.7 + 4.8 )", warning=FALSE}
###4.6
myColors6<-(brewer.pal(9, "Blues"))
myColors6<-rev(myColors6[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors6)
...
p1 <- ggplot(xs$Fundamental, aes(number, Freq, fill=time), width=0.7) + 
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,50), breaks = c(0,10,20,30,40,50))
...
#pdf(file="figures/4.10(appendix)_13Apr.pdf", height=7, width =11)
...
#pdf(file="figures/4.4_12Apr.pdf", height=7, width =11)
...
```{r, echo=TRUE, fig.cap="Fig (4.12+4.13) Importance of practical applications and external partnerships.", warning=FALSE}
###4.12
myColors12<-(brewer.pal(9, "Blues"))
myColors12<-rev(myColors12[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors12)
...
#pdf(file="figures/4.14(appendix)_14Apr.pdf", height=7, width =11)
...
**4.4 Perspectives on the State of Fundamental Research in Israel**  
...
ggplot(data=perceive, aes(x=Var1, y=Freq, fill=Var1)) + 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,90)) + colScale
```
...
ggplot(data=per.change, aes(x=what.type, higher.priority, fill=what.type))+
        ...
        theme(aspect.ratio=3/4, 
              axis.text.x = element_text(angle=45, vjust=0.4, size=9),
              panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(), axis.title.y=element_text(size=11),  axis.text.y=element_text(size=9)) +
        labs(x="", y="Number of responses", fill="") +
...
```{r, echo=TRUE, fig.cap="Fig (4.15 + 4.16) Anticipated future of research funding."  - 4.15 + 4.16}
...
p13<-ggplot(data=perceive, aes(x=Var1, y=Freq, fill=Var1)) + 
      ...
      theme(aspect.ratio=3/4, axis.text.x = element_text(angle=0, vjust=0.4, size=9),
            panel.grid.major = element_blank(), 
            panel.grid.minor = element_blank(), axis.title.y=element_text(size=11),  axis.text.y=element_text(size=9)) +
  scale_y_continuous(expand = c(0, 0), limits = c(0,90)) + colScale+annotate('text', 0.6,525, label="(a)", size=5)+ theme(plot.margin=unit(c(1,1,1,1), "cm"))
p13
...
per.change$what.type<-factor(per.change$what.type, levels(per.change$what.type)[c(2,4,1,3)])
##colors
myColors16<-(brewer.pal(9, "Paired"))
myColors16<-(myColors16[c(2,4,6,1)])
colScale <- scale_fill_manual(name = "Var1",values = myColors16)
...
p14<-ggplot(data=per.change, aes(x=what.type, higher.priority, fill=what.type))+
  ...
  geom_text(aes(label=higher.priority, vjust=-0.5), size=3) +scale_y_continuous(expand = c(0, 0), limits = c(0,107))+ colScale+annotate('text', 0.6,1200, label="(b)", size=5)+ theme(plot.margin=unit(c(1,1,1,-1), "cm"))
p14

#pdf(file="figures/4.5_13Apr.pdf", width = 11, height = 7)

grid.arrange(p13, p14, nrow=1,widths=c(0.535,0.465))

#dev.off()
```


```{r, echo=FALSE, fig.cap="Fig 4.17 Anticipated change in research funding in next five years in Israel. Researchers were asked whether the availability of research funding would change in the next five years, for each research category. Responses were/were not significantly different between genders. "}
...
#pdf(file="figures/4.17_12Apr.pdf", height=7, width =11)
...
```{r, echo=FALSE, fig.cap="Fig 4.18 Effect of change in research funding on research careers of next generation in Israel. Researchers were asked if they though that changes in funding availability would influence the likelihood of the next generation pursuing careers in research. Responses were/were not significantly different between genders."}
...
ggplot(data=impact, aes(x=Var1, Freq, fill=Var1))+ 
 ...
 labs(x="", y="Number of responses")+
  ...
  geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,60))+colScale
```
