---
title: "Developing countries online survey on fundamental research"
author: ''
date: "4 Jan 2018"
output:
  pdf_document: null
...
**Introduction**  
We developed and ran a quantitative online global to query researchers about their perceptions of, and experiences with, funding for fundamental research. An important aim of the global was to provide an understanding researcher's personal experiences and outlook on the research funding landscape. We had an excellent response to the survey, with over xxxx  researchers completing it, suggesting that fundamental research funding is a high priority topic for researchers. Herein, we detail the survey questions and results. 
...
n.gbl.dev<-survey[survey$nation%in% developing,]


#Count how many responses from researchers w/out CA

#Gender:
n.gbl.res.men <- n.gbl.dev[n.gbl.dev$gender=="Male",]               #Count how many responses from  male researchers
percent.men<-((dim(n.gbl.res.men)[1])/(dim(n.gbl.dev)[1]))*100      #Calculate the % of responses from  male researchers
n.gbl.res.women <- n.gbl.dev[n.gbl.dev$gender=="Female",]           #Count how many responses from  female researchers
percent.women<-((dim(n.gbl.res.women)[1])/(dim(n.gbl.dev)[1]))*100  #Calculate the % of responses from  female researchers

#Career Stage Tabulations:
#table(n.gbl.dev$what_participant_group)
n.seniorac <- n.gbl.dev[n.gbl.dev$what_participant_group=="Senior academic researcher with > 10 years of experience applying for research grants",]
percent.seniorac<-((dim(n.seniorac)[1])/(dim(n.gbl.dev)[1]))*100 #Calculate the % of responses from  senior academic researchers
n.earlyac <- n.gbl.dev[n.gbl.dev$what_participant_group=="Early career academic researcher with <10 years experience applying for research grants since completion of PhD",]
percent.earlyac<-((dim(n.earlyac)[1])/(dim(n.gbl.dev)[1]))*100 #Calculate the % of responses from  early academic researchers
n.pdf <- n.gbl.dev[n.gbl.dev$what_participant_group=="Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future",]
percent.pdf<-((dim(n.pdf)[1])/(dim(n.gbl.dev)[1]))*100 #Calculate the % of responses from  post-docs
n.nonACearly <- n.gbl.dev[n.gbl.dev$what_participant_group=="Non-academic researcher conducting or managing research in industry or government with <10 years of experience",]
percent.nonACearly<-((dim(n.nonACearly)[1])/(dim(n.gbl.dev)[1]))*100 #Calculate the % of responses from early career non-academics
n.nonACsenior <- n.gbl.dev[n.gbl.dev$what_participant_group=="Non-academic researcher conducting or managing research in industry or government with > 10 years of experience",]
percent.nonACsenior<-((dim(n.nonACsenior)[1])/(dim(n.gbl.dev)[1]))*100 #Calculate the % of responses from early career non-academics
n.unk <- n.gbl.dev[n.gbl.dev$what_participant_group=="",]
percent.unk<-((dim(n.unk)[1])/(dim(n.gbl.dev)[1]))*100 #Calculate the % of responses from unknown career stage

#Discipline Tabulations:
n.natural <- n.gbl.dev[n.gbl.dev$field_research=="Natural Science",]
percent.natural <- ((dim(n.natural)[1])/(dim(n.gbl.dev)[1]))*100 #Calculate the % of responses from  natural scientists
n.physics <- n.gbl.dev[n.gbl.dev$field_research=="Physical Science (eg. math, physics, chemistry, computer science)",]
percent.physics <- ((dim(n.physics)[1])/(dim(n.gbl.dev)[1]))*100 #Calculate the % of responses from  physical scientists
n.med <- n.gbl.dev[n.gbl.dev$field_research=="Medicine and Life Science",]
percent.med <- ((dim(n.med)[1])/(dim(n.gbl.dev)[1]))*100 #Calculate the % of responses from  med/life scientists
n.eng <- n.gbl.dev[n.gbl.dev$field_research=="Engineering",]
percent.eng <- ((dim(n.eng)[1])/(dim(n.gbl.dev)[1]))*100 #Calculate the % of responses from  engineering scientists
n.int <- n.gbl.dev[n.gbl.dev$field_research=="Interdisciplinary Science",]
percent.int <- ((dim(n.int)[1])/(dim(n.gbl.dev)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists
n.ssh <- n.gbl.dev[n.gbl.dev$field_research=="Social Science / Humanities",]
percent.ssh <- ((dim(n.ssh)[1])/(dim(n.gbl.dev)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists
...
locations<-aggregate(gender ~ nation, n.gbl.dev, length)
locations<-locations[!locations$nation=="",]
locations<-droplevels(locations)

#pdf(file="figures/developing/responses_country.pdf", width = 11, height= 8)
...
ggplot(locations, aes(x = reorder(nation, -gender), gender)) + geom_bar(stat='identity',position = position_dodge(width=0.5),fill='#1f78b4') + theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5)) + labs(y="Number of responses", x="", title="Responses by Country")  + 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+scale_y_continuous(expand = c(0,0), limits=c(0, 130)) 
...
#pdf(file="figures/developing/responses_country_gapplot.pdf", height=8, width=11)

par(mar=c(11,4,4,2))
gap.barplot(sort(locations$gender,decreasing=TRUE), gap=c(51,109),horiz=F, xaxlab=barlabs, xlab="     ", 
            ylab="Number of responses", ytics=c(0,10,20,30,40,50,110,118), main = "Responses by country",las=2, col=c(blue2red(29)))
#dev.off()


## >10 responses

locations.more<-aggregate(gender ~ nation, n.gbl.dev, length)
...
#pdf(file="figures/developing/responses_country_10+_gapplot.pdf", height=8, width=11)

par(mar=c(7,4,4,2))
gap.barplot(sort(locations.more$gender,decreasing=TRUE), gap=c(51,109),horiz=F, xaxlab=barlabs, xlab="     ", 
            ylab="Number of responses", ytics=c(0,10,20,30,40,50,110,118), main = "Countries with greater than 10 responses",las=2, col=color1)
...
locations.less<-aggregate(gender ~ nation, n.gbl.dev, length)
...
color1<-c('#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4')
#pdf(file="figures/developing/responses_country_10+&10-_gapplot.pdf", height=8, width=15)
layout(matrix(c(1,2),1,2, byrow = TRUE), widths=c(0.7,2))
par(mar=c(11,4,4,2))
gap.barplot(sort(locations.more$gender,decreasing=TRUE), gap=c(51,109),horiz=F, xaxlab=barlabs, xlab="     ", 
            ylab="Number of responses", ytics=c(0,10,20,30,40,50,110,118), main = "Countries with greater\nthan 10 responses",las=2, col=color1)
par(mar=c(11,2,4,1))
barplot(sort(locations.less$gender,decreasing=TRUE), names.arg = locations.less$nation[order(locations.less$gender, decreasing=TRUE)] ,horiz=F, xlab="", 
...
```{r, echo=FALSE, message=FALSE, fig.cap="Figure 4.1 Number of survey respondents from developing nations by career stage", warning=FALSE, fig.width=14}
...
experience<-data.frame(table(n.gbl.dev$what_participant_group))
...
#pdf(file="figures/developing/4.1_CareerStage.pdf", width = 11, height= 7)
...
ggplot(experience, aes(x = reorder(Var1, -Freq), Freq, fill=Var1)) + 
  ...
  labs(y="Number of responses", x="", title="Responses by career stage (Developing nations)") +
...
field<-data.frame(table(n.gbl.dev$field_research))
...
#pdf(file="figures/developing/4.2_FieldofResearch.pdf", width = 11, height= 7)

ggplot(field, aes(x = reorder(Var1, -Freq), Freq, fill=Var1)) + 
    geom_bar(stat='identity',position = position_dodge(width=0.5)) + 
  labs(y="Number of responses",x="", title="Responses by field of research (Developing nations)")  +
...
#pdf(file="figures/devloping/4.1and4.2(old)_CareerStageAndFieldOfResearch.pdf", width = 11, height= 7)
...
research.dev <- research[research$nation%in% developing,]

## make sure that Use-inspired is the middle level in the legend
res.plot<-subset(research.dev, type%in%c("percent_fundemental_research_current",  "percent_Applied_Research_current" , "percent_Use_inspired_Research_current","percent_Applied_Research_past",  "percent_Fundamental_Research_past" , "percent_Use_inspired_Research_past"))
...
#pdf(file="figures/developing/4.3.1_ResearchProportion.pdf", width = 11, height= 7)
...
ggplot(res.plot, aes(percent, fill=ID)) +
    ...
    labs(x="Percent", y="Number of responses", title="Reseach type by proportion (Developing nations)")+
  ...
  scale_x_continuous(breaks=seq(0,100,10))+scale_y_continuous(expand=c(0,0), limits=c(0,65),breaks=c(0,10,20,30,40,50,60))
...
p1 <- ggplot(xs$Fundamental,aes(percent, fill=time)) +
  ...
  scale_x_continuous(breaks=seq(0,100,10))+scale_y_continuous(expand=c(0,0), limits=c(0,63),breaks=c(0,10,20,30,40,50,60))

p2 <- p1 %+% xs$`Use-inspired`
p3 <- p1 %+% xs$Applied
p1<-p1+scale_fill_manual(name = "ID",values =mycols[4:5], labels=c("2006-2010", "2011-2015"))+labs(x="", title="Type of research by proportion (Developing nations)")+ theme(plot.margin=unit(c(1,1,-0.45,1), "cm"), plot.title = element_text(hjust = 0.5))
...
#pdf(file="figures/developing/4.3.2_ResearchProportion.pdf", width = 11, height= 7)
...
#selecting for developing nations
research.change.dev <- research.change[research.change$nation%in% developing,]

#Change yes or no
yes.no<-subset(research.change.dev, select=c("Location","Country", "Country_work",  "gender", "changed_10yrs"))
...
p1<-ggplot(data=global.yes.no, aes(x=changed_10yrs, y=gender, fill=gender))+ geom_bar(stat='identity') + guides(fill=FALSE)+
  theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(), axis.title.x=element_blank(), plot.title = element_text(hjust = 0.5))+geom_text(aes(label=gender), vjust=-0.5)+labs(y="Number of responses", title="Change in research type\n(Developing nations)")+ scale_y_continuous(expand = c(0, 0), limits = c(0,140))+ annotate('text', 0.6,136, label="(a)", size=5)

####reason for change  
reason<-subset(research.change.dev, select=c("Location","Country","Country_work",  "gender", "Main_reason_change_interest_related",
...
p2<-ggplot(sum.global, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 86))+annotate('text', 5.35,84, label="(b)", size=5)+geom_text(aes(label=Freq), vjust=-0.5, size=3)
  
#pdf(file="figures/developing/4.4_ReasonForFieldChange.pdf", width = 11, height= 7)

#print(p2)

#dev.off()

pdf(file="figures/developing/4.4a&b_ReasonForFieldChange.pdf", width = 11, height= 7)
...
research.dev <- research[research$nation%in% developing,]
#head(research.dev)
#dim(research.dev)     #780   11

##data frame with whether or not they changed 
#selecting for developing nations
research.change.dev <- research.change[research.change$nation%in% developing,]
#head(research.change.dev)
#dim(research.change.dev)   #272  14
...
res.port.dev <- spread(research.dev, type, percent)
#head(res.port.dev)
#dim(res.port.dev)  #130   16   - this is just the ones that had answers for both current and past research proportions (there was only one person that answered 'yes' to if their research type had changed but did not provide 'past' proportions)

##add in the yes/no response to the proportions data frame using the survey id
#remove 'no' and 'can't comment' first
yes.dev<- research.change.dev[!research.change.dev$changed_10yrs == "No",]
yes.dev<- yes.dev[!yes.dev$changed_10yrs == "Can't comment (new researcher)",]
yes.dev<- yes.dev[!yes.dev$changed_10yrs == "",]
yes.dev<- droplevels(yes.dev)
#dim(yes.dev)  # 131  14
#head(yes.dev)

res.port.dev$changed<-yes.dev$changed_10yrs[match(res.port.dev$id, yes.dev$id)]
#head(res.port.dev)
#colnames(res.port.dev)
#str(res.port.dev)
#dim(res.port.dev)

##### ok have the data - now we have to see how people's research changed
# look at what their majority was for the two time points
#head(res.port.dev)
#add a column indicating which was the majority currently
res.port.dev$majority_current <- "fund.current"
#head(res.port.dev)
res.port.dev$majority_current <- ifelse((res.port.dev$percent_Applied_Research_current >= 50), "app.current", res.port.dev$majority_current)
res.port.dev$majority_current <- ifelse((res.port.dev$percent_Use_inspired_Research_current >= 50), "use.current", res.port.dev$majority_current)


#now past
res.port.dev$majority_past <- "fund.past"
#head(res.port.dev)
res.port.dev$majority_past <- ifelse((res.port.dev$percent_Applied_Research_past >= 50), "app.past", res.port.dev$majority_past)
res.port.dev$majority_past <- ifelse((res.port.dev$percent_Use_inspired_Research_past >= 50), "use.past", res.port.dev$majority_past)

#head(res.port.dev)

#now deal with the 50%  (consider app and use the same though so there are less categories)
res.port.dev$majority_current <- ifelse(((res.port.dev$percent_Applied_Research_current == 50 |  res.port.dev$percent_Use_inspired_Research_current == 50) & res.port.dev$percent_fundemental_research_current == 50), "fund.app.half.current", res.port.dev$majority_current)
res.port.dev$majority_current <- ifelse((res.port.dev$percent_Applied_Research_current == 50 & res.port.dev$percent_Use_inspired_Research_current == 50), "app.current", res.port.dev$majority_current)


res.port.dev$majority_past <- ifelse(((res.port.dev$percent_Fundamental_Research_past == 50 | res.port.dev$percent_Use_inspired_Research_past == 50) & res.port.dev$percent_Applied_Research_past == 50), "fund.app.half.past", res.port.dev$majority_past)
#res.port.dev$majority_past <- ifelse((res.port.dev$percent_Fundamental_Research_past == 50 & res.port.dev$percent_Use_inspired_Research_past == 50), "fund.use.half.past", res.port.dev$majority_past)
res.port.dev$majority_past <- ifelse((res.port.dev$percent_Use_inspired_Research_past == 50 & res.port.dev$percent_Applied_Research_past == 50), "app.past", res.port.dev$majority_past)

#head(res.port.dev)

# now add in a column with the applied + use added together and negative
res.port.dev$app.use.fund.current <- ifelse((res.port.dev$majority_current == "use.current" | res.port.dev$majority_current == "app.current"), (-(res.port.dev$percent_Use_inspired_Research_current  + res.port.dev$percent_Applied_Research_current)), res.port.dev$percent_fundemental_research_current)

res.port.dev$app.use.fund.past <- ifelse((res.port.dev$majority_past == "use.past" | res.port.dev$majority_past == "app.past"), (-(res.port.dev$percent_Use_inspired_Research_past  + res.port.dev$percent_Applied_Research_past)), res.port.dev$percent_Fundamental_Research_past)

#head(res.port.dev)


# add in column with current - past for each type
res.port.dev$curr_past.app <- (res.port.dev$percent_Applied_Research_current - res.port.dev$percent_Applied_Research_past)
res.port.dev$curr_past.use <- (res.port.dev$percent_Use_inspired_Research_current - res.port.dev$percent_Use_inspired_Research_past)
res.port.dev$curr_past.fund <- (res.port.dev$percent_fundemental_research_current - res.port.dev$percent_Fundamental_Research_past)

#head(res.port.dev)

# going to calculate average and sd for the current - past columns for each to plot
res.port.dev$app_mean <- mean(res.port.dev$curr_past.app)
res.port.dev$app_sd <- sd(res.port.dev$curr_past.app)
res.port.dev$use_mean <- mean(res.port.dev$curr_past.use)
res.port.dev$use_sd <- sd(res.port.dev$curr_past.use)
res.port.dev$fund_mean <- mean(res.port.dev$curr_past.fund)
res.port.dev$fund_sd <- sd(res.port.dev$curr_past.fund)


#head(res.port.dev)

#save as csv
write.csv(res.port.dev, file = "data/developing_research_direction_change.csv", row.names = FALSE)
...
change <- subset(res.port.dev, select = c("app_mean", "use_mean", "fund_mean", "nation", "id"))
#head(change)

colnames(change) <- c("app_mean" = "Applied", "use_mean" = "Use-inspired", "fund_mean" = "Fundamental", "nation"="nation", "id"="id")

change.long <- gather(change, type, aver, -nation, -id)
#head(change.long)
change.long <- change.long[-c(2:259, 262:390),]
#change.long

#add in sd
change.long$sd <- 1
change.long$sd <- ifelse(change.long$type == "Applied", res.port.dev$app_sd, change.long$sd)
change.long$sd <- ifelse(change.long$type == "Use-inspired", res.port.dev$use_sd, change.long$sd)
change.long$sd <- ifelse(change.long$type == "Fundamental", res.port.dev$fund_sd, change.long$sd)
#head(change.long)
...
#pdf(file="figures/developing/4.3.4_ResearchProportion_DirectionChange.pdf", width = 11, height= 7)
#pdf(file="figures/developing/4.3.4_ResearchProportion_DirectionChange_coordflip.pdf", width = 11, height= 7)
...
ggplot(change.long, aes(type, aver, fill = type))+ geom_bar(stat = "identity") +
  ...
  labs(x = "", y = "Average change", fill = "", title ="Reseach type change (Developing nations)") +
  theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),legend.text=element_text(size = 9),
          strip.text.y = element_text(size=12, face="bold", color="black"),
          strip.background = element_rect(colour="white", fill="white"),
...
#<---------------------------->
#########now aggregate
pastmajor <- aggregate(gender ~ majority_past, res.port.dev, length)
#pastmajor
# change the column names so they can be joined
colnames(pastmajor) <- c("major.research.past", "gender")
#pastmajor


currentmajor <- aggregate(gender ~ majority_current, res.port.dev, length)
#currentmajor
colnames(currentmajor) <- c("major.research.current", "gender")
#currentmajor
# join them   - not doing this anymore   Anina wants two figures
...
# majority$major.research <- factor(majority$major.research, levels(majority$major.research)[c(6,5,8,7,11,10,9,2,1,4,3)])

# graph the aggregates

myColors.major<-(brewer.pal(9, "Blues"))
myColors.major<-myColors.major[c(4,4,4,4)]

pastmajor$major.research.past<-revalue(pastmajor$major.research.past, c("app.past"="Applied", "fund.past"="Fundamental", "use.past"="Use-inspired", "fund.app.half.past"="Half Fundamental\n/Half Applied"))
pastmajor$major.research.past<-reorder(pastmajor$major.research.past, rev(pastmajor$gender))

#pdf(file="figures/developing/4.3.3_ResearchProportion_Past.pdf", width = 11, height= 7)

ggplot(pastmajor, aes(major.research.past, gender, fill = major.research.past)) + geom_bar(stat='identity')+
   geom_text(aes(label=gender), position=position_dodge(width=1), vjust=-0.5, size=4) + labs(x = "Majority type of research", y = "Number of responses", title = "Respondent's major proportion of past research type\n(Developing countries)") + guides(fill =FALSE) +
  theme(plot.title = element_text(size = 11, hjust = 0.5), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        legend.key=element_blank(),legend.text=element_text(size = 9),
        axis.text.x = element_text(angle=0, size=9), axis.text.y=element_text(size = 9))+scale_fill_manual(name='major.research.past', values=myColors.major) +scale_y_continuous(expand = c(0, 0), limits = c(0,105))+ annotate('text', 4.4,100, label= sum(pastmajor$gender), size=5) +
  annotate('text', 4.23, 100, label= "n = ", size=5)

#dev.off()

### current

currentmajor$major.research.current<-revalue(currentmajor$major.research.current, c("app.current"="Applied", "fund.current"="Fundamental", "use.current"="Use-inspired", "fund.app.half.current"="Half Fundamental\n/Half Applied"))
currentmajor$major.research.current<-reorder(currentmajor$major.research.current, rev(currentmajor$gender))


#pdf(file="figures/developing/4.3.3_ResearchProportion_Current.pdf", width = 11, height= 7)

ggplot(currentmajor, aes(major.research.current, gender, fill = major.research.current)) + geom_bar(stat='identity') +
   geom_text(aes(label=gender), position=position_dodge(width=1), vjust=-0.5, size=4) + labs(x = "Majority type of research", y = "Number of responses", title = "Respondent's major proportion of current research type\n(Developing countries)") + guides(fill =FALSE) +
  theme(plot.title = element_text(size = 11, hjust = 0.5), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        legend.key=element_blank(),legend.text=element_text(size = 9),
        axis.text.x = element_text(angle=0, size=9), axis.text.y=element_text(size = 9))+scale_fill_manual(name='major.research.current', values=myColors.major)+scale_y_continuous(expand = c(0, 0), limits = c(0,85))+ annotate('text', 4.4,80, label= sum(currentmajor$gender), size=5) +
  annotate('text', 4.23, 80, label= "n = ", size=5)

#dev.off()


# do a plot with centraly y axis
#need to make it long version 

res.port.dev.long <- gather(res.port.dev, curr.past, overall.per, -Location, -nation, -id, -Country, -Country_work, -gender, -what_participant_group, -field_research, -total_research, -percent_Applied_Research_current, -percent_Applied_Research_past, -percent_Fundamental_Research_past, -percent_fundemental_research_current, -percent_Use_inspired_Research_current, -percent_Use_inspired_Research_past, -changed, -majority_current, -majority_past)

head(res.port.dev.long)

# add colours to res.plot
mycols<-c("blue", "red")
mycols<-as.data.frame(mycols)
mycols$curr.past<-c("app.use.fund.current", "app.use.fund.past")
res.port.dev.long$col<-mycols$mycols[match(res.port.dev.long$curr.past, mycols$curr.past)]
#res.port.dev.long$col<-factor(res.port.dev.long$col, levels(res.port.dev.long$col)[c(3,1,4,2,6,5)])
mycols<-as.character(unique(res.port.dev.long$col))
colScale <- scale_fill_manual(name = "curr.past",values =mycols)

#!!!!!!!!!!!!!!!!  dont think it is working right !!!!!!!!!!!!!!!!

ggplot() + geom_histogram(data = 
                          res.port.dev.long[res.port.dev.long$overall.per < 0, ],
                          aes(x = overall.per, y = -..count.., fill = curr.past), binwidth=10, bins=seq(50, 100, 10), position='dodge') +
           geom_histogram(data = 
                          res.port.dev.long[res.port.dev.long$overall.per > 0, ],
                          aes(x = overall.per, y = ..count.., fill = curr.past),  binwidth=10, bins=seq(50, 100, 10), position='dodge') +colScale+
...
res.port.dev.long <- gather(res.port.dev, time, what, - Location, -nation, -id, -Country, -Country_work, -gender, -what_participant_group, -field_research, -total_research, -percent_Applied_Research_current, -percent_Applied_Research_past, -percent_Fundamental_Research_past, -percent_fundemental_research_current, -percent_Use_inspired_Research_current, -percent_Use_inspired_Research_past, -changed)
...
colnames(res.port.dev)
res.port.dev$"app_mean" <- NULL
res.port.dev$"app_sd"  <- NULL
res.port.dev$"use_mean"  <- NULL
res.port.dev$"use_sd"  <- NULL
res.port.dev$"fund_mean"  <- NULL
res.port.dev$"fund_sd"  <- NULL
res.port.dev$"changed"  <- NULL
res.port.dev$"majority_current"  <- NULL
res.port.dev$"majority_past"  <- NULL
res.port.dev$"Country_work"  <- NULL
res.port.dev$"Country"  <- NULL
res.port.dev$"Location"  <- NULL
res.port.dev$"app.use.fund.past"  <- NULL
res.port.dev$"app.use.fund.past"  <- NULL
...
str(res.port.dev)
fund.dec <- res.port.dev[(res.port.dev$curr_past.fund <= -25),]
...
head(n.gbl.dev)
colnames(n.gbl.dev)
fund.dec.view.change <- n.gbl.dev[,c(4,12:18, 74, 77:79)]
...
head(fund.dec.view.change)
dim(fund.dec)
dim(fund.dec.view.change)
# they both have 30 rows
...
#pdf(file="figures/developing/4.5.2_ViewFieldChange_decreasedFund.pdf", width = 11, height= 7)
...
ggplot(fund.dec.view, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
  ...
  theme(panel.grid.major = element_blank(),legend.text=element_text(size = 9), panel.grid.minor = element_blank())+ labs(x="", y="Number of responses", title="Opinion of change in research type when their\nFundamental research decreased by 25%+ (Developing nations)")+geom_text(aes(label=gender), vjust=-0.5, size=4)+scale_y_continuous(expand = c(0, 0), limits = c(0,17.5))+scale_fill_manual(name='view_change_of_type', values=myColors.view)+annotate('text', 5.4,16.5, label= sum(fund.dec.view$gender), size=5) +annotate('text', 5.15,16.5, label= "n = ", size=5)
...
#pdf(file="figures/developing/4.4.2_ReasonForFieldChange_FundDecrease.pdf", width = 11, height= 7)
...
ggplot(sum.fund.dec, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 22))+annotate('text', 5.35,84, label="(b)", size=5)+geom_text(aes(label=Freq), vjust=-0.5, size=3)
#+
...
head(res.port.dev)
fund.inc <- res.port.dev[(res.port.dev$curr_past.fund >= 25),]
...
head(n.gbl.dev)
colnames(n.gbl.dev)
fund.inc.view.change <- n.gbl.dev[,c(4,12:18, 74, 77:79)]
...
myColors.view<-(brewer.pal(9, "Blues"))
myColors.view<-myColors.view[c(4,4)]

#pdf(file="figures/developing/4.5.3_ViewFieldChange_increaseFund.pdf", width = 11, height= 7)
...
ggplot(fund.inc.view, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
  ...
  theme(panel.grid.major = element_blank(),legend.text=element_text(size = 9), panel.grid.minor = element_blank())+ labs(x="", y="Number of responses", title="Opinion of change in research type when their\nFundamental research increased by 25%+ (Developing nations)")+geom_text(aes(label=gender), vjust=-0.5, size=4)+scale_y_continuous(expand = c(0, 0), limits = c(0,6))+scale_fill_manual(name='view_change_of_type', values=myColors.view)+annotate('text', 2.5,5.5, label= sum(fund.inc.view$gender), size=5) +annotate('text', 2.43,5.5, label= "n = ", size=5)
...
#pdf(file="figures/developing/4.4.3_ReasonForFieldChange_FundIncrease.pdf", width = 11, height= 7)
...
ggplot(sum.fund.inc, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 5.5))+annotate('text', 5.35,84, label="(b)", size=5)+geom_text(aes(label=Freq), vjust=-0.5, size=3)
#+
#  annotate('text', 5.45,21, label= sum(sum.fund.dec$Freq), size=5) +annotate('text', 5.25,21, label= "n = ", size=5)
...
head(res.port.dev)
app.inc<- res.port.dev[((res.port.dev$curr_past.app >= 25)|(res.port.dev$curr_past.use >= 25)),]
...
head(n.gbl.dev)
colnames(n.gbl.dev)
app.inc.view.change <- n.gbl.dev[,c(4,12:18, 74, 77:79)]
...
#pdf(file="figures/developing/4.5.4_ViewFieldChange_increasedApp.pdf", width = 11, height= 7)

ggplot(app.inc.view, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
  theme( axis.text.x = element_text(angle=0, vjust=0.5, size=9), axis.text.y = element_text(size = 9), axis.title.y = element_text(size = 11), axis.title.x = element_text(size = 11), plot.title = element_text(hjust = 0.5)) +guides(fill=FALSE)+
  theme(panel.grid.major = element_blank(),legend.text=element_text(size = 9), panel.grid.minor = element_blank())+ labs(x="", y="Number of responses", title="Opinion of change in research type when\nUse-inspired or Applied research increased by 25%+ (Developing nations)")+geom_text(aes(label=gender), vjust=-0.5, size=4)+scale_y_continuous(expand = c(0, 0), limits = c(0,21))+scale_fill_manual(name='view_change_of_type', values=myColors.view)+annotate('text', 5.4,20, label= sum(app.inc.view$gender), size=5) +annotate('text', 5.25, 20, label= "n = ", size=5)
...
#pdf(file="figures/developing/4.4.4_ReasonForFieldChange_IncreasedApp.pdf", width = 11, height= 7)
...
ggplot(sum.app.inc, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 24))+annotate('text', 5.35,84, label="(b)", size=5)+geom_text(aes(label=Freq), vjust=-0.5, size=3)
#+
#  annotate('text', 5.45,21, label= sum(sum.fund.dec$Freq), size=5) +annotate('text', 5.25,21, label= "n = ", size=5)
...
view.propor<-n.gbl.dev[!n.gbl.dev$view_change_of_type=="",]
...
pdf(file="figures/developing/4.5_ViewFieldChange.pdf", width = 11, height= 7)

ggplot(change.view.glb, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
  theme(aspect.ratio=3/4, axis.text.x = element_text(angle=0, vjust=0.5, size=9), axis.text.y = element_text(size = 9), axis.title.y = element_text(size = 11), axis.title.x = element_text(size = 11), plot.title = element_text(hjust = 0.5)) +guides(fill=FALSE)+
  theme(panel.grid.major = element_blank(),legend.text=element_text(size = 9), panel.grid.minor = element_blank())+ labs(x="", y="Number of responses", title="Opinion of change in research type (Developing nations)")+geom_text(aes(label=gender), vjust=-0.5, size=3)+scale_y_continuous(expand = c(0, 0), limits = c(0,63))+scale_fill_manual(name='view_change_of_type', values=myColors5)#+annotate('text', 5.4,133, label="(c)", size=5)
...
#pdf(file="figures/developing/4.1_13apr_with.wrong.a.pdf", width = 8, height = 11)
...
#pdf(file="figures/developing/4.1b.c._ResearchFoci_13apr_blank.a.pdf", width = 8, height = 11)
multiplot(g1, g2, g3, layout = matrix(c(1,1,2,3),nrow=2, byrow=TRUE))

##trying to put the right (old version) into the layout ###havent gotten it to work yet
#pdf(file="figures/developing/4.1_13apr_right.a.pdf", width = 8, height = 11)
...
b.part<-data.frame(table(n.gbl.dev$partnership_outside_before))
b.part<-b.part[!b.part$Var1=="",]
b.part<-droplevels(b.part)
b.part$time<-"Past"
a.part<-data.frame(table(n.gbl.dev$partnership_outside))
...
#pdf(file="figures/developing/4.6_LevelsOfPartnership.pdf", width = 11, height= 7)
...
ggplot(part, aes(x=Var1, y=Freq, fill = time),  ylim(0,375)) + 
  ...
  labs(x="", y="Number of responses", fill="", title="Level of partnership outside of academia (Developing nations)")+
  theme(aspect.ratio=3/4, panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        legend.key=element_blank(),legend.text=element_text(size = 9),
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,140))
...
change.part.glb<-data.frame(table(n.gbl.dev$partnership_change_10yrs))
...
g1<-ggplot(data=change.part.glb, aes(x=Var1, y=Freq, fill=Var1))+ 
    ...
    guides(fill=FALSE) + 
    labs(x="", y="Number of responses",title="Change in level of external partnerships (Developing nations)")+
...
#selecting for developing nations
part2.reason.dev <- part2.reason[part2.reason$nation%in% developing,]

reason.pt.long<-gather(part2.reason.dev, change.reason, yes, -Country,-Country_work, -nation, -gender, -Location, -what_participant_group, -field_research)
...
g2<-ggplot(sum.global, aes(x=Var1, y=Freq, fill=Var1)) + 
    ...
    theme(axis.text.x = element_text(angle=0, vjust=0.5, size = 9), axis.text.y=element_text(size = 9), axis.title.y=element_text(size = 11),
          ...
          panel.grid.minor = element_blank(), plot.title = element_text(hjust = 0.5)) + 
  geom_text(aes(label=Freq), vjust=-0.5, size=3)+
  labs(x="", y="Number of responses", title="Reason for change in level of external partnerships (Developing nations)")+scale_fill_manual(name = "Var1",values = myColors7)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,75))

#annotate('text', 5,420, label="(b)", size=7)+ 

#pdf(file="figures/developing/4.7a&b_PartnershipChange&Reason.pdf", width = 11, height= 7)
...
view<-data.frame(table(n.gbl.dev$view_change_partnership))
view<-view[!view$Var1=="",]
...
#pdf(file="figures/developing/4.8_ViewChangePartnership.pdf", width = 11, height= 7)
...
ggplot(view, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+
  ...
  labs(x="", y="Number of responses", title="Opinion of change in level of external partnerships (Developing nations)") + colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,50), breaks = seq(0,50,5))
...
#pdf(file="figures/developing/4.2_13Apr.pdf")
...
#selecting for developing nations
part3.grants.long <- part3.grants.long[part3.grants.long$nation%in% developing,]
...
grants.gbl<-data.frame(with(part3.grants.long, table(type.grant,time,number)))
...
#pdf(file="figures/developing/4.9_NumberGrantApps.pdf", height=7, width =11)
...
p1 <- ggplot(xs$Fundamental, aes(number, Freq, fill=time), width=0.7) + 
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,200), breaks = c(0,50,100,150,200))
...
grid.arrange(p1,p2,p3, nrow=1, top=textGrob("Number of research grant applications (Developing nations)",gp=gpar(fontsize=17)))
...
#selecting for developing nations
part3.success.long <- part3.success.long[part3.success.long$nation%in% developing,]
...
#pdf(file="figures/developing/4.10_GrantAppSuccess.pdf", height=7, width =11)
...
#selecting for developing nations
part3.change.dev <- part3.change[part3.change$nation%in% developing,]

##switch to long format
change.long.gbl<-gather(part3.change.dev, type, level, -Location, -nation, -gender, -Country, -Country_work, -what_participant_group, -field_research, -id)
...
#setting it up to have the n values added to figure

n.use <- with(success.change, sum(Freq[type == "Use-inspired"]))
n.fund <- with(success.change, sum(Freq[type == "Fundamental"]))
n.app <- with(success.change, sum(Freq[type == "Applied"]))

success.change$n <- ifelse(success.change$type == "Fundamental", n.fund, 1)
success.change$n <- ifelse(success.change$type == "Applied", n.app, success.change$n)
success.change$n <- ifelse(success.change$type == "Use-inspired", n.use, success.change$n)


#pdf(file="figures/developing/4.11_ChangeSuccessRate.pdf", height=7, width =11)
...
ggplot(data=success.change, aes(x=level, Freq, fill=type))+ facet_wrap(~type, nrow=3)+
  ...
  labs(x="", y="Number of responses", fill="", title="Change in grant success rate (Developing nations)")+ colScale+
  scale_y_continuous(expand = c(0, 0), limits = c(0,140))+geom_text(data=success.change, aes(x=6.4, y=130, label=n), colour="black", inherit.aes=FALSE, parse=FALSE, size = 4.5)+annotate('text', 6.09,130, label= "n = ", size=5)
...
#selecting for developing nations
part3.prac.long.dev <- part3.prac.long[part3.prac.long$nation%in% developing,]


part3.prac.long.dev$year<-as.character(part3.prac.long.dev$year)

part3.prac.long.dev$year[part3.prac.long.dev$year=="practical_applications_important_11_15"]<-"2011-2015"
part3.prac.long.dev$year[part3.prac.long.dev$year=="practical_applications_important_6_10"]<-"2006-2010"


prac.gbl<-data.frame(table(part3.prac.long.dev$year, part3.prac.long.dev$level))
...
#pdf(file="figures/developing/4.12_ImportancePracticalApplication.pdf", height=7, width =11)
...
ggplot(prac.gbl, aes(Var2, Freq, fill=Var1)) + 
  ...
  labs(x="Importance of suggesting practical research applications", y="Number of responses", title="Importance of practical application for grant success (Developing nations)")+ colScale+
...
#selecting for developing nations
part3.part.long <- part3.part.long[part3.part.long$nation%in% developing,]
...
#pdf(file="figures/developing/4.13_ImportancePartnersApplication.pdf", height=7, width =11)
...
ggplot(part.gbl, aes(Var2, Freq, fill=Var1))+ 
  geom_bar(stat="identity", position = "dodge")+geom_text(aes(label=Freq), vjust=-0.7, hjust=0.5, size=3, position = position_dodge(width = 0.8))+
    guides(fill=guide_legend(title=NULL, reverse=FALSE)) +
    labs(x="Importance of external partnerships", y="Number of responses", title="Importance of external partnership to the success of grants (Developing nations)")+
...
#pdf(file="figures/developing/4.3_13Apr.pdf", width = 11, height= 7)
...
#selecting for developing nations
p3_master.long <- p3_master.long[p3_master.long$nation%in% developing,]

##revalue the year values
p3_master.long$year<- revalue(p3_master.long$year, c("11_15"="2011-2015", "6_10"="2006-2010"))
#change order of levels
p3_master.long$year<-factor(p3_master.long$year, levels(p3_master.long$year)[c(2,1)])

##For-Profit
fp<-p3_master.long[p3_master.long$type=="For-Profit",]
...
##Government
gov<-p3_master.long[p3_master.long$type=="Government",]
...
int<-p3_master.long[p3_master.long$type=="Internal",]
...
ngov<-p3_master.long[p3_master.long$type=="Non-governmental",]
...
other<-p3_master.long[p3_master.long$type=="Other",]
...
#pdf(file="figures/developing/4.14_DistributionFunding.pdf", height=7, width =11)

grid.arrange(arrangeGrob(p1+ theme(legend.position="none"), p2,
                         p3+ theme(legend.position="none"), p4+ theme(legend.position="none"),
                         p5,
                         layout_matrix=rbind(c(1,1,1,2,2,2),c(3,3,4,4,5,5))), top=textGrob("Distribution of research funding (Developing nations)",gp=gpar(fontsize=17)))
...
#selecting for developing nations
part4.dev <- part4[part4$nation%in% developing,]

## order bars from very important (left) to can't comment (right)
important<-subset(part4.dev, select=c("Location","Country",'Country_work', "gender","opinion_fundamental_important"))
...
#pdf(file="figures/developing/4.15_PerceivedImportanceFundamentalGovernment.pdf", height=7, width =11)
...
ggplot(data=perceive, aes(x=Var1, y=Freq, fill=Var1)) + 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,112)) + colScale + annotate('text', 5.4,107, label= sum(perceive$Freq), size=5) +annotate('text', 5.15,107, label= "n = ", size=5)
...
priority<-subset(part4.dev, select=c("Location","Country",'nation','Country_work', "gender", "high_priority_fundamental", "high_priority_use_inspired", "high_priority_applied", "high_priority_no_change"))
...
#pdf(file="figures/developing/4.16_PerceivedChangeResearchPriorityGovernment.pdf", height=7, width =11)
...
ggplot(data=per.change, aes(x=what.type, higher.priority, fill=what.type))+
        ...
        theme(aspect.ratio=3/4, 
              ...
              panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(), axis.title.y=element_text(size=11),  axis.text.y=element_text(size=9), plot.title = element_text(hjust = 0.5)) +
        labs(x="", y="Number of responses", fill="", title="Perceived change in research priority by their government\n(Developing nations)") +
  scale_x_discrete(labels=c( "Fundamental", "Use-inspired","Applied", "No change"))   +
  geom_text(aes(label=higher.priority, vjust=-0.5), size=3) +scale_y_continuous(expand = c(0, 0), limits = c(0,250))+ colScale+ annotate('text', 4.4,240, label= sum(per.change$higher.priority), size=5) +annotate('text', 4.2,240, label= "n = ", size=5)
...
availability.change<-subset(part4.dev, select=c("Location", "Country","nation",'Country_work', "gender", "available_funding_fundamental",  
...
n.use <- with(availability.gbl, sum(Freq[what.type == "Use-inspired"]))
n.fund <- with(availability.gbl, sum(Freq[what.type == "Fundamental"]))
n.app <- with(availability.gbl, sum(Freq[what.type == "Applied"]))

availability.gbl$n <- ifelse(availability.gbl$what.type == "Fundamental", n.fund, 1)
availability.gbl$n <- ifelse(availability.gbl$what.type == "Applied", n.app, availability.gbl$n)
availability.gbl$n <- ifelse(availability.gbl$what.type == "Use-inspired", n.use, availability.gbl$n)

#pdf(file="figures/developing/4.17_AnticipatedChangeFunding.pdf", height=7, width =11)
...
ggplot(data=availability.gbl, aes(x=level, Freq, fill=what.type))+
    ...
    guides(fill=FALSE)+
    labs(x="", y="Number of responses", fill="", title="Anticipated change in funding (Developing nations)")+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,105), breaks = seq(0,120,20))+annotate('text', 6,95, label= "n = ", size=5)+geom_text(data=availability.gbl, aes(x=6.35, y=95, label=n), colour="black", inherit.aes=FALSE, parse=FALSE, size = 4.5)
...
## order bars into increase considerably (left) to decrease considerably (right), then no comment at the far right
next.generation<-subset(part4.dev, select=c("Location", "Country","Country_work", "gender","next_generation"))
# remove non-response
...
#pdf(file="figures/developing/4.18_EffectNextGeneration.pdf", height=7, width =11)
...
ggplot(data=impact, aes(x=Var1, Freq, fill=Var1))+ 
    geom_bar(stat='identity', position="dodge")  +
        scale_x_discrete(labels=c("Increase\n considerably", "Increase\n slightly", 
                                    "Stay the same", "Decrease\n slightly", "Decrease\n considerably", "Can't\n comment"))+
    guides(fill=FALSE) +
 labs(x="", y="Number of responses", title="Effect of funding changes on the next generation of researchers (Developing nations)")+
  ...
  geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,63))+colScale
