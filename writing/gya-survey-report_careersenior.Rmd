...
##selecting for senior career stage respondents
survey$what_participant_group<-revalue(survey$what_participant_group, c("Early career academic researcher with <10 years experience applying for research grants since completion of PhD" = "Early career academic researcher", "Non-academic researcher conducting or managing research in industry or government with >10 years of experience" = "Non-academic researcher >10 yrs experience", "Non-academic researcher conducting or managing research in industry or government with <10 years of experience" = "Non-academic researcher <10 yrs experience", "Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future" = "Postdoc or RA", "Senior academic researcher with >10 years of experience applying for research grants" = "Senior academic researcher"))

gbl.senior<-survey[(survey$what_participant_group%in%c("Senior academic researcher" ,"Non-academic researcher >10 yrs experience")),]

#check
unique(gbl.senior$what_participant_group)
gbl.senior <- droplevels(gbl.senior)


#Gender:
n.gbl.senior.res.men <- gbl.senior[gbl.senior$gender=="Male",]               #Count how many responses from  male researchers
percent.men<-((dim(n.gbl.senior.res.men)[1])/(dim(gbl.senior)[1]))*100      #Calculate the % of responses from  male researchers
n.gbl.senior.res.women <- gbl.senior[gbl.senior$gender=="Female",]           #Count how many responses from  female researchers
percent.women<-((dim(n.gbl.senior.res.women)[1])/(dim(gbl.senior)[1]))*100  #Calculate the % of responses from  female researchers
# 
# #Career Stage Tabulations:
# #table(gbl.senior$what_participant_group)
n.seniorac <- gbl.senior[gbl.senior$what_participant_group=="Senior academic researcher",]
percent.seniorac<-((dim(n.seniorac)[1])/(dim(gbl.senior)[1]))*100 #Calculate the % of responses from  senior academic researchers

n.nonACsenior <- gbl.senior[gbl.senior$what_participant_group=="Non-academic researcher >10 yrs experience",]
percent.nonACsenior<-((dim(n.nonACsenior)[1])/(dim(gbl.senior)[1]))*100 #Calculate the % of responses from early career non-academics

#Discipline Tabulations:
n.natural <- gbl.senior[gbl.senior$field_research=="Natural Science",]
percent.natural <- ((dim(n.natural)[1])/(dim(gbl.senior)[1]))*100 #Calculate the % of responses from  natural scientists
n.physics <- gbl.senior[gbl.senior$field_research=="Physical Science (eg. math, physics, chemistry, computer science)",]
percent.physics <- ((dim(n.physics)[1])/(dim(gbl.senior)[1]))*100 #Calculate the % of responses from  physical scientists
n.med <- gbl.senior[gbl.senior$field_research=="Medicine and Life Science",]
percent.med <- ((dim(n.med)[1])/(dim(gbl.senior)[1]))*100 #Calculate the % of responses from  med/life scientists
n.eng <- gbl.senior[gbl.senior$field_research=="Engineering",]
percent.eng <- ((dim(n.eng)[1])/(dim(gbl.senior)[1]))*100 #Calculate the % of responses from  engineering scientists
n.int <- gbl.senior[gbl.senior$field_research=="Interdisciplinary Science",]
percent.int <- ((dim(n.int)[1])/(dim(gbl.senior)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists
n.ssh <- gbl.senior[gbl.senior$field_research=="Social Science / Humanities",]
percent.ssh <- ((dim(n.ssh)[1])/(dim(gbl.senior)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists


```

**Results**  
In total, `r dim(gbl.senior)[1]`  researchers completed the online survey. Of these, almost xxxxx were male (`r ceiling(percent.men)`%) and xxxxx were female (`r round(percent.women)`%); xxxx proportion either did not input their gender or selected other. `r round(percent.seniorac)` we senior academics and `r round(percent.nonACsenior)` were senior non academics.
...
locations<-aggregate(gender ~ nation, gbl.senior, length)
locations<-locations[!locations$nation=="",]
locations<-droplevels(locations)

#pdf(file="figures/senior/responses_country_13Mar18.pdf", width = 11, height= 8)
...
ggplot(locations, aes(x = reorder(nation, -gender), gender)) + geom_bar(stat='identity',position = position_dodge(width=0.5),fill='#1f78b4') + theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5)) + labs(y="Number of responses", x="", title="Responses by Country")  + 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+scale_y_continuous(expand = c(0,0), limits=c(0, 930)) 
#dev.off()  
...
#pdf(file="figures/senior/responses_country_gapplot_13Mar18.pdf", height=8, width=11)

par(mar=c(11,4,4,2))
gap.barplot(sort(locations$gender,decreasing=TRUE), gap=c(260,799),horiz=F, xaxlab=barlabs, xlab="     ", 
            ylab="Number of responses", ytics=c(0,50,100,150,200,250, 884), main = "Responses by country",las=2, col=c(blue2red(29)))
...
locations.more<-aggregate(gender ~ nation, gbl.senior, length)
...
#pdf(file="figures/senior/responses_country_10+_gapplot_13Mar18.pdf", height=8, width=11)

par(mar=c(7,4,4,2))
gap.barplot(sort(locations.more$gender,decreasing=TRUE), gap=c(260,799),horiz=F, xaxlab=barlabs, xlab="     ", 
            ylab="Number of responses", ytics=c(0,50,100,150,200,250, 884), main = "Countries with greater than 10 responses",las=2, col=color1)
...
locations.less<-aggregate(gender ~ nation, gbl.senior, length)
...
color1<-c('#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4')
#pdf(file="figures/senior/responses_country_10+&10-_gapplot_13Mar18.pdf", height=8, width=15)
...
```{r echo=FALSE, fig.cap="Figure 4.1 Number of  survey respondents by career stage", fig.height=7, fig.width=11, message=FALSE, warning=FALSE}
## load required packages
require(gridExtra); require(tidyr); require(ggplot2); require(stringr);require(RColorBrewer); require(colorRamps); require(plotrix); require(plyr); require(grid);require(gtable)
...
experience<-data.frame(table(gbl.senior$what_participant_group))
...
#pdf(file="figures/senior/4.1_CareerStage_13March18.pdf", width = 11, height= 7)

ggplot(experience, aes(x = reorder(Var1, -Freq), Freq, fill=Var1)) + 
  geom_bar(stat='identity',position = position_dodge(width=0.5)) + 
  geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  scale_x_discrete(labels=c("Senior academic", "Senior\nnon-academic")) +
...
```{r echo=FALSE, fig.cap="Figure 4.2 Survey responses by field of research", fig.height=7, fig.width=11, message=FALSE, warning=FALSE}
field<-data.frame(table(gbl.senior$field_research))
...
#pdf(file="figures/senior/4.2_FieldofResearch_13March18.pdf", width = 11, height= 7)
...
#pdf(file="figures/senior/4.1and4.2_CareerStageAndFieldOfResearch.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.3 Respondents type of research describe in proportal amounts of fundamental use-inspired and applied research. Reseachers were questioned about the percentage of funding allocated to Fundamental Use-inspired or Applied research in the past and in their current research.", fig.height=7, fig.width=11, message=FALSE, warning=FALSE}

##selecting for senior career stage respondents
research$what_participant_group<-revalue(research$what_participant_group, c("Early career academic researcher with <10 years experience applying for research grants since completion of PhD" = "Early career academic researcher", "Non-academic researcher conducting or managing research in industry or government with >10 years of experience" = "Non-academic researcher >10 yrs experience", "Non-academic researcher conducting or managing research in industry or government with <10 years of experience" = "Non-academic researcher <10 yrs experience", "Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future" = "Postdoc or RA", "Senior academic researcher with >10 years of experience applying for research grants" = "Senior academic researcher"))

research.gbl.senior<-research[(research$what_participant_group%in%c("Senior academic researcher" ,"Non-academic researcher >10 yrs experience")),]
research.gbl.senior <- droplevels(research.gbl.senior)

## make sure that Use-inspired is the middle level in the legend
res.plot<-subset(research.gbl.senior, type%in%c("percent_fundemental_research_current",  "percent_Applied_Research_current" , "percent_Use_inspired_Research_current","percent_Applied_Research_past",  "percent_Fundamental_Research_past" , "percent_Use_inspired_Research_past"))
...
#pdf(file="figures/senior/4.3.1_ResearchProportion_13March18.pdf", width = 11, height= 7)
...
ggplot(res.plot, aes(percent, fill=ID)) +
    ...
    theme(aspect.ratio=3/4, panel.grid.major = element_blank(),
          ...
          legend.position=c(0.7, 0.92), axis.title.y=element_text(size=11),
  ...
  scale_x_continuous(breaks=seq(0,100,10))+scale_y_continuous(expand=c(0,0), limits=c(0,450),breaks=c(0,100, 200, 300, 400))
...
p1 <- ggplot(xs$Fundamental,aes(percent, fill=time)) +
  ...
  scale_x_continuous(breaks=seq(0,100,10))+scale_y_continuous(expand=c(0,0), limits=c(0,450),breaks=seq(0,450,50))
...
#pdf(file="figures/senior/4.3.2_ResearchProportion_13March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.4a&b Change in research type proportions and the reasons. Researchers were asked to answer yes, no, or can't comment on if their type of research had changed in the last 10 years and to select what reasons for the change applied to them.",   fig.height=7, fig.width=11, message=FALSE, warning=FALSE}

##selecting for senior career stage respondents
research.change$what_participant_group<-revalue(research.change$what_participant_group, c("Early career academic researcher with <10 years experience applying for research grants since completion of PhD" = "Early career academic researcher", "Non-academic researcher conducting or managing research in industry or government with >10 years of experience" = "Non-academic researcher >10 yrs experience", "Non-academic researcher conducting or managing research in industry or government with <10 years of experience" = "Non-academic researcher <10 yrs experience", "Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future" = "Postdoc or RA", "Senior academic researcher with >10 years of experience applying for research grants" = "Senior academic researcher"))

research.change.gbl.senior<-research.change[(research.change$what_participant_group%in%c("Senior academic researcher" ,"Non-academic researcher >10 yrs experience")),]
research.change.gbl.senior <- droplevels(research.change.gbl.senior)

#Change yes or no
yes.no<-subset(research.change.gbl.senior, select=c("Location","Country", "Country_work",  "gender", "changed_10yrs"))

gbl.senior.yes.no<-aggregate(Country~ changed_10yrs, yes.no, length)
gbl.senior.yes.no<-gbl.senior.yes.no[!gbl.senior.yes.no$changed_10yrs=="",]

p1<-ggplot(data=gbl.senior.yes.no, aes(x=changed_10yrs, y=Country, fill=Country))+ geom_bar(stat='identity') + guides(fill=FALSE)+
  theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(), axis.title.x=element_blank(), plot.title = element_text(hjust = 0.5))+
  geom_text(aes(label=Country), vjust=-0.5)+labs(y="Number of responses", title="Change in research type")+ scale_y_continuous(expand = c(0, 0), limits = c(0,1045))+ annotate('text', 0.5,1022, label="(a)", size=5)
...
reason<-subset(research.change.gbl.senior, select=c("Location","Country","Country_work",  "gender", "Main_reason_change_interest_related",
...
require(tidyr)
gbl.senior.long<-gather(reason, reason.change.gbl.senior, yes.gbl.senior, -Location, -gender, -Country_work, -Country)
# using table to count cases of each category
sum.gbl.senior<-data.frame(table(gbl.senior.long$reason.change.gbl.senior, gbl.senior.long$yes.gbl.senior))
#remove non-response
sum.gbl.senior<-sum.gbl.senior[!sum.gbl.senior$Var1=="",]

sum.gbl.senior$Var1<-revalue(sum.gbl.senior$Var1, c("Main_reason_change_Career_related" = "Career", "Main_reason_change_Funding_related" = "Funding", "Main_reason_change_interest_related" = "Interest", "Main_reason_change_Other" = "Other", "Main_reason_change_Socially_related"  = "Social"))
# change order of the levels
sum.gbl.senior$Var1<-factor(sum.gbl.senior$Var1,levels(sum.gbl.senior$Var1)[c(2,3,1,5,4)])

myColors4<-(brewer.pal(9, "Blues"))
myColors4<-myColors4[c(9, 8,7,6,5,3)]

p2<-ggplot(sum.gbl.senior, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 623))+annotate('text', 5.4,613, label="(b)", size=5)+geom_text(aes(label=Freq), vjust=-0.5, size=3)


#pdf(file="figures/senior/4.4_ReasonForFieldChange.pdf", width = 11, height= 7)

#print(p2)

#dev.off()

#pdf(file="figures/senior/4.4a&b_ReasonForFieldChange_14March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.5 View of change in proportion of research. Researchers were asked how they viewed the change in the type of research they conduct/supervise.", fig.height=7, fig.width=11, message=FALSE, warning=FALSE}

view.propor<-gbl.senior[!gbl.senior$view_change_of_type=="",]
view.propor<-droplevels(view.propor)
change.view.gbl.senior<-aggregate(gender~ view_change_of_type, view.propor, length)
# change order of the levels
change.view.gbl.senior$view_change_of_type<-factor(change.view.gbl.senior$view_change_of_type, 
                                        levels(change.view.gbl.senior$view_change_of_type)[c(5,3,1,2,4)])

myColors5<-(brewer.pal(9, "Blues"))
myColors5<-myColors5[c(4,4,4,4,4,4)]

#pdf(file="figures/senior/4.5_ViewFieldChange_14March18.pdf", width = 11, height= 7)

ggplot(change.view.gbl.senior, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
...
```{r echo=FALSE, fig.cap="Figure 4.6 Current vs past level of partnership outside of academia. Researchers indicated the level of partnership that their current and past (10 years ago) research program had outside of academia).", fig.height=7, fig.width=11, message=FALSE, warning=FALSE}

b.part<-data.frame(table(gbl.senior$partnership_outside_before))
b.part<-b.part[!b.part$Var1=="",]
b.part<-droplevels(b.part)
b.part$time<-"Past"
a.part<-data.frame(table(gbl.senior$partnership_outside))
...
#pdf(file="figures/senior/4.6_LevelsOfPartnership_14March18.pdf", width = 11, height= 7)
...
ggplot(part, aes(x=Var1, y=Freq, fill = time),  ylim(0,375)) + 
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,790))

#dev.off()
```

```{r echo=FALSE, fig.cap="Figure 4.7a&b Did it change and reasons for change in level of external research partnerships over the past decade.", fig.height=7, fig.width=11, message=FALSE, warning=FALSE}
### was there change

change.part.gbl.senior<-data.frame(table(gbl.senior$partnership_change_10yrs))
change.part.gbl.senior<-change.part.gbl.senior[!change.part.gbl.senior$Var1=="",]
change.part.gbl.senior$Var1<-revalue(change.part.gbl.senior$Var1, c("Can't comment (new researcher)"="Can't comment"))
change.part.gbl.senior$Var1<-factor(change.part.gbl.senior$Var1, levels=c("Yes", "No", "Can't comment"))
myColors<-(brewer.pal(9, "Blues"))
myColors<-myColors[c(9, 7,5)]
g1<-ggplot(data=change.part.gbl.senior, aes(x=Var1, y=Freq, fill=Var1))+ 
...
##selecting for senior career stage respondents
part2.reason$what_participant_group<-revalue(part2.reason$what_participant_group, c("Early career academic researcher with <10 years experience applying for research grants since completion of PhD" = "Early career academic researcher", "Non-academic researcher conducting or managing research in industry or government with >10 years of experience" = "Non-academic researcher >10 yrs experience", "Non-academic researcher conducting or managing research in industry or government with <10 years of experience" = "Non-academic researcher <10 yrs experience", "Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future" = "Postdoc or RA", "Senior academic researcher with >10 years of experience applying for research grants" = "Senior academic researcher"))

part2.reason.gbl.senior<-part2.reason[(part2.reason$what_participant_group%in%c("Senior academic researcher" ,"Non-academic researcher >10 yrs experience")),]
part2.reason.gbl.senior <- droplevels(part2.reason.gbl.senior)

reason.pt.long<-gather(part2.reason.gbl.senior, change.reason, yes, -Country,-Country_work, -gender, -nation, -Location, -what_participant_group, -field_research)
sum.gbl.senior<-data.frame(table(reason.pt.long$change.reason, reason.pt.long$yes))

sum.gbl.senior$Var1<-revalue(sum.gbl.senior$Var1, c("reason_partnership_change_career" = "Career", "reason_partnership_change_funding" = "Funding", "reason_partnership_change_interest" = "Interest", "reason_partnership_change_other" = "Other", "reason_partnership_change_socially"  = "Social"))
sum.gbl.senior$Var1<-reorder(sum.gbl.senior$Var1, -sum.gbl.senior$Freq)

myColors7<-(brewer.pal(9, "Blues"))
myColors7<-myColors7[c(9, 8,7,6,5,3)]

g2<-ggplot(sum.gbl.senior, aes(x=Var1, y=Freq, fill=Var1)) + 
...
#pdf(file="figures/senior/4.7a&b_PartnershipChange&Reason_14March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.8 View of change in external partnerships. Researchers were asked how they viewed the change in the level of partnership with external groups.", fig.height=7, fig.width=11, message=FALSE, warning=FALSE}

view<-data.frame(table(gbl.senior$view_change_partnership))
view<-view[!view$Var1=="",]
...
#pdf(file="figures/senior/4.8_ViewChangePartnership_14March18.pdf", width = 11, height= 7)
...
ggplot(view, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+
  ...
  labs(x="", y="Number of responses", title="Opinion of change in partnership level") + colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,270), breaks = seq(0,250,50))
...
```{r echo=FALSE, fig.cap="Fig. 4.9 Number of research grant applications by research category in 2006-2010 and 2011-2015.", fig.height=7, fig.width=11, message=FALSE, warning=FALSE}
part3.grants.long<-read.csv(file="../data/gya-part3.grants.long.csv")

##selecting for senior career stage respondents
part3.grants.long$what_participant_group<-revalue(part3.grants.long$what_participant_group, c("Early career academic researcher with <10 years experience applying for research grants since completion of PhD" = "Early career academic researcher", "Non-academic researcher conducting or managing research in industry or government with >10 years of experience" = "Non-academic researcher >10 yrs experience", "Non-academic researcher conducting or managing research in industry or government with <10 years of experience" = "Non-academic researcher <10 yrs experience", "Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future" = "Postdoc or RA", "Senior academic researcher with >10 years of experience applying for research grants" = "Senior academic researcher"))

part3.grants.long.gbl.senior<-part3.grants.long[(part3.grants.long$what_participant_group%in%c("Senior academic researcher" ,"Non-academic researcher >10 yrs experience")),]
part3.grants.long.gbl.senior <- droplevels(part3.grants.long.gbl.senior)

part3.grants.long.gbl.senior$type.grant<-as.character(part3.grants.long.gbl.senior$type.grant)


#add time variable
part3.grants.long.gbl.senior$time<-ifelse(grepl("11_15", part3.grants.long.gbl.senior$type.grant), "2011-2015", "2006-2010")
part3.grants.long.gbl.senior$type.grant[part3.grants.long.gbl.senior$type.grant=="external_pi_grant_11_15_fundamental"]<-"Fundamental"
part3.grants.long.gbl.senior$type.grant[part3.grants.long.gbl.senior$type.grant=="external_pi_grant_6_10_fundamental"]<-"Fundamental"
part3.grants.long.gbl.senior$type.grant[part3.grants.long.gbl.senior$type.grant=="external_pi_grant_11_15_use"]<-"Use-Inspired"
part3.grants.long.gbl.senior$type.grant[part3.grants.long.gbl.senior$type.grant=="external_pi_grant_6_10_use"]<-"Use-Inspired"
part3.grants.long.gbl.senior$type.grant[part3.grants.long.gbl.senior$type.grant=="external_pi_grant_11_15_applied"]<-"Applied"
part3.grants.long.gbl.senior$type.grant[part3.grants.long.gbl.senior$type.grant=="external_pi_grant_6_10_applied"]<-"Applied"

grants.gbl.senior<-data.frame(with(part3.grants.long.gbl.senior, table(type.grant,time,number)))
grants.gbl.senior$type.grant<-as.factor(grants.gbl.senior$type.grant)


#change order of levels
grants.gbl.senior$number<-factor(grants.gbl.senior$number, levels(grants.gbl.senior$number)[c(1,2,6,7,3,4,5)])
grants.gbl.senior$type.grant<-factor(grants.gbl.senior$type.grant, levels(grants.gbl.senior$type.grant)[c(2,3,1)])

#myColors9<-(brewer.pal(9, "Blues"))
#myColors9<-rev(myColors9[c(9, 5)])
#colScale <- scale_fill_manual(name = "time",values = myColors9)

grants.gbl.senior$ID<-paste(grants.gbl.senior$type.grant, grants.gbl.senior$time)

# change order of the levels
grants.gbl.senior$ID<-as.factor(grants.gbl.senior$ID)
grants.gbl.senior$ID<-factor(grants.gbl.senior$ID, levels(grants.gbl.senior$ID)[c(4,3,6,5,2,1)])

##colors
mycols<-c('#33a02c','#a6cee3','#fb9a99','#1f78b4', '#e31a1c','#b2df8a')
#mycols<-as.data.frame(mycols)

#pdf(file="figures/senior/4.9_NumberGrantApps_14March18.pdf", height=7, width =11)


#####add (a)
xs <- split(grants.gbl.senior,f = grants.gbl.senior$type.grant)
p1 <- ggplot(xs$Fundamental, aes(number, Freq, fill=time), width=0.7) + 
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,1200), breaks = c(0,200,400,600,800,1000,1200))
...
```{r echo=FALSE, fig.cap="Fig 4.10 Research grant application success over the past 10 years. Researchers were asked to estimate the percentage of their research grant applications that were successful, in 2006-2010 and in 2011-2015. Respondents also had the choice to answer No need for applications for this research type.",fig.height=7, fig.width=11, message=FALSE, warning=FALSE}
part3.success.long<-read.csv(file="../data/gya-part3.success.long.csv")

##selecting for senior career stage respondents
part3.success.long$what_participant_group<-revalue(part3.success.long$what_participant_group, c("Early career academic researcher with <10 years experience applying for research grants since completion of PhD" = "Early career academic researcher", "Non-academic researcher conducting or managing research in industry or government with >10 years of experience" = "Non-academic researcher >10 yrs experience", "Non-academic researcher conducting or managing research in industry or government with <10 years of experience" = "Non-academic researcher <10 yrs experience", "Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future" = "Postdoc or RA", "Senior academic researcher with >10 years of experience applying for research grants" = "Senior academic researcher"))

part3.success.long.gbl.senior<-part3.success.long[(part3.success.long$what_participant_group%in%c("Senior academic researcher" ,"Non-academic researcher >10 yrs experience")),]
part3.success.long.gbl.senior <- droplevels(part3.success.long.gbl.senior)

part3.success.long.gbl.senior$type<-as.character(part3.success.long.gbl.senior$type)

#add time variable
part3.success.long.gbl.senior$time<-ifelse(grepl("11_15", part3.success.long.gbl.senior$type), "2011-2015", "2006-2010")
part3.success.long.gbl.senior$type[part3.success.long.gbl.senior$type=="successful_grants_11_15_fundamental"]<-"Fundamental"
part3.success.long.gbl.senior$type[part3.success.long.gbl.senior$type=="successful_grants_6_10_fundamental"]<-"Fundamental"
part3.success.long.gbl.senior$type[part3.success.long.gbl.senior$type=="successful_grants_11_15_use"]<-"Use-Inspired"
part3.success.long.gbl.senior$type[part3.success.long.gbl.senior$type=="successful_grants_6_10_use"]<-"Use-Inspired"
part3.success.long.gbl.senior$type[part3.success.long.gbl.senior$type=="successful_grants_11_15_applied"]<-"Applied"
part3.success.long.gbl.senior$type[part3.success.long.gbl.senior$type=="successful_grants_6_10_applied"]<-"Applied"

success.gbl.senior<-data.frame(with(part3.success.long.gbl.senior, table(type, percent,time)))



#change order of types
success.gbl.senior$type<-as.factor(success.gbl.senior$type)
success.gbl.senior$type<-factor(success.gbl.senior$type, levels(success.gbl.senior$type)[c(2,3,1)])

success.gbl.senior$id<-paste(success.gbl.senior$type, success.gbl.senior$time)

# change order of the levels
success.gbl.senior$id<-as.factor(success.gbl.senior$id)
success.gbl.senior$id<-factor(success.gbl.senior$id, levels(success.gbl.senior$id)[c(3,4,5,6,1,2)])

mycols10<-c('#1f78b4','#a6cee3','#33a02c','#b2df8a','#e31a1c', '#fb9a99')
mycols10<-as.data.frame(mycols10)
mycols10$id<-levels(success.gbl.senior$id)
success.gbl.senior$col<-mycols10$mycols10[match(success.gbl.senior$id, mycols10$id)]
success.gbl.senior$col<-factor(success.gbl.senior$col, levels(success.gbl.senior$col)[c(3,1,4,2,6,5)])

#pdf(file="figures/senior/4.10_GrantAppSuccess_14March18.pdf", height=7, width =11)

out2<-by(data = success.gbl.senior, INDICES = success.gbl.senior$type, FUN = function(n){
  n <- droplevels(n)
...
```{r echo=FALSE, fig.cap="Fig 4.11 Change in grant success rates over the past 10 years. Researchers were asked if they thought that grant sucecss rates have changed in the past 10 years, for each research category.", fig.height=7, fig.width=11, message=FALSE, warning=FALSE}

##selecting for senior career stage respondents
part3.change$what_participant_group<-revalue(part3.change$what_participant_group, c("Early career academic researcher with <10 years experience applying for research grants since completion of PhD" = "Early career academic researcher", "Non-academic researcher conducting or managing research in industry or government with >10 years of experience" = "Non-academic researcher >10 yrs experience", "Non-academic researcher conducting or managing research in industry or government with <10 years of experience" = "Non-academic researcher <10 yrs experience", "Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future" = "Postdoc or RA", "Senior academic researcher with >10 years of experience applying for research grants" = "Senior academic researcher"))

part3.change.gbl.senior<-part3.change[(part3.change$what_participant_group%in%c("Senior academic researcher" ,"Non-academic researcher >10 yrs experience")),]
part3.change.gbl.senior <- droplevels(part3.change.gbl.senior)

##switch to long format
change.long.gbl.senior<-gather(part3.change.gbl.senior, type, level, -Location, -gender, -Country,-nation, -Country_work, -what_participant_group, -field_research)
success.change<-with(change.long.gbl.senior, data.frame(table(type, level)))
#remove no response
...
#pdf(file="figures/senior/4.11_ChangeSuccessRate_14March18.pdf", height=7, width =11)
...
```{r echo=FALSE, fig.cap="Fig 4.12 Importance of practical application of research over the past 10 years. Researchers were asked how important it was to suggest practical applications of their research to ensure that the grant was successful, in 2006-2010 and in 2011-2015.", fig.height=7, fig.width=11, message=FALSE, warning=FALSE}

##selecting for senior career stage respondents
part3.prac.long$what_participant_group<-revalue(part3.prac.long$what_participant_group, c("Early career academic researcher with <10 years experience applying for research grants since completion of PhD" = "Early career academic researcher", "Non-academic researcher conducting or managing research in industry or government with >10 years of experience" = "Non-academic researcher >10 yrs experience", "Non-academic researcher conducting or managing research in industry or government with <10 years of experience" = "Non-academic researcher <10 yrs experience", "Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future" = "Postdoc or RA", "Senior academic researcher with >10 years of experience applying for research grants" = "Senior academic researcher"))

part3.prac.long.gbl.senior<-part3.prac.long[(part3.prac.long$what_participant_group%in%c("Senior academic researcher" ,"Non-academic researcher >10 yrs experience")),]
part3.prac.long.gbl.senior <- droplevels(part3.prac.long.gbl.senior)

part3.prac.long.gbl.senior$year<-as.character(part3.prac.long.gbl.senior$year)

part3.prac.long.gbl.senior$year[part3.prac.long.gbl.senior$year=="practical_applications_important_11_15"]<-"2011-2015"
part3.prac.long.gbl.senior$year[part3.prac.long.gbl.senior$year=="practical_applications_important_6_10"]<-"2006-2010"


prac.gbl.senior<-data.frame(table(part3.prac.long.gbl.senior$year, part3.prac.long.gbl.senior$level))

#change order of levels
prac.gbl.senior$Var2<-factor(prac.gbl.senior$Var2, levels(prac.gbl.senior$Var2)[c(2,6,4,5,3,1)])

myColors12<-(brewer.pal(9, "Blues"))
myColors12<-rev(myColors12[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors12)

#pdf(file="figures/senior/4.12_ImportancePracticalApplication_14March18.pdf", height=7, width =11)

ggplot(prac.gbl.senior, aes(Var2, Freq, fill=Var1)) + 
...
##selecting for senior career stage respondents
part3.part.long$what_participant_group<-revalue(part3.part.long$what_participant_group, c("Early career academic researcher with <10 years experience applying for research grants since completion of PhD" = "Early career academic researcher", "Non-academic researcher conducting or managing research in industry or government with >10 years of experience" = "Non-academic researcher >10 yrs experience", "Non-academic researcher conducting or managing research in industry or government with <10 years of experience" = "Non-academic researcher <10 yrs experience", "Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future" = "Postdoc or RA", "Senior academic researcher with >10 years of experience applying for research grants" = "Senior academic researcher"))

part3.part.long.gbl.senior<-part3.part.long[(part3.part.long$what_participant_group%in%c("Senior academic researcher" ,"Non-academic researcher >10 yrs experience")),]
part3.part.long.gbl.senior <- droplevels(part3.part.long.gbl.senior)

part3.part.long.gbl.senior$year<-as.character(part3.part.long.gbl.senior$year)

part3.part.long.gbl.senior$year[part3.part.long.gbl.senior$year=="include_nonacademia_partners_success_11_15"]<-"2011-2015"
part3.part.long.gbl.senior$year[part3.part.long.gbl.senior$year=="include_nonacademia_partners_success_6_10"]<-"2006-2010"

part.gbl.senior<-data.frame(table(part3.part.long.gbl.senior$year, part3.part.long.gbl.senior$level))
#change order of levels
part.gbl.senior$Var2<-factor(part.gbl.senior$Var2, levels(part.gbl.senior$Var2)[c(2,5,6,4,3,1)])
...
#pdf(file="figures/senior/4.13_ImportancePartnersApplication_14March18.pdf", height=7, width =11)

ggplot(part.gbl.senior, aes(Var2, Freq, fill=Var1))+ 
...
```{r echo=FALSE, fig.cap="Fig 4.14 Distribution of research funding over the past 10 years. Researchers were asked to estimate the distribution of their research funding sources in 2006-2010 and 2011-2015.", fig.height=7, fig.width=11, message=FALSE, warning=FALSE}
p3_master.long<-read.csv(file="../data/gya-p3_master.long.csv")

##selecting for senior career stage respondents
p3_master.long$what_participant_group<-revalue(p3_master.long$what_participant_group, c("Early career academic researcher with <10 years experience applying for research grants since completion of PhD" = "Early career academic researcher", "Non-academic researcher conducting or managing research in industry or government with >10 years of experience" = "Non-academic researcher >10 yrs experience", "Non-academic researcher conducting or managing research in industry or government with <10 years of experience" = "Non-academic researcher <10 yrs experience", "Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future" = "Postdoc or RA", "Senior academic researcher with >10 years of experience applying for research grants" = "Senior academic researcher"))

p3_master.long.gbl.senior<-p3_master.long[(p3_master.long$what_participant_group%in%c("Senior academic researcher" ,"Non-academic researcher >10 yrs experience")),]
p3_master.long.gbl.senior <- droplevels(p3_master.long.gbl.senior)

##revalue the year values
p3_master.long.gbl.senior$year<- revalue(p3_master.long.gbl.senior$year, c("11_15"="2011-2015", "6_10"="2006-2010"))
#change order of levels
p3_master.long.gbl.senior$year<-factor(p3_master.long.gbl.senior$year, levels(p3_master.long.gbl.senior$year)[c(2,1)])

##For-Profit
fp<-p3_master.long.gbl.senior[p3_master.long.gbl.senior$type=="For-Profit",]
...
##Government
gov<-p3_master.long.gbl.senior[p3_master.long.gbl.senior$type=="Government",]
...
int<-p3_master.long.gbl.senior[p3_master.long.gbl.senior$type=="Internal",]
...
ngov<-p3_master.long.gbl.senior[p3_master.long.gbl.senior$type=="Non-governmental",]
...
other<-p3_master.long.gbl.senior[p3_master.long.gbl.senior$type=="Other",]
...
#pdf(file="figures/senior/4.14_DistributionFunding_14March18.pdf", height=7, width =11)
...
```{r echo=FALSE, fig.cap="Fig 4.15 Perceived importance of fundamental research to their government. Researchers were asked how important they thought fundamental research was to the their government. Responses were/were not significantly different between genders.", fig.height=7, fig.width=11, message=FALSE, warning=FALSE}

##selecting for senior career stage respondents
part4$what_participant_group<-revalue(part4$what_participant_group, c("Early career academic researcher with <10 years experience applying for research grants since completion of PhD" = "Early career academic researcher", "Non-academic researcher conducting or managing research in industry or government with >10 years of experience" = "Non-academic researcher >10 yrs experience", "Non-academic researcher conducting or managing research in industry or government with <10 years of experience" = "Non-academic researcher <10 yrs experience", "Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future" = "Postdoc or RA", "Senior academic researcher with >10 years of experience applying for research grants" = "Senior academic researcher"))

part4.gbl.senior<-part4[(part4$what_participant_group%in%c("Senior academic researcher" ,"Non-academic researcher >10 yrs experience")),]
part4.gbl.senior <- droplevels(part4.gbl.senior)

## order bars from very important (left) to can't comment (right)
important<-subset(part4.gbl.senior, select=c("Location","Country",'Country_work', "gender","opinion_fundamental_important"))
...
#pdf(file="figures/senior/4.15_PerceivedImportanceFundamentalGovernment_14March18.pdf", height=7, width =11)
...
ggplot(data=perceive, aes(x=Var1, y=Freq, fill=Var1)) + 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,710)) + colScale

#dev.off()
```

```{r echo=FALSE, fig.cap="Fig 4.16 Perceived change in research priority by their government. Researchers were asked whether any types of research had become higher priority for the their government. Responses were/were not significantly different between genders.", fig.height=7, fig.width=11, message=FALSE, warning=FALSE}
...
priority<-subset(part4.gbl.senior, select=c("Location","Country",'Country_work', "gender", "high_priority_fundamental", "high_priority_use_inspired", "high_priority_applied", "high_priority_no_change"))
...
#pdf(file="figures/senior/4.16_PerceivedChangeResearchPriorityGovernment_14March18.pdf", height=7, width =11)
...
ggplot(data=per.change, aes(x=what.type, higher.priority, fill=what.type))+
  ...
  geom_text(aes(label=higher.priority, vjust=-0.5), size=3) +scale_y_continuous(expand = c(0, 0), limits = c(0,1600))+ colScale

#dev.off()
```


```{r echo=FALSE, fig.cap="Fig 4.17 Anticipated change in research funding in next five years. Researchers were asked whether the availability of research funding would change in the next five years.", fig.height=7, fig.width=11, message=FALSE, warning=FALSE}
...
availability.change<-subset(part4.gbl.senior, select=c("Location", "Country",'Country_work', "gender", "available_funding_fundamental",  
                                             "available_funding_use_inspired", "available_funding_applied"))
## switch to long format
availability.change.long.gbl.senior<-gather(availability.change, what.type, level, -gender, -Location, -Country, -Country_work)
availability.gbl.senior<-with(availability.change.long.gbl.senior, data.frame(table(what.type, level)))

#remove non response
availability.gbl.senior<-availability.gbl.senior[!availability.gbl.senior$level=="",]

## change type to factor with levels
availability.gbl.senior$level<-str_replace_all(availability.gbl.senior$level, "Will ", "")
availability.gbl.senior$what.type<-as.factor(availability.gbl.senior$what.type)
availability.gbl.senior$level<-as.factor(availability.gbl.senior$level)

## change order of levels
availability.gbl.senior$level<-factor(availability.gbl.senior$level, levels(availability.gbl.senior$level)[c(4,5,6,3,2,1 )])
availability.gbl.senior$what.type<-factor(availability.gbl.senior$what.type, levels(availability.gbl.senior$what.type)[c(2,3,1)])

availability.gbl.senior$what.type<-revalue(availability.gbl.senior$what.type, c("available_funding_fundamental"="Fundamental",
...
#pdf(file="figures/senior/4.17_AnticipatedChangeFunding_14March18.pdf", height=7, width =11)

ggplot(data=availability.gbl.senior, aes(x=level, Freq, fill=what.type))+
    geom_bar(stat="identity", position="dodge")+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,670), breaks = seq(0,650,100))
...
## order bars into increase considerably (left) to decrease considerably (right), then no comment at the far right
next.generation<-subset(part4.gbl.senior, select=c("Location", "Country","Country_work", "gender","next_generation"))
# remove non-response
...
#pdf(file="figures/senior/4.18_EffectNextGeneration_14March18.pdf", height=7, width =11)
...
ggplot(data=impact, aes(x=Var1, Freq, fill=Var1))+ 
 ...
 labs(x="", y="Number of responses", title="Effect of funding changes on the next generation of researchers")+
  ...
  geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,550))+colScale
