...
res.port.devp$majority_current <- ifelse(((res.port.devp$percent_Applied_Research_current == 50 | res.port.devp$percent_Use_inspired_Research_current == 50) & res.port.devp$percent_fundemental_research_current == 50), "fund.app.half.current", res.port.devp$majority_current)
res.port.devp$majority_current <- ifelse((res.port.devp$percent_Applied_Research_current == 50 & res.port.devp$percent_Use_inspired_Research_current == 50), "app.current", res.port.devp$majority_current)
...
res.port.devp$majority_past <- ifelse((res.port.devp$percent_Fundamental_Research_past == 50 & (res.port.devp$percent_Use_inspired_Research_past == 50 | res.port.devp$percent_Applied_Research_past == 50)), "fund.app.half.past", res.port.devp$majority_past)
...
res.port.devp$majority_past <- ifelse((res.port.devp$percent_Use_inspired_Research_past == 50 & res.port.devp$percent_Applied_Research_past == 50), "app.past", res.port.devp$majority_past)
...
change <- subset(res.port.devp, select = c("app_mean", "use_mean", "fund_mean", "nation", "id"))
#head(change)
...
#currentmajor
# join them   not doing this anymore Anina wants two seperate figures
...
ggplot(pastmajor, aes(major.research.past, gender, fill = major.research.past)) + geom_bar(stat='identity')+
  ...
  theme(plot.title = element_text(size = 11, hjust = 0.5), panel.grid.major = element_blank(),
        ...
        axis.text.x = element_text(angle=0, size=9), axis.text.y=element_text(size = 9))+scale_fill_manual(name='major.research.past', values=myColors.major) +scale_y_continuous(expand = c(0, 0), limits = c(0,670))+ annotate('text', 4.4,625, label= sum(pastmajor$gender), size=5) +
  annotate('text', 4.23, 625, label= "n = ", size=5)
...
#pdf(file="figures/developed_randCA/4.3.3_ResearchProportion_Current.pdf", width = 11, height= 7)
...
ggplot(currentmajor, aes(major.research.current, gender, fill = major.research.current)) + geom_bar(stat='identity') +
  ...
  theme(plot.title = element_text(size = 11, hjust = 0.5), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        legend.key=element_blank(),legend.text=element_text(size = 9),
        axis.text.x = element_text(angle=0, size=9), axis.text.y=element_text(size = 9))+scale_fill_manual(name='major.research.current', values=myColors.major)+scale_y_continuous(expand = c(0, 0), limits = c(0,515))+ annotate('text', 4.4,485, label= sum(currentmajor$gender), size=5) +
  annotate('text', 4.23, 485, label= "n = ", size=5)
...
ggplot(data=success.change, aes(x=level, Freq, fill=type))+ facet_wrap(~type, nrow=3)+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,700))+geom_text(data=success.change, aes(x=6.4, y=650, label=n), colour="black", inherit.aes=FALSE, parse=FALSE, size = 4.5)+annotate('text', 6.09,650, label= "n = ", size=5)
...
ggplot(data=perceive, aes(x=Var1, y=Freq, fill=Var1)) + 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,585)) + colScale + annotate('text', 5.4,555, label= sum(perceive$Freq), size=5) +annotate('text', 5.15,555, label= "n = ", size=5)
...
ggplot(data=per.change, aes(x=what.type, higher.priority, fill=what.type))+
  ...
  geom_text(aes(label=higher.priority, vjust=-0.5), size=3) +scale_y_continuous(expand = c(0, 0), limits = c(0,1350), breaks = seq(0,1350, 200))+ colScale+ annotate('text', 4.4,1290, label= sum(per.change$higher.priority), size=5) +annotate('text', 4.2,1290, label= "n = ", size=5)
...
ggplot(data=availability.gbl, aes(x=level, Freq, fill=what.type))+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,620), breaks = seq(0,600,100)) +annotate('text', 6,580, label= "n = ", size=5)+geom_text(data=availability.gbl, aes(x=6.35, y=580, label=n), colour="black", inherit.aes=FALSE, parse=FALSE, size = 4.5)
