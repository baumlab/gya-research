...
##selecting for central and south american countries + Caribbean
cen.south.america<-read.csv("../data/central_south_america.csv")
cen.south.america <- as.data.frame(cen.south.america)
cen.south.america <- cen.south.america[ , 1]

c.s.america<-survey[survey$nation%in%cen.south.america,]

#check
unique(c.s.america$nation)
c.s.america <- droplevels(c.s.america)


#Gender:
n.c.s.america.res.men <- c.s.america[c.s.america$gender=="Male",]               #Count how many responses from  male researchers
percent.men<-((dim(n.c.s.america.res.men)[1])/(dim(c.s.america)[1]))*100      #Calculate the % of responses from  male researchers
n.c.s.america.res.women <- c.s.america[c.s.america$gender=="Female",]           #Count how many responses from  female researchers
percent.women<-((dim(n.c.s.america.res.women)[1])/(dim(c.s.america)[1]))*100  #Calculate the % of responses from  female researchers

#Career Stage Tabulations:
#table(c.s.america$what_participant_group)
n.seniorac <- c.s.america[c.s.america$what_participant_group=="Senior academic researcher with >10 years of experience applying for research grants",]
percent.seniorac<-((dim(n.seniorac)[1])/(dim(c.s.america)[1]))*100 #Calculate the % of responses from  senior academic researchers
n.earlyac <- c.s.america[c.s.america$what_participant_group=="Early career academic researcher with <10 years experience applying for research grants since completion of PhD",]
percent.earlyac<-((dim(n.earlyac)[1])/(dim(c.s.america)[1]))*100 #Calculate the % of responses from  early academic researchers
n.pdf <- c.s.america[c.s.america$what_participant_group=="Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future",]
percent.pdf<-((dim(n.pdf)[1])/(dim(c.s.america)[1]))*100 #Calculate the % of responses from  post-docs
n.nonACearly <- c.s.america[c.s.america$what_participant_group=="Non-academic researcher conducting or managing research in industry or government with <10 years of experience",]
percent.nonACearly<-((dim(n.nonACearly)[1])/(dim(c.s.america)[1]))*100 #Calculate the % of responses from early career non-academics
n.nonACsenior <- c.s.america[c.s.america$what_participant_group=="Non-academic researcher conducting or managing research in industry or government with >10 years of experience",]
percent.nonACsenior<-((dim(n.nonACsenior)[1])/(dim(c.s.america)[1]))*100 #Calculate the % of responses from early career non-academics
n.unk <- c.s.america[c.s.america$what_participant_group=="",]
percent.unk<-((dim(n.unk)[1])/(dim(c.s.america)[1]))*100 #Calculate the % of responses from unknown career stage

#Discipline Tabulations:
n.natural <- c.s.america[c.s.america$field_research=="Natural Science",]
percent.natural <- ((dim(n.natural)[1])/(dim(c.s.america)[1]))*100 #Calculate the % of responses from  natural scientists
n.physics <- c.s.america[c.s.america$field_research=="Physical Science (eg. math, physics, chemistry, computer science)",]
percent.physics <- ((dim(n.physics)[1])/(dim(c.s.america)[1]))*100 #Calculate the % of responses from  physical scientists
n.med <- c.s.america[c.s.america$field_research=="Medicine and Life Science",]
percent.med <- ((dim(n.med)[1])/(dim(c.s.america)[1]))*100 #Calculate the % of responses from  med/life scientists
n.eng <- c.s.america[c.s.america$field_research=="Engineering",]
percent.eng <- ((dim(n.eng)[1])/(dim(c.s.america)[1]))*100 #Calculate the % of responses from  engineering scientists
n.int <- c.s.america[c.s.america$field_research=="Interdisciplinary Science",]
percent.int <- ((dim(n.int)[1])/(dim(c.s.america)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists
n.ssh <- c.s.america[c.s.america$field_research=="Social Science / Humanities",]
percent.ssh <- ((dim(n.ssh)[1])/(dim(c.s.america)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists
...
locations<-aggregate(gender ~ nation, c.s.america, length)
locations<-locations[!locations$nation=="",]
locations<-droplevels(locations)

#pdf(file="figures/central_south_america/responses_country_12Mar18.pdf", width = 11, height= 8)
...
#pdf(file="figures/central_south_america/responses_country_gapplot_12Mar18.pdf", height=8, width=11)

par(mar=c(11,4,4,2))
gap.barplot(sort(locations$gender,decreasing=TRUE), gap=c(28,114),horiz=F, xaxlab=barlabs, xlab="     ",
            ylab="Number of responses", ytics=c(0,10,20,115,120,125), main = "Responses by country",las=2, col=c(blue2red(29)))
...
locations.more<-aggregate(gender ~ nation, c.s.america, length)
...
#pdf(file="figures/central_south_america/responses_country_10+_gapplot_12Mar18.pdf", height=8, width=11)

par(mar=c(7,4,4,2))
gap.barplot(sort(locations.more$gender,decreasing=TRUE), gap=c(28,114),horiz=F, xaxlab=barlabs, xlab="     ",
            ylab="Number of responses", ytics=c(25,115,120,125), main = "Countries with greater than 10 responses",las=2, col=color1)
...
locations.less<-aggregate(gender ~ nation, c.s.america, length)
...
color1<-c('#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4')
#pdf(file="figures/central_south_america/responses_country_10+&10-_gapplot_12Mar18.pdf", height=8, width=15)
layout(matrix(c(1,2),1,2, byrow = TRUE), widths=c(0.7,2))
par(mar=c(11,4,4,2))
gap.barplot(sort(locations.more$gender,decreasing=TRUE), gap=c(28,114),horiz=F, xaxlab=barlabs, xlab="     ",
            ylab="Number of responses", ytics=c(25,115,120,125), main = "Countries with greater\nthan 10 responses",las=2, col=color1)
par(mar=c(11,2,4,1))
barplot(sort(locations.less$gender,decreasing=TRUE), names.arg = locations.less$nation[order(locations.less$gender, decreasing=TRUE)] ,horiz=F, xlab="",
...
experience<-data.frame(table(c.s.america$what_participant_group))
...
#pdf(file="figures/central_south_america/4.1_CareerStage_12March18.pdf", width = 11, height= 7)
...
field<-data.frame(table(c.s.america$field_research))
...
#pdf(file="figures/central_south_america/4.2_FieldofResearch_12March18.pdf", width = 11, height= 7)
...
```{r eval=FALSE, fig.cap="Figure 4.1 + 4.2 Career Stage and Field of research", fig.height=7, fig.width=11, message=FALSE, warning=FALSE, include=FALSE}
##plot1
...
#pdf(file="figures/central_south_america/4.1and4.2_CareerStageAndFieldOfResearch.pdf", width = 11, height= 7)
...
#select for just central and south america
c.s.america<-research[research$nation%in%cen.south.america,]
c.s.america <- droplevels(c.s.america)

## make sure that Use-inspired is the middle level in the legend
res.plot<-subset(c.s.america, type%in%c("percent_fundemental_research_current",  "percent_Applied_Research_current" , "percent_Use_inspired_Research_current","percent_Applied_Research_past",  "percent_Fundamental_Research_past" , "percent_Use_inspired_Research_past"))
...
#pdf(file="figures/central_south_america/4.3.1_ResearchProportion_12March18.pdf", width = 11, height= 7)
...
ggplot(res.plot, aes(percent, fill=ID)) +
  ...
  scale_x_continuous(breaks=seq(0,100,10))+scale_y_continuous(expand=c(0,0), limits=c(0,50),breaks=c(0,10,20,30,40,50))
...
#pdf(file="figures/central_south_america/4.3.2_ResearchProportion_12march18.pdf", width = 11, height= 7)
...
#select for just central and south america
research.change.c.s.america<-research.change[research.change$nation%in%cen.south.america,]
research.change.c.s.america <- droplevels(research.change.c.s.america)

#Change yes or no
yes.no<-subset(research.change.c.s.america, select=c("Location","Country", "Country_work",  "gender", "changed_10yrs"))

c.s.am.yes.no<-aggregate(Country~ changed_10yrs, yes.no, length)
c.s.am.yes.no<-c.s.am.yes.no[!c.s.am.yes.no$changed_10yrs=="",]

p1<-ggplot(data=c.s.am.yes.no, aes(x=changed_10yrs, y=Country, fill=Country))+ geom_bar(stat='identity') + guides(fill=FALSE)+
  theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(), axis.title.x=element_blank(), plot.title = element_text(hjust = 0.5))+
  geom_text(aes(label=Country), vjust=-0.5)+labs(y="Number of responses", title="Change in research type")+ scale_y_continuous(expand = c(0, 0), limits = c(0,90))+ annotate('text', 0.7,86, label="(a)", size=5)

####reason for change 
#select for just central and south america
research.change.c.s.america<-research.change[research.change$nation%in%cen.south.america,]
research.change.c.s.america <- droplevels(research.change.c.s.america)
reason<-subset(research.change.c.s.america, select=c("Location","Country","Country_work",  "gender", "Main_reason_change_interest_related",
...
require(tidyr)
c.s.am.long<-gather(reason, reason.change.c.s.am, yes.c.s.am, -Location, -gender, -Country_work, -Country)
# using table to count cases of each category
sum.c.s.am<-data.frame(table(c.s.am.long$reason.change.c.s.am, c.s.am.long$yes.c.s.am))
#remove non-response
sum.c.s.am<-sum.c.s.am[!sum.c.s.am$Var1=="",]

sum.c.s.am$Var1<-revalue(sum.c.s.am$Var1, c("Main_reason_change_Career_related" = "Career", "Main_reason_change_Funding_related" = "Funding", "Main_reason_change_interest_related" = "Interest", "Main_reason_change_Other" = "Other", "Main_reason_change_Socially_related"  = "Social"))
# change order of the levels
sum.c.s.am$Var1<-factor(sum.c.s.am$Var1,levels(sum.c.s.am$Var1)[c(2,3,1,5,4)])

myColors4<-(brewer.pal(9, "Blues"))
myColors4<-myColors4[c(9, 8,7,6,5,3)]

p2<-ggplot(sum.c.s.am, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 55))+annotate('text', 5.2,52, label="(b)", size=5)+geom_text(aes(label=Freq), vjust=-0.5, size=3)


#pdf(file="figures/central_south_america/4.4_ReasonForFieldChange.pdf", width = 11, height= 7)

#print(p2)

#dev.off()

#pdf(file="figures/central_south_america/4.4a&b_ReasonForFieldChange_12March18.pdf", width = 11, height= 7)
...
```{r eval=FALSE, fig.cap="Figure 4.5 View of change in proportion of research. Researchers were asked how they viewed the change in the type of research they conduct/supervise.", fig.height=7, fig.width=11, message=FALSE, include=FALSE}

view.propor<-c.s.america[!c.s.america$view_change_of_type=="",]
view.propor<-droplevels(view.propor)
change.view<-aggregate(gender~ view_change_of_type, view.propor, length)
# change order of the levels
...
#pdf(file="figures/central_south_america/4.5_ViewFieldChange_12March18.pdf", width = 11, height= 7)
...
ggplot(change.view, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
  ...
  theme(panel.grid.major = element_blank(),legend.text=element_text(size = 9), panel.grid.minor = element_blank())+ labs(x="", y="Number of responses", title="Opinion of research type change")+geom_text(aes(label=gender), vjust=-0.5, size=3)+scale_y_continuous(expand = c(0, 0), limits = c(0,34))+scale_fill_manual(name='view_change_of_type', values=myColors5)#+annotate('text', 5.4,133, label="(c)", size=5)

#dev.off()
```

**Figure 4.5 wont knit - will try harder later to get it to knit in but in the mean time there is a pdf of the plot in the folder**
...
```{r eval=FALSE, fig.cap="Figure 4.6 Current vs past level of partnership outside of academia. Researchers indicated the level of partnership that their current and past (10 years ago) research program had outside of academia).", fig.height=7, fig.width=11, message=FALSE, warning=FALSE, include=FALSE}

b.part<-data.frame(table(c.s.america$partnership_outside_before))
b.part<-b.part[!b.part$Var1=="",]
b.part<-droplevels(b.part)
b.part$time<-"Past"
a.part<-data.frame(table(c.s.america$partnership_outside))
...
#pdf(file="figures/central_south_america/4.6_LevelsOfPartnership_12March18.pdf", width = 11, height= 7)
...
ggplot(part, aes(x=Var1, y=Freq, fill = time),  ylim(0,375)) + 
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,81))

#dev.off()
```
**Figure 4.6 wont knit either - will try harder later to get it to knit in but in the mean time there is a pdf of the plot in the folder**
...
change.part.c.s.am<-data.frame(table(c.s.america$partnership_change_10yrs))
change.part.c.s.am<-change.part.c.s.am[!change.part.c.s.am$Var1=="",]
change.part.c.s.am$Var1<-revalue(change.part.c.s.am$Var1, c("Can't comment (new researcher)"="Can't comment"))
change.part.c.s.am$Var1<-factor(change.part.c.s.am$Var1, levels=c("Yes", "No", "Can't comment"))
myColors<-(brewer.pal(9, "Blues"))
myColors<-myColors[c(9, 7,5)]
g1<-ggplot(data=change.part.c.s.am, aes(x=Var1, y=Freq, fill=Var1))+ 
...
### reason
#select for just central and south america
part2.reason.c.s.america<-part2.reason[part2.reason$nation%in%cen.south.america,]
part2.reason.c.s.america <- droplevels(part2.reason.c.s.america)

reason.pt.long<-gather(part2.reason.c.s.america, change.reason, yes, -Country,-Country_work, -gender, -nation, -Location, -what_participant_group, -field_research)
sum.c.s.am<-data.frame(table(reason.pt.long$change.reason, reason.pt.long$yes))

sum.c.s.am$Var1<-revalue(sum.c.s.am$Var1, c("reason_partnership_change_career" = "Career", "reason_partnership_change_funding" = "Funding", "reason_partnership_change_interest" = "Interest", "reason_partnership_change_other" = "Other", "reason_partnership_change_socially"  = "Social"))
sum.c.s.am$Var1<-reorder(sum.c.s.am$Var1, -sum.c.s.am$Freq)

myColors7<-(brewer.pal(9, "Blues"))
myColors7<-myColors7[c(9, 8,7,6,5,3)]

g2<-ggplot(sum.c.s.am, aes(x=Var1, y=Freq, fill=Var1)) + 
...
#pdf(file="figures/central_south_america/4.7a&b_PartnershipChange&Reason_12March18.pdf", width = 11, height= 7)
...
view<-data.frame(table(c.s.america$view_change_partnership))
view<-view[!view$Var1=="",]
...
#pdf(file="figures/central_south_america/4.8_ViewChangePartnership_12March18.pdf", width = 11, height= 7)
...
ggplot(view, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+
  ...
  labs(x="", y="Number of responses", title="Opinion of change in partnership level") + colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,27), breaks = seq(0,25,5))
...
#select for just central and south america
part3.grants.long.c.s.america<-part3.grants.long[part3.grants.long$nation%in%cen.south.america,]
part3.grants.long.c.s.america <- droplevels(part3.grants.long.c.s.america)

part3.grants.long.c.s.america$type.grant<-as.character(part3.grants.long.c.s.america$type.grant)


#add time variable
part3.grants.long.c.s.america$time<-ifelse(grepl("11_15", part3.grants.long.c.s.america$type.grant), "2011-2015", "2006-2010")
part3.grants.long.c.s.america$type.grant[part3.grants.long.c.s.america$type.grant=="external_pi_grant_11_15_fundamental"]<-"Fundamental"
part3.grants.long.c.s.america$type.grant[part3.grants.long.c.s.america$type.grant=="external_pi_grant_6_10_fundamental"]<-"Fundamental"
part3.grants.long.c.s.america$type.grant[part3.grants.long.c.s.america$type.grant=="external_pi_grant_11_15_use"]<-"Use-Inspired"
part3.grants.long.c.s.america$type.grant[part3.grants.long.c.s.america$type.grant=="external_pi_grant_6_10_use"]<-"Use-Inspired"
part3.grants.long.c.s.america$type.grant[part3.grants.long.c.s.america$type.grant=="external_pi_grant_11_15_applied"]<-"Applied"
part3.grants.long.c.s.america$type.grant[part3.grants.long.c.s.america$type.grant=="external_pi_grant_6_10_applied"]<-"Applied"

grants.c.s.am<-data.frame(with(part3.grants.long.c.s.america, table(type.grant,time,number)))
grants.c.s.am$type.grant<-as.factor(grants.c.s.am$type.grant)


#change order of levels
grants.c.s.am$number<-factor(grants.c.s.am$number, levels(grants.c.s.am$number)[c(1,2,6,7,3,4,5)])
grants.c.s.am$type.grant<-factor(grants.c.s.am$type.grant, levels(grants.c.s.am$type.grant)[c(2,3,1)])

#myColors9<-(brewer.pal(9, "Blues"))
#myColors9<-rev(myColors9[c(9, 5)])
#colScale <- scale_fill_manual(name = "time",values = myColors9)

grants.c.s.am$ID<-paste(grants.c.s.am$type.grant, grants.c.s.am$time)

# change order of the levels
grants.c.s.am$ID<-as.factor(grants.c.s.am$ID)
grants.c.s.am$ID<-factor(grants.c.s.am$ID, levels(grants.c.s.am$ID)[c(4,3,6,5,2,1)])

##colors
mycols<-c('#33a02c','#a6cee3','#fb9a99','#1f78b4', '#e31a1c','#b2df8a')
#mycols<-as.data.frame(mycols)

#pdf(file="figures/central_south_america/4.9_NumberGrantApps_12March18.pdf", height=7, width =11)


#####add (a)
xs <- split(grants.c.s.am,f = grants.c.s.am$type.grant)
p1 <- ggplot(xs$Fundamental, aes(number, Freq, fill=time), width=0.7) + 
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,2100), breaks = c(0,200,400,600,800,1000,1200,1400,1600,1800,2000))
...
#select for just central and south america
part3.success.long.c.s.america<-part3.success.long[part3.success.long$nation%in%cen.south.america,]
part3.success.long.c.s.america <- droplevels(part3.success.long.c.s.america)

part3.success.long.c.s.america$type<-as.character(part3.success.long.c.s.america$type)

#add time variable
part3.success.long.c.s.america$time<-ifelse(grepl("11_15", part3.success.long.c.s.america$type), "2011-2015", "2006-2010")
part3.success.long.c.s.america$type[part3.success.long.c.s.america$type=="successful_grants_11_15_fundamental"]<-"Fundamental"
part3.success.long.c.s.america$type[part3.success.long.c.s.america$type=="successful_grants_6_10_fundamental"]<-"Fundamental"
part3.success.long.c.s.america$type[part3.success.long.c.s.america$type=="successful_grants_11_15_use"]<-"Use-Inspired"
part3.success.long.c.s.america$type[part3.success.long.c.s.america$type=="successful_grants_6_10_use"]<-"Use-Inspired"
part3.success.long.c.s.america$type[part3.success.long.c.s.america$type=="successful_grants_11_15_applied"]<-"Applied"
part3.success.long.c.s.america$type[part3.success.long.c.s.america$type=="successful_grants_6_10_applied"]<-"Applied"

success.c.s.am<-data.frame(with(part3.success.long.c.s.america, table(type, percent,time)))



#change order of types
success.c.s.am$type<-as.factor(success.c.s.am$type)
success.c.s.am$type<-factor(success.c.s.am$type, levels(success.c.s.am$type)[c(2,3,1)])

success.c.s.am$id<-paste(success.c.s.am$type, success.c.s.am$time)

# change order of the levels
success.c.s.am$id<-as.factor(success.c.s.am$id)
success.c.s.am$id<-factor(success.c.s.am$id, levels(success.c.s.am$id)[c(3,4,5,6,1,2)])

mycols10<-c('#1f78b4','#a6cee3','#33a02c','#b2df8a','#e31a1c', '#fb9a99')
mycols10<-as.data.frame(mycols10)
mycols10$id<-levels(success.c.s.am$id)
success.c.s.am$col<-mycols10$mycols10[match(success.c.s.am$id, mycols10$id)]
success.c.s.am$col<-factor(success.c.s.am$col, levels(success.c.s.am$col)[c(3,1,4,2,6,5)])

#pdf(file="figures/central_south_america/4.10_GrantAppSuccess_12March18.pdf", height=7, width =11)

out2<-by(data = success.c.s.am, INDICES = success.c.s.am$type, FUN = function(n){
  n <- droplevels(n)
...
#select for just central and south america
part3.change.c.s.america<-part3.change[part3.change$nation%in%cen.south.america,]
part3.change.c.s.america <- droplevels(part3.change.c.s.america)

##switch to long format
change.long.c.s.am<-gather(part3.change.c.s.america, type, level, -Location, -gender, -Country,-nation, -Country_work, -what_participant_group, -field_research)
success.change<-with(change.long.c.s.am, data.frame(table(type, level)))
#remove no response
...
#pdf(file="figures/central_south_america/4.11_ChangeSuccessRate_12March18.pdf", height=7, width =11)
...
#select for just central and south america
part3.prac.long.c.s.america<-part3.prac.long[part3.prac.long$nation%in%cen.south.america,]
part3.prac.long.c.s.america <- droplevels(part3.prac.long.c.s.america)

part3.prac.long.c.s.america$year<-as.character(part3.prac.long.c.s.america$year)

part3.prac.long.c.s.america$year[part3.prac.long.c.s.america$year=="practical_applications_important_11_15"]<-"2011-2015"
part3.prac.long.c.s.america$year[part3.prac.long.c.s.america$year=="practical_applications_important_6_10"]<-"2006-2010"


prac.c.s.am<-data.frame(table(part3.prac.long.c.s.america$year, part3.prac.long.c.s.america$level))

#change order of levels
prac.c.s.am$Var2<-factor(prac.c.s.am$Var2, levels(prac.c.s.am$Var2)[c(2,6,4,5,3,1)])

myColors12<-(brewer.pal(9, "Blues"))
myColors12<-rev(myColors12[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors12)

#pdf(file="figures/central_south_america/4.12_ImportancePracticalApplication_12March18.pdf", height=7, width =11)

ggplot(prac.c.s.am, aes(Var2, Freq, fill=Var1)) + 
...
#select for just central and south america
part3.part.long.c.s.america<-part3.part.long[part3.part.long$nation%in%cen.south.america,]
part3.part.long.c.s.america <- droplevels(part3.part.long.c.s.america)

part3.part.long.c.s.america$year<-as.character(part3.part.long.c.s.america$year)

part3.part.long.c.s.america$year[part3.part.long.c.s.america$year=="include_nonacademia_partners_success_11_15"]<-"2011-2015"
part3.part.long.c.s.america$year[part3.part.long.c.s.america$year=="include_nonacademia_partners_success_6_10"]<-"2006-2010"

part.c.s.am<-data.frame(table(part3.part.long.c.s.america$year, part3.part.long.c.s.america$level))
#change order of levels
part.c.s.am$Var2<-factor(part.c.s.am$Var2, levels(part.c.s.am$Var2)[c(2,5,6,4,3,1)])
...
#pdf(file="figures/central_south_america/4.13_ImportancePartnersApplication_12March18.pdf", height=7, width =11)

ggplot(part.c.s.am, aes(Var2, Freq, fill=Var1))+ 
...
```{r eval=FALSE, fig.cap="Fig (4.12+4.13) Importance of practical applications and external partnerships.", fig.height=7, fig.width=11, message=FALSE, warning=FALSE, include=FALSE}
###4.12
myColors12<-(brewer.pal(9, "Blues"))
myColors12<-rev(myColors12[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors12)
...
p3_master.long<-read.csv(file="../data/gya-p3_master.long.csv")
#select for just central and south america
p3_master.long.c.s.america<-p3_master.long[p3_master.long$nation%in%cen.south.america,]
p3_master.long.c.s.america <- droplevels(p3_master.long.c.s.america)

##revalue the year values
p3_master.long.c.s.america$year<- revalue(p3_master.long.c.s.america$year, c("11_15"="2011-2015", "6_10"="2006-2010"))
#change order of levels
p3_master.long.c.s.america$year<-factor(p3_master.long.c.s.america$year, levels(p3_master.long.c.s.america$year)[c(2,1)])

##For-Profit
fp<-p3_master.long.c.s.america[p3_master.long.c.s.america$type=="For-Profit",]
...
##Government
gov<-p3_master.long.c.s.america[p3_master.long.c.s.america$type=="Government",]
...
int<-p3_master.long.c.s.america[p3_master.long.c.s.america$type=="Internal",]
...
ngov<-p3_master.long.c.s.america[p3_master.long.c.s.america$type=="Non-governmental",]
...
other<-p3_master.long.c.s.america[p3_master.long.c.s.america$type=="Other",]
...
#pdf(file="figures/central_south_america/4.14_DistributionFunding_12March18.pdf", height=7, width =11)
...
#select for just central and south america
part4.c.s.america<-part4[part4$nation%in%cen.south.america,]
part4.c.s.america <- droplevels(part4.c.s.america)

## order bars from very important (left) to can't comment (right)
important<-subset(part4.c.s.america, select=c("Location","Country",'Country_work', "gender","opinion_fundamental_important"))
...
#pdf(file="figures/central_south_america/4.15_PerceivedImportanceFundamentalGovernment_12March18.pdf", height=7, width =11)
...
priority<-subset(part4.c.s.america, select=c("Location","Country",'Country_work', "gender", "high_priority_fundamental", "high_priority_use_inspired", "high_priority_applied", "high_priority_no_change"))
...
#pdf(file="figures/central_south_america/4.16_PerceivedChangeResearchPriorityGovernment_12March18.pdf", height=7, width =11)
...
ggplot(data=per.change, aes(x=what.type, higher.priority, fill=what.type))+
  ...
  geom_text(aes(label=higher.priority, vjust=-0.5), size=3) +scale_y_continuous(expand = c(0, 0), limits = c(0,155))+ colScale
...
availability.change<-subset(part4.c.s.america, select=c("Location", "Country",'Country_work', "gender", "available_funding_fundamental", "available_funding_use_inspired", "available_funding_applied"))
## switch to long format
availability.change.long.c.s.am<-gather(availability.change, what.type, level, -gender, -Location, -Country, -Country_work)
availability.c.s.am<-with(availability.change.long.c.s.am, data.frame(table(what.type, level)))

#remove non response
availability.c.s.am<-availability.c.s.am[!availability.c.s.am$level=="",]

## change type to factor with levels
availability.c.s.am$level<-str_replace_all(availability.c.s.am$level, "Will ", "")
availability.c.s.am$what.type<-as.factor(availability.c.s.am$what.type)
availability.c.s.am$level<-as.factor(availability.c.s.am$level)

## change order of levels
availability.c.s.am$level<-factor(availability.c.s.am$level, levels(availability.c.s.am$level)[c(4,5,6,3,2,1 )])
availability.c.s.am$what.type<-factor(availability.c.s.am$what.type, levels(availability.c.s.am$what.type)[c(2,3,1)])

availability.c.s.am$what.type<-revalue(availability.c.s.am$what.type, c("available_funding_fundamental"="Fundamental",
...
#pdf(file="figures/central_south_america/4.17_AnticipatedChangeFunding_12March18.pdf", height=7, width =11)

ggplot(data=availability.c.s.am, aes(x=level, Freq, fill=what.type))+
    geom_bar(stat="identity", position="dodge")+
...
## order bars into increase considerably (left) to decrease considerably (right), then no comment at the far right
next.generation<-subset(part4.c.s.america, select=c("Location", "Country","Country_work", "gender","next_generation"))
# remove non-response
...
#pdf(file="figures/central_south_america/4.18_EffectNextGeneration_12March18.pdf", height=7, width =11)
...
ggplot(data=impact, aes(x=Var1, Freq, fill=Var1))+ 
 ...
 labs(x="", y="Number of responses", title="Effect of funding changes on the next generation of researchers")+
  ...
  geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,43))+colScale
