...
locations<-locations[!locations$Country=="",]
locations<-droplevels(locations)

pdf(file="figures/global/ResponsesByCountry.pdf", width = 11, height= 7)

ggplot(locations, aes(x = reorder(Country, -gender), gender)) + geom_bar(stat='identity',position = position_dodge(width=0.5)) + theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5)) + labs(y="Number of responses", x="")  + 
  geom_text(aes(label=gender), angle=90, hjust=-1, size=3) +
  #geom_text(aes(label=Country), angle=90, hjust=-0.5) 
  lims(y=c(0, 1200)) + theme(legend.title=element_text(size=12), 
