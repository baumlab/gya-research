...
**Introduction**  
We developed and ran a quantitative online survey to query researchers about their perceptions of, and experiences with, funding for fundamental research. An important aim of the survey was to provide an understanding of researcher's personal experiences and outlook on the research funding landscape. We had an excellent response to the survey, with over xxxx researchers completing it, suggesting that fundamental research funding is a high priority topic for researchers. Herein, we detail the survey questions and results. 
...
#Gender:
n.gbl.res.men <- survey[survey$gender=="Male",]               #Count how many responses from  male researchers
percent.men<-((dim(n.gbl.res.men)[1])/(dim(survey)[1]))*100      #Calculate the % of responses from  male researchers
n.gbl.res.women <- survey[survey$gender=="Female",]           #Count how many responses from  female researchers
percent.women<-((dim(n.gbl.res.women)[1])/(dim(survey)[1]))*100  #Calculate the % of responses from  female researchers

#Career Stage Tabulations:
#table(survey$what_participant_group)
...
percent.seniorac<-((dim(n.seniorac)[1])/(dim(survey)[1]))*100 #Calculate the % of responses from  senior academic researchers
...
###write csv for Anina with country numbers
# 
# locations<-aggregate(gender ~ nation, survey, length)
# locations<-locations[!locations$nation=="",]
# locations<-droplevels(locations)
# 
# write.csv(locations, file="data/country_responses_March18.csv", row.names = FALSE)
...
#pdf(file="figures/global/responses_country_12March18.pdf", width = 11, height= 8)
...
ggplot(locations, aes(x = reorder(nation, -gender), gender)) + geom_bar(stat='identity',position = position_dodge(width=0.5),fill='#1f78b4') + theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5)) + labs(y="Number of responses", x="", title="Responses by Country")  + 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+scale_y_continuous(expand = c(0,0), limits=c(0, 1600)) 
...
#pdf(file="figures/global/responses_country_gapplot_12March18.pdf", height=8, width=11)
...
locations.more<-aggregate(gender ~ nation, survey, length)
...
#pdf(file="figures/global/responses_country_10+_gapplot_12March18.pdf", height=8, width=11)
...
locations.less<-aggregate(gender ~ nation, survey, length)
...
color1<-c('#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4')
#pdf(file="figures/global/responses_country_10+&10-_gapplot_12March18.pdf", height=8, width=15)
...
```{r echo=FALSE, fig.cap="Figure 4.1 Number of  survey respondents by career stage", fig.width=11, fig.height=7, message=FALSE, warning=FALSE}
...
#pdf(file="figures/global/4.1_CareerStage_10March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.2 Survey responses by field of research", fig.width=11, fig.height=7, message=FALSE, warning=FALSE}
...
#pdf(file="figures/global/4.2_FieldofResearch_10March18.pdf", width = 11, height= 7)
...
```{r eval=FALSE, echo=FALSE, fig.cap="Figure 4.1 + 4.2 Career Stage and Field of research", message=FALSE, warning=FALSE, fig.width=11, fig.height=7}
##plot1
...
#pdf(file="figures/global/4.3.1_ResearchProportion_10March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.3 (different layout) Respondents type of research describe in proportal amounts of fundamental, use-inspired and applied research. Reseachers were questioned about the percentage of funding allocated to Fundamental, Use-inspired or Applied research in the past and in their current research.", message=FALSE, fig.width=11, fig.height=7, warning=FALSE}
...
#pdf(file="figures/global/4.3.2_ResearchProportion_10March18.pdf", width = 11, height= 7)
...
#pdf(file="figures/global/4.4a&b_ReasonForFieldChange_10March18.pdf", width = 11, height= 7)
...
#pdf(file="figures/global/4.5_ViewFieldChange_10March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.6 Current vs past level of partnership outside of academia. Researchers indicated the level of partnership that their current and past (10 years ago) research program had outside of academia).", fig.width=11, fig.height=7, message=FALSE, warning=FALSE}
...
#pdf(file="figures/global/4.6_LevelsOfPartnership_10March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.7a&b Did it change and reasons for change in level of external research partnerships over the past decade.", fig.width=11, fig.height=7,message=FALSE, warning=FALSE}
### was there change
...
#pdf(file="figures/global/4.7a&b_PartnershipChange&Reason_10March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.8 View of change in external partnerships. Researchers were asked how they viewed the change in the level of partnership with external groups.", fig.width=11, fig.height=7,message=FALSE, warning=FALSE}
...
#pdf(file="figures/global/4.8_ViewChangePartnership_10March18.pdf", width = 11, height= 7)
...
```{r, eval=FALSE, fig.cap="Figure ( 4.6 + 4.7 + 4.8 ) External Research Partnerships. ", message=FALSE, warning=FALSE, include=FALSE, fig.width=11, fig.height=7}
###4.6
myColors6<-(brewer.pal(9, "Blues"))
myColors6<-rev(myColors6[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors6)
...
#pdf(file="figures/global/4.9_NumberGrantApps_10March18.pdf", height=7, width =11)
...
#pdf(file="figures/global/4.10_GrantAppSuccess_10March18.pdf", height=7, width =11)
...
```{r echo=FALSE, fig.cap="Fig 4.11 Change in grant success rates over the past 10 years. Researchers were asked if they thought that grant sucecss rates have changed in the past 10 years, for each research category.", message=FALSE, fig.width=11, fig.height=7}
...
#pdf(file="figures/global/4.11_ChangeSuccessRate_10March18.pdf", height=7, width =11)
...
#pdf(file="figures/global/4.12_ImportancePracticalApplication_10March18.pdf", height=7, width =11)
...
#pdf(file="figures/global/4.13_ImportancePartnersApplication_10March18.pdf", height=7, width =11)
...
```{r, eval=FALSE, fig.cap="Fig (4.12+4.13) Importance of practical applications and external partnerships.", message=FALSE, warning=FALSE,fig.width=11, fig.height=7, include=FALSE}
###4.12
myColors12<-(brewer.pal(9, "Blues"))
myColors12<-rev(myColors12[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors12)
...
```{r echo=FALSE, fig.cap="Fig 4.14 Distribution of research funding over the past 10 years. Researchers were asked to estimate the distribution of their research funding sources in 2006-2010 and 2011-2015.",fig.width=11, fig.height=7, message=FALSE, warning=FALSE}
p3_master.long<-read.csv(file="../data/gya-p3_master.long.csv")
...
#pdf(file="figures/global/4.14_DistributionFunding_10March18.pdf", height=7, width =11)
...
#pdf(file="figures/global/4.15_PerceivedImportanceFundamentalGovernment_10March18.pdf", height=7, width =11)
...
#pdf(file="figures/global/4.16_PerceivedChangeResearchPriorityGovernment_10March18.pdf", height=7, width =11)
...
#pdf(file="figures/global/4.17_AnticipatedChangeFunding_10March18.pdf", height=7, width =11)
...
#pdf(file="figures/global/4.18_EffectNextGeneration_10March18.pdf", height=7, width =11)
