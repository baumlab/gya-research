...
p1<-ggplot(data=canada.yesno, aes(x=changed_10yrs, y=Country, fill=Country))+ geom_bar(stat='identity') + guides(fill=FALSE)+
  theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(), axis.title.x=element_blank())+geom_text(aes(label=Country), vjust=-0.5)
  labs(y="No. responses")
  ...
  facet_wrap(~type, nrow=5) + colScale +
   guides(fill=guide_legend(title=NULL)) +
    theme(legend.position=c(0.9, 0.08),
          ...
          panel.grid.minor = element_blank()) +
  labs(x="% research funding", y = "No. of respondents")+ scale_y_continuous(breaks=c(0, 250, 500, 750, 1000, 1300))+
  scale_x_continuous(breaks = seq(0, 100, 10))
```
