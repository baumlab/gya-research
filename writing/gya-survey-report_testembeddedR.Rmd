...
ggplot(change.view.ca, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ labs(x="", y="Number of responses")+geom_text(aes(label=gender), vjust=-0.5, size=3)+scale_y_continuous(expand = c(0, 0), limits = c(0,137))+scale_fill_manual(name='view_change_of_type', values=myColors5)+annotate('text', 5.4,133, label="(c)", size=5)
...
multiplot <- function(g1, g2, g3, plotlist=NULL, file, cols=1, layout=TRUE) {
  library(grid)

  # Make a list from the ... arguments and plotlist
  plots <- c(list(g1, g2, g3), plotlist)
...
require(grid)
grid.arrange(arrangeGrob(p4,p5,p6, ncol=1, grobs=list(left = textGrob("Global Y-axis Label", rot = 90, vjust = 0.5))), heights=c(0.315,0.375,0.315))
...
ggplot(grants.ca, aes(number, Freq, fill=time)) + 
    ...
    theme(aspect.ratio=4/3, panel.grid.major = element_blank(),
          ...
          legend.key=element_blank(),
           strip.text.y = element_text(size=12, face="bold", color="black"),
          strip.background = element_rect(colour="white", fill="white"),
          legend.position=c(0.95, 0.92), legend.text=element_text(size = 9), axis.title.x=element_text(size = 11),
...
ggplot(canada, aes(percent, fill=year))+ geom_histogram(position='dodge', binwidth=10)  + 
  facet_wrap(~type, nrow=2, scales = "free_y") + colScale +
   guides(fill=guide_legend(title=NULL)) +
    theme(aspect.ratio=3/4, legend.position=c(0.922, 0.98),
          strip.text.y = element_text(size=12, face="bold", color="black"),
