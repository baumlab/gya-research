...
require(gridExtra); require(tidyr); require(ggplot2); require(stringr);require(RColorBrewer); require(colorRamps); require(plotrix); require(plyr); require(gridExtra)
...
ggplot(res.plot, aes(percent, fill=ID)) +
    ...
    theme(panel.grid.major = element_blank(),
          ...
          legend.position=c(0.72, 0.92), legend.margin=unit(100,"cm"), axis.title.y=element_text(size=11),
          axis.title.x=element_text(size=11), axis.text.x=element_text(size=9), axis.text.y=element_text(size=9), title=element_text(size = 12)) + colScale + 
  scale_x_continuous(breaks=seq(0,100,20))+scale_y_continuous(expand=c(0,0), limits=c(0,400),breaks=c(0,100,200,300,400)) 
...
ggplot(res.plot, aes(percent, fill=ID)) +
    ...
    theme(panel.grid.major = element_blank(),
          ...
          legend.position=c(0.72, 0.92), legend.margin=unit(100,"cm"), axis.title.y=element_text(size=11),
          axis.title.x=element_text(size=11), axis.text.x=element_text(size=9), axis.text.y=element_text(size=9), title=element_text(size = 12)) + colScale + 
  scale_x_continuous(breaks=seq(0,100,30))+scale_y_continuous(expand=c(0,0), limits=c(0,400),breaks=c(0,100,200,300,400)) 
...
p2<-ggplot(sum.canada, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  theme(axis.text.x = element_text(angle=0, vjust=0.5, size=9), axis.text.y=element_text(size = 9), axis.title=element_text(size = 11),panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),axis.text.x = element_text(angle=0, vjust=0.5),axis.title.x=element_blank()) +guides(fill=FALSE) +scale_fill_manual(name = "Var1",values = myColors4)+
  labs(y="")+
...
ggplot(change.view.ca, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ labs(x="", y="Number of responses")+geom_text(aes(label=gender), vjust=-0.5, size=3)+scale_y_continuous(expand = c(0, 0), limits = c(0,137))+scale_fill_manual(name='view_change_of_type', values=myColors5)
...
```{r, eval=FALSE, fig.cap="Figure 4.2 External Research Partnerships. ( 4.6 + 4.7 + 4.8 )", warning=FALSE, include=FALSE}
###4.6
myColors6<-(brewer.pal(9, "Blues"))
myColors6<-rev(myColors6[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors6)
...
p6<-ggplot(view, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+
  ...
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        axis.text.x = element_text(angle=0, vjust=0, size=9), axis.text.y=element_text(size = 9), axis.title.y=element_text(size=11)) + 
  labs(x="", y="Number of responses") + colScale + scale_y_continuous(expand = c(0, 0), limits = c(0,190), breaks = seq(0,160,40))

#grid.arrange(p4,p5,p6, ncol=1)

multiplot <- function(p4, p5, p6, plotlist = NULL, file, cols = 1, layout = NULL) {
  require(grid)

  plots <- c(list(p4,p5,p6), plotlist)
...
ggplot(grants.ca, aes(number, Freq, fill=time)) + 
    ...
    theme(panel.grid.major = element_blank(),
          ...
          legend.key=element_blank(),
           strip.text.y = element_text(size=12, face="bold", color="black"),
          strip.background = element_rect(colour="white", fill="white"),
          legend.position=c(0.9, 0.97), legend.text=element_text(size = 9), axis.title.x=element_text(size = 11),
...
ggplot(part.ca, aes(Var2, Freq, fill=Var1))+ 
  ...
  scale_x_discrete(labels=c("Mandatory", "Quite\n important", 'Somewhat\n important',"Not very\n important", 'Not at all\n important', "Can't\n comment"))+
    theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
          legend.key=element_blank(),
          legend.position=c(0.91, 0.92), axis.text.x=element_text(size = 9), axis.text.y=element_text(size=9),
...
```{r, eval=FALSE, fig.cap="Fig 4.4 Importance of practical applications and external partnerships. (4.12 + 4.13)", warning=FALSE, include=FALSE}
###4.12
myColors12<-(brewer.pal(9, "Blues"))
myColors12<-rev(myColors12[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors12)
...
```{r, echo=FALSE, fig.cap="Fig 4.14 Distribution of research funding over the past 10 years. Researchers were asked to estimate the distribution of their research funding sources in 2006-2010 and 2011-2015.", warning=FALSE,fig.height=12}
...
ggplot(canada, aes(percent, fill=year))+ geom_histogram(position='dodge', binwidth=10)  + 
  facet_wrap(~type, nrow=2, scales = "free_y") + colScale +
   guides(fill=guide_legend(title=NULL)) +
...
```{r, eval=FALSE, fig.cap="Fig 4.5 Anticipated future of research funding."  - 4.15 + 4.16, include=FALSE}
