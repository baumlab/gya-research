...
ggplot(data=per.change, aes(x=what.type, higher.priority))+
  ...
  geom_text(aes(label=higher.priority, vjust=-0.5), size=3) +scale_y_continuous(expand = c(0, 0), limits = c(0,1250))
```

```{r, echo=FALSE, fig.cap="Fig 4.5 Anticipated future of research funding."  - 4.15 + 4.16}


p1<-ggplot(data=perceive, aes(x=Var1, y=Freq)) + 
      geom_bar(stat='identity', position='dodge',fill=myColors[2])+ 
      labs(x="", y="Number of responses", fill="")+
      scale_x_discrete(labels=c("Very\n important", "Somewhat\n important", 'Not very\n important', 'Not at all\n important', "Can't\n comment"))+
      geom_text(aes(label=Freq, vjust=-0.5), size=3) +
      theme(axis.text.x = element_text(angle=0, vjust=0.4, hjust=0.6), 
            panel.grid.major = element_blank(), 
            panel.grid.minor = element_blank(), axis.title.y=element_text(size=11), axis.text.x=element_text(size=9), axis.text.y=element_text(size=9)) +
  scale_y_continuous(expand = c(0, 0), limits = c(0,535))

p2<-ggplot(data=per.change, aes(x=what.type, higher.priority))+
        geom_bar(stat="identity", position=position_dodge(width = 20), fill=myColors[2])+ 
