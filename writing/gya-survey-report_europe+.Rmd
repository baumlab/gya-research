---
title: "Europe+ online survey on fundamental research"
author: ''
date: "March 2018"
output:
  pdf_document: null
...
##selecting for some of Europe, USA, Canada, Australia, New Zealand, Russia  (Europe+)
europe.plus<-read.csv("../data/europe_plus_countries.csv")
europe.plus <- as.data.frame(europe.plus)
europe.plus <- europe.plus[ , 1]

n.euro.europe.plus<-survey[survey$nation%in%europe.plus,]

#check
unique(n.euro.europe.plus$nation)
n.euro.europe.plus <- droplevels(n.euro.europe.plus)


#Gender:
n.eu.plus.res.men <- n.euro.europe.plus[n.euro.europe.plus$gender=="Male",]               #Count how many responses from  male researchers
percent.men<-((dim(n.eu.plus.res.men)[1])/(dim(n.euro.europe.plus)[1]))*100      #Calculate the % of responses from  male researchers
n.eu.plus.res.women <- n.euro.europe.plus[n.euro.europe.plus$gender=="Female",]           #Count how many responses from  female researchers
percent.women<-((dim(n.eu.plus.res.women)[1])/(dim(n.euro.europe.plus)[1]))*100  #Calculate the % of responses from  female researchers

#Career Stage Tabulations:
#table(n.euro.europe.plus$what_participant_group)
n.seniorac <- n.euro.europe.plus[n.euro.europe.plus$what_participant_group=="Senior academic researcher with >10 years of experience applying for research grants",]
percent.seniorac<-((dim(n.seniorac)[1])/(dim(n.euro.europe.plus)[1]))*100 #Calculate the % of responses from  senior academic researchers
n.earlyac <- n.euro.europe.plus[n.euro.europe.plus$what_participant_group=="Early career academic researcher with <10 years experience applying for research grants since completion of PhD",]
percent.earlyac<-((dim(n.earlyac)[1])/(dim(n.euro.europe.plus)[1]))*100 #Calculate the % of responses from  early academic researchers
n.pdf <- n.euro.europe.plus[n.euro.europe.plus$what_participant_group=="Postdoctoral fellow or research assistant with experience applying for research grants, or anticipating the need to apply for grants in the near future",]
percent.pdf<-((dim(n.pdf)[1])/(dim(n.euro.europe.plus)[1]))*100 #Calculate the % of responses from  post-docs
n.nonACearly <- n.euro.europe.plus[n.euro.europe.plus$what_participant_group=="Non-academic researcher conducting or managing research in industry or government with <10 years of experience",]
percent.nonACearly<-((dim(n.nonACearly)[1])/(dim(n.euro.europe.plus)[1]))*100 #Calculate the % of responses from early career non-academics
n.nonACsenior <- n.euro.europe.plus[n.euro.europe.plus$what_participant_group=="Non-academic researcher conducting or managing research in industry or government with >10 years of experience",]
percent.nonACsenior<-((dim(n.nonACsenior)[1])/(dim(n.euro.europe.plus)[1]))*100 #Calculate the % of responses from early career non-academics
n.unk <- n.euro.europe.plus[n.euro.europe.plus$what_participant_group=="",]
percent.unk<-((dim(n.unk)[1])/(dim(n.euro.europe.plus)[1]))*100 #Calculate the % of responses from unknown career stage

#Discipline Tabulations:
n.natural <- n.euro.europe.plus[n.euro.europe.plus$field_research=="Natural Science",]
percent.natural <- ((dim(n.natural)[1])/(dim(n.euro.europe.plus)[1]))*100 #Calculate the % of responses from  natural scientists
n.physics <- n.euro.europe.plus[n.euro.europe.plus$field_research=="Physical Science (eg. math, physics, chemistry, computer science)",]
percent.physics <- ((dim(n.physics)[1])/(dim(n.euro.europe.plus)[1]))*100 #Calculate the % of responses from  physical scientists
n.med <- n.euro.europe.plus[n.euro.europe.plus$field_research=="Medicine and Life Science",]
percent.med <- ((dim(n.med)[1])/(dim(n.euro.europe.plus)[1]))*100 #Calculate the % of responses from  med/life scientists
n.eng <- n.euro.europe.plus[n.euro.europe.plus$field_research=="Engineering",]
percent.eng <- ((dim(n.eng)[1])/(dim(n.euro.europe.plus)[1]))*100 #Calculate the % of responses from  engineering scientists
n.int <- n.euro.europe.plus[n.euro.europe.plus$field_research=="Interdisciplinary Science",]
percent.int <- ((dim(n.int)[1])/(dim(n.euro.europe.plus)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists
n.ssh <- n.euro.europe.plus[n.euro.europe.plus$field_research=="Social Science / Humanities",]
percent.ssh <- ((dim(n.ssh)[1])/(dim(n.euro.europe.plus)[1]))*100 #Calculate the % of responses from  interdisciplinary scientists
...
locations<-aggregate(gender ~ nation, n.euro.europe.plus, length)
locations<-locations[!locations$nation=="",]
locations<-droplevels(locations)

#pdf(file="figures/europe_plus/responses_country_12Mar18.pdf", width = 11, height= 8)
...
#pdf(file="figures/europe_plus/responses_country_gapplot_12Mar18.pdf", height=8, width=11)
...
locations.more<-aggregate(gender ~ nation, n.euro.europe.plus, length)
...
#pdf(file="figures/europe_plus/responses_country_10+_gapplot_12Mar18.pdf", height=8, width=11)
...
locations.less<-aggregate(gender ~ nation, n.euro.europe.plus, length)
...
color1<-c('#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4','#1f78b4')
#pdf(file="figures/europe_plus/responses_country_10+&10-_gapplot_12Mar18.pdf", height=8, width=15)
...
gap.barplot(sort(locations.more$gender,decreasing=TRUE), gap=c(351,1250),horiz=F, xaxlab=barlabs, xlab="     ", 
            ylab="Number of responses", ytics=c(0,50,100,150,200,250,300,350,1300,1350), main = "Countries with greater than 10 responses",las=2, col=color1)
par(mar=c(11,2,4,1))
barplot(sort(locations.less$gender,decreasing=TRUE), names.arg = locations.less$nation[order(locations.less$gender, decreasing=TRUE)] ,horiz=F, xlab="", 
...
```{r echo=FALSE, fig.cap="Figure 4.1 Number of  survey respondents by career stage", fig.height=11, fig.width=7, message=FALSE, warning=FALSE}
...
experience<-data.frame(table(n.euro.europe.plus$what_participant_group))
...
#pdf(file="figures/europe_plus/4.1_CareerStage_11March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.2 Survey responses by field of research", fig.height=11, fig.width=7, message=FALSE, warning=FALSE}
field<-data.frame(table(n.euro.europe.plus$field_research))
...
#pdf(file="figures/europe_plus/4.2_FieldofResearch_13March18.pdf", width = 11, height= 7)
...
#pdf(file="figures/europe_plus/4.1and4.2_CareerStageAndFieldOfResearch.pdf", width = 11, height= 7)
...
#select for europe +
research.euro<-research[research$nation%in%europe.plus,]
research.euro <- droplevels(research.euro)

## make sure that Use-inspired is the middle level in the legend
res.plot<-subset(research.euro, type%in%c("percent_fundemental_research_current",  "percent_Applied_Research_current" , "percent_Use_inspired_Research_current","percent_Applied_Research_past",  "percent_Fundamental_Research_past" , "percent_Use_inspired_Research_past"))
...
#pdf(file="figures/europe_plus/4.3.1_ResearchProportion_11March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.3 (different layout) Respondents type of research describe in proportal amounts of fundamental, use-inspired and applied research. Reseachers were questioned about the percentage of funding allocated to Fundamental, Use-inspired or Applied research in the past and in their current research.", message=FALSE,fig.height=11, fig.width=7}
#select for europe +
research.euro<-research[research$nation%in%europe.plus,]
research.euro <- droplevels(research.euro)
...
#pdf(file="figures/europe_plus/4.3.2_ResearchProportion_11March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.4a&b Change in research type proportions and the reasons. Researchers were asked to answer yes, no, or can't comment on if their type of research had changed in the last 10 years and to select what reasons for the change applied to them.", fig.height=11, fig.width=7, message=FALSE}
#select for europe +
research.change.euro<-research.change[research.change$nation%in%europe.plus,]
research.change.euro <- droplevels(research.change.euro)

#Change yes or no
yes.no<-subset(research.change.euro, select=c("Location","Country", "Country_work",  "gender", "changed_10yrs"))

euro.yes.no<-aggregate(Country~ changed_10yrs, yes.no, length)
euro.yes.no<-euro.yes.no[!euro.yes.no$changed_10yrs=="",]

p1<-ggplot(data=euro.yes.no, aes(x=changed_10yrs, y=Country, fill=Country))+ geom_bar(stat='identity') + guides(fill=FALSE)+
  theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(), axis.title.x=element_blank(), plot.title = element_text(hjust = 0.5))+
  geom_text(aes(label=Country), vjust=-0.5)+labs(y="Number of responses", title="Change in research type")+ scale_y_continuous(expand = c(0, 0), limits = c(0,1300))+ annotate('text', 0.5,1270, label="(a)", size=5)
...
require(tidyr)
euro.long<-gather(reason, reason.change.euro, yes.euro, -Location, -gender, -Country_work, -Country)
# using table to count cases of each category
sum.euro<-data.frame(table(euro.long$reason.change.euro, euro.long$yes.euro))
#remove non-response
sum.euro<-sum.euro[!sum.euro$Var1=="",]

sum.euro$Var1<-revalue(sum.euro$Var1, c("Main_reason_change_Career_related" = "Career", "Main_reason_change_Funding_related" = "Funding", "Main_reason_change_interest_related" = "Interest", "Main_reason_change_Other" = "Other", "Main_reason_change_Socially_related"  = "Social"))
# change order of the levels
sum.euro$Var1<-factor(sum.euro$Var1,levels(sum.euro$Var1)[c(2,3,1,5,4)])

myColors4<-(brewer.pal(9, "Blues"))
myColors4<-myColors4[c(9, 8,7,6,5,3)]

p2<-ggplot(sum.euro, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+ 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 900))+annotate('text', 5.4,885, label="(b)", size=5)+geom_text(aes(label=Freq), vjust=-0.5, size=3)


#pdf(file="figures/europe_plus/4.4_ReasonForFieldChange.pdf", width = 11, height= 7)

#print(p2)

#dev.off()

#pdf(file="figures/europe_plus/4.4a&b_ReasonForFieldChange_11March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.5 View of change in proportion of research. Researchers were asked how they viewed the change in the type of research they conduct/supervise.", fig.height=11, fig.width=7, message=FALSE, warning=FALSE}

view.propor<-n.euro.europe.plus[!n.euro.europe.plus$view_change_of_type=="",]
view.propor<-droplevels(view.propor)
change.view.euro<-aggregate(gender~ view_change_of_type, view.propor, length)
# change order of the levels
change.view.euro$view_change_of_type<-factor(change.view.euro$view_change_of_type, 
                                        levels(change.view.euro$view_change_of_type)[c(5,3,1,2,4)])

myColors5<-(brewer.pal(9, "Blues"))
myColors5<-myColors5[c(4,4,4,4,4,4)]

#pdf(file="figures/europe_plus/4.5_ViewFieldChange_11March18.pdf", width = 11, height= 7)

ggplot(change.view.euro, aes(x=view_change_of_type, y=gender, fill=view_change_of_type)) + geom_bar(stat='identity')+
  ...
  theme(panel.grid.major = element_blank(),legend.text=element_text(size = 9), panel.grid.minor = element_blank())+ labs(x="", y="Number of responses", title="Opinion of research type change")+geom_text(aes(label=gender), vjust=-0.5, size=3)+scale_y_continuous(expand = c(0, 0), limits = c(0,320))+scale_fill_manual(name='view_change_of_type', values=myColors5)#+annotate('text', 5.4,133, label="(c)", size=5)

#dev.off()
```

```{r eval=FALSE, fig.cap="Figure Type of research (4.3, 4.4, 4.5 )", fig.height=11, fig.width=7, message=FALSE, warning=FALSE, include=FALSE}
...
```{r echo=FALSE, fig.cap="Figure 4.6 Current vs past level of partnership outside of academia. Researchers indicated the level of partnership that their current and past (10 years ago) research program had outside of academia).", fig.height=11, fig.width=7, message=FALSE, warning=FALSE}

b.part<-data.frame(table(n.euro.europe.plus$partnership_outside_before))
b.part<-b.part[!b.part$Var1=="",]
b.part<-droplevels(b.part)
b.part$time<-"Past"
a.part<-data.frame(table(n.euro.europe.plus$partnership_outside))
...
#pdf(file="figures/europe_plus/4.6_LevelsOfPartnership_11March18.pdf", width = 11, height= 7)
...
ggplot(part, aes(x=Var1, y=Freq, fill = time),  ylim(0,375)) + 
  ...
  colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,1050))

#dev.off()
```

```{r echo=FALSE, fig.cap="Figure 4.7a&b Did it change and reasons for change in level of external research partnerships over the past decade.", fig.height=11, fig.width=7, message=FALSE, warning=FALSE}
### was there change

change.part.euro<-data.frame(table(n.euro.europe.plus$partnership_change_10yrs))
change.part.euro<-change.part.euro[!change.part.euro$Var1=="",]
change.part.euro$Var1<-revalue(change.part.euro$Var1, c("Can't comment (new researcher)"="Can't comment"))
change.part.euro$Var1<-factor(change.part.euro$Var1, levels=c("Yes", "No", "Can't comment"))
myColors<-(brewer.pal(9, "Blues"))
myColors<-myColors[c(9, 7,5)]
g1<-ggplot(data=change.part.euro, aes(x=Var1, y=Freq, fill=Var1))+ 
...
reason.pt.long<-gather(part2.reason, change.reason, yes, -Country,-Country_work, -gender, -nation, -Location, -what_participant_group, -field_research)
sum.euro<-data.frame(table(reason.pt.long$change.reason, reason.pt.long$yes))

sum.euro$Var1<-revalue(sum.euro$Var1, c("reason_partnership_change_career" = "Career", "reason_partnership_change_funding" = "Funding", "reason_partnership_change_interest" = "Interest", "reason_partnership_change_other" = "Other", "reason_partnership_change_socially"  = "Social"))
sum.euro$Var1<-reorder(sum.euro$Var1, -sum.euro$Freq)

myColors7<-(brewer.pal(9, "Blues"))
myColors7<-myColors7[c(9, 8,7,6,5,3)]

g2<-ggplot(sum.euro, aes(x=Var1, y=Freq, fill=Var1)) + 
...
#pdf(file="figures/europe_plus/4.7a&b_PartnershipChange&Reason_11March18.pdf", width = 11, height= 7)
...
```{r echo=FALSE, fig.cap="Figure 4.8 View of change in external partnerships. Researchers were asked how they viewed the change in the level of partnership with external groups.", fig.height=11, fig.width=7, message=FALSE, warning=FALSE}

view<-data.frame(table(n.euro.europe.plus$view_change_partnership))
view<-view[!view$Var1=="",]
...
#pdf(file="figures/europe_plus/4.8_ViewChangePartnership_11March18.pdf", width = 11, height= 7)
...
ggplot(view, aes(x=Var1, y=Freq, fill=Var1)) + geom_bar(stat='identity')+
  ...
  labs(x="", y="Number of responses", title="Opinion of change in partnership level") + colScale+scale_y_continuous(expand = c(0, 0), limits = c(0,320), breaks = seq(0,320,50))
...
#select for europe +
part3.grants.long.euro<-part3.grants.long[part3.grants.long$nation%in%europe.plus,]
part3.grants.long.euro <- droplevels(part3.grants.long.euro)

part3.grants.long.euro$type.grant<-as.character(part3.grants.long.euro$type.grant)


#add time variable
part3.grants.long.euro$time<-ifelse(grepl("11_15", part3.grants.long.euro$type.grant), "2011-2015", "2006-2010")
part3.grants.long.euro$type.grant[part3.grants.long.euro$type.grant=="external_pi_grant_11_15_fundamental"]<-"Fundamental"
part3.grants.long.euro$type.grant[part3.grants.long.euro$type.grant=="external_pi_grant_6_10_fundamental"]<-"Fundamental"
part3.grants.long.euro$type.grant[part3.grants.long.euro$type.grant=="external_pi_grant_11_15_use"]<-"Use-Inspired"
part3.grants.long.euro$type.grant[part3.grants.long.euro$type.grant=="external_pi_grant_6_10_use"]<-"Use-Inspired"
part3.grants.long.euro$type.grant[part3.grants.long.euro$type.grant=="external_pi_grant_11_15_applied"]<-"Applied"
part3.grants.long.euro$type.grant[part3.grants.long.euro$type.grant=="external_pi_grant_6_10_applied"]<-"Applied"

grants.euro<-data.frame(with(part3.grants.long.euro, table(type.grant,time,number)))
grants.euro$type.grant<-as.factor(grants.euro$type.grant)


#change order of levels
grants.euro$number<-factor(grants.euro$number, levels(grants.euro$number)[c(1,2,6,7,3,4,5)])
grants.euro$type.grant<-factor(grants.euro$type.grant, levels(grants.euro$type.grant)[c(2,3,1)])

#myColors9<-(brewer.pal(9, "Blues"))
#myColors9<-rev(myColors9[c(9, 5)])
#colScale <- scale_fill_manual(name = "time",values = myColors9)

grants.euro$ID<-paste(grants.euro$type.grant, grants.euro$time)

# change order of the levels
grants.euro$ID<-as.factor(grants.euro$ID)
grants.euro$ID<-factor(grants.euro$ID, levels(grants.euro$ID)[c(4,3,6,5,2,1)])

##colors
mycols<-c('#33a02c','#a6cee3','#fb9a99','#1f78b4', '#e31a1c','#b2df8a')
#mycols<-as.data.frame(mycols)

#pdf(file="figures/europe_plus/4.9_NumberGrantApps_11March18.pdf", height=7, width =11)


#####add (a)
xs <- split(grants.euro,f = grants.euro$type.grant)
p1 <- ggplot(xs$Fundamental, aes(number, Freq, fill=time), width=0.7) + 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,1800), breaks = c(0,200,400,600,800,1000,1200,1400,1600,1800))
...
```{r echo=FALSE, fig.cap="Fig 4.10 Research grant application success over the past 10 years. Researchers were asked to estimate the percentage of their research grant applications that were successful, in 2006-2010 and in 2011-2015. Respondents also had the choice to answer No need for applications for this research type.",  fig.height=11, fig.width=7, message=FALSE, warning=FALSE}
part3.success.long<-read.csv(file="../data/gya-part3.success.long.csv")

#select for europe +
part3.success.long.euro<-part3.success.long[part3.success.long$nation%in%europe.plus,]
part3.success.long.euro <- droplevels(part3.success.long.euro)

part3.success.long.euro$type<-as.character(part3.success.long.euro$type)

#add time variable
part3.success.long.euro$time<-ifelse(grepl("11_15", part3.success.long.euro$type), "2011-2015", "2006-2010")
part3.success.long.euro$type[part3.success.long.euro$type=="successful_grants_11_15_fundamental"]<-"Fundamental"
part3.success.long.euro$type[part3.success.long.euro$type=="successful_grants_6_10_fundamental"]<-"Fundamental"
part3.success.long.euro$type[part3.success.long.euro$type=="successful_grants_11_15_use"]<-"Use-Inspired"
part3.success.long.euro$type[part3.success.long.euro$type=="successful_grants_6_10_use"]<-"Use-Inspired"
part3.success.long.euro$type[part3.success.long.euro$type=="successful_grants_11_15_applied"]<-"Applied"
part3.success.long.euro$type[part3.success.long.euro$type=="successful_grants_6_10_applied"]<-"Applied"

success.euro<-data.frame(with(part3.success.long.euro, table(type, percent,time)))



#change order of types
success.euro$type<-as.factor(success.euro$type)
success.euro$type<-factor(success.euro$type, levels(success.euro$type)[c(2,3,1)])

success.euro$id<-paste(success.euro$type, success.euro$time)

# change order of the levels
success.euro$id<-as.factor(success.euro$id)
success.euro$id<-factor(success.euro$id, levels(success.euro$id)[c(3,4,5,6,1,2)])

mycols10<-c('#1f78b4','#a6cee3','#33a02c','#b2df8a','#e31a1c', '#fb9a99')
mycols10<-as.data.frame(mycols10)
mycols10$id<-levels(success.euro$id)
success.euro$col<-mycols10$mycols10[match(success.euro$id, mycols10$id)]
success.euro$col<-factor(success.euro$col, levels(success.euro$col)[c(3,1,4,2,6,5)])

#pdf(file="figures/europe_plus/4.10_GrantAppSuccess_11March18.pdf", height=7, width =11)

out2<-by(data = success.euro, INDICES = success.euro$type, FUN = function(n){
  n <- droplevels(n)
...
#select for europe +
part3.change.euro<-part3.change[part3.change$nation%in%europe.plus,]
part3.change.euro <- droplevels(part3.change.euro)

##switch to long format
change.long.euro<-gather(part3.change.euro, type, level, -Location, -gender, -Country,-nation, -Country_work, -what_participant_group, -field_research)
success.change<-with(change.long.euro, data.frame(table(type, level)))
#remove no response
...
#pdf(file="figures/europe_plus/4.11_ChangeSuccessRate_11March18.pdf", height=7, width =11)
...
#select for europe +
part3.prac.long.euro<-part3.prac.long[part3.prac.long$nation%in%europe.plus,]
part3.prac.long.euro <- droplevels(part3.prac.long.euro)

part3.prac.long.euro$year<-as.character(part3.prac.long.euro$year)

part3.prac.long.euro$year[part3.prac.long.euro$year=="practical_applications_important_11_15"]<-"2011-2015"
part3.prac.long.euro$year[part3.prac.long.euro$year=="practical_applications_important_6_10"]<-"2006-2010"


prac.euro<-data.frame(table(part3.prac.long.euro$year, part3.prac.long.euro$level))

#change order of levels
prac.euro$Var2<-factor(prac.euro$Var2, levels(prac.euro$Var2)[c(2,6,4,5,3,1)])

myColors12<-(brewer.pal(9, "Blues"))
myColors12<-rev(myColors12[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors12)

#pdf(file="figures/europe_plus/4.12_ImportancePracticalApplication_11March18.pdf", height=7, width =11)

ggplot(prac.euro, aes(Var2, Freq, fill=Var1)) + 
...
#select for europe +
part3.part.long.euro<-part3.part.long[part3.part.long$nation%in%europe.plus,]
part3.part.long.euro <- droplevels(part3.part.long.euro)

part3.part.long.euro$year<-as.character(part3.part.long.euro$year)

part3.part.long.euro$year[part3.part.long.euro$year=="include_nonacademia_partners_success_11_15"]<-"2011-2015"
part3.part.long.euro$year[part3.part.long.euro$year=="include_nonacademia_partners_success_6_10"]<-"2006-2010"

part.euro<-data.frame(table(part3.part.long.euro$year, part3.part.long.euro$level))
#change order of levels
part.euro$Var2<-factor(part.euro$Var2, levels(part.euro$Var2)[c(2,5,6,4,3,1)])
...
#pdf(file="figures/europe_plus/4.13_ImportancePartnersApplication_11March18.pdf", height=7, width =11)

ggplot(part.euro, aes(Var2, Freq, fill=Var1))+ 
...
```{r eval=FALSE, fig.cap="Fig (4.12+4.13) Importance of practical applications and external partnerships.", fig.height=11, fig.width=7, message=FALSE, warning=FALSE, include=FALSE}
###4.12
myColors12<-(brewer.pal(9, "Blues"))
myColors12<-rev(myColors12[c(9, 5)])
colScale <- scale_fill_manual(name = "Var1",values = myColors12)
...
#select for europe +
p3_master.long.euro<-p3_master.long[p3_master.long$nation%in%europe.plus,]
p3_master.long.euro <- droplevels(p3_master.long.euro)

##revalue the year values
p3_master.long.euro$year<- revalue(p3_master.long.euro$year, c("11_15"="2011-2015", "6_10"="2006-2010"))
#change order of levels
p3_master.long.euro$year<-factor(p3_master.long.euro$year, levels(p3_master.long.euro$year)[c(2,1)])

##For-Profit
fp<-p3_master.long.euro[p3_master.long.euro$type=="For-Profit",]
...
p1<-ggplot(fp, aes(percent, fill=year))+ geom_histogram(position='dodge', binwidth=10)  + colScale +
  ...
  scale_y_continuous(expand =c(0,0),limits=c(0,2400), breaks=seq(0, 2400, 200))+
  scale_x_continuous( breaks = seq(0, 100, 10))


##Government
gov<-p3_master.long.euro[p3_master.long.euro$type=="Government",]
...
int<-p3_master.long.euro[p3_master.long.euro$type=="Internal",]
...
ngov<-p3_master.long.euro[p3_master.long.euro$type=="Non-governmental",]
...
other<-p3_master.long.euro[p3_master.long.euro$type=="Other",]
...
#pdf(file="figures/europe_plus/4.14_DistributionFunding_11March18.pdf", height=7, width =11)
...
#select for europe +
part4.euro<-part4[part4$nation%in%europe.plus,]
part4.euro <- droplevels(part4.euro)

## order bars from very important (left) to can't comment (right)
important<-subset(part4.euro, select=c("Location","Country",'Country_work', "gender","opinion_fundamental_important"))
...
#pdf(file="figures/europe_plus/4.15_PerceivedImportanceFundamentalGovernment_11March19.pdf", height=7, width =11)
...
ggplot(data=perceive, aes(x=Var1, y=Freq, fill=Var1)) + 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,900)) + colScale
...
priority<-subset(part4.euro, select=c("Location","Country",'Country_work', "gender", "high_priority_fundamental", "high_priority_use_inspired", "high_priority_applied", "high_priority_no_change"))
...
#pdf(file="figures/europe_plus/4.16_PerceivedChangeResearchPriorityGovernment_11March18.pdf", height=7, width =11)
...
ggplot(data=per.change, aes(x=what.type, higher.priority, fill=what.type))+
  ...
  geom_text(aes(label=higher.priority, vjust=-0.5), size=3) +scale_y_continuous(expand = c(0, 0), limits = c(0,2140))+ colScale

#dev.off()
```


```{r echo=FALSE, fig.cap="Fig 4.17 Anticipated change in research funding in next five years. Researchers were asked whether the availability of research funding would change in the next five years.", fig.height=11, fig.width=7, message=FALSE, warning=FALSE}
#select for europe +
part4.euro<-part4[part4$nation%in%europe.plus,]
part4.euro <- droplevels(part4.euro)
...
availability.change<-subset(part4.euro, select=c("Location", "Country",'Country_work', "gender", "available_funding_fundamental",  
                                             "available_funding_use_inspired", "available_funding_applied"))
## switch to long format
availability.change.long.euro<-gather(availability.change, what.type, level, -gender, -Location, -Country, -Country_work)
availability.euro<-with(availability.change.long.euro, data.frame(table(what.type, level)))

#remove non response
availability.euro<-availability.euro[!availability.euro$level=="",]

## change type to factor with levels
availability.euro$level<-str_replace_all(availability.euro$level, "Will ", "")
availability.euro$what.type<-as.factor(availability.euro$what.type)
availability.euro$level<-as.factor(availability.euro$level)

## change order of levels
availability.euro$level<-factor(availability.euro$level, levels(availability.euro$level)[c(4,5,6,3,2,1 )])
availability.euro$what.type<-factor(availability.euro$what.type, levels(availability.euro$what.type)[c(2,3,1)])

availability.euro$what.type<-revalue(availability.euro$what.type, c("available_funding_fundamental"="Fundamental",
...
#pdf(file="figures/europe_plus/4.17_AnticipatedChangeFunding_11March19.pdf", height=7, width =11)

ggplot(data=availability.euro, aes(x=level, Freq, fill=what.type))+
    geom_bar(stat="identity", position="dodge")+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,850), breaks = seq(0,800,200))
...
## order bars into increase considerably (left) to decrease considerably (right), then no comment at the far right
next.generation<-subset(part4.euro, select=c("Location", "Country","Country_work", "gender","next_generation"))
# remove non-response
...
#pdf(file="figures/europe_plus/4.18_EffectNextGeneration_11March18.pdf", height=7, width =11)
...
ggplot(data=impact, aes(x=Var1, Freq, fill=Var1))+ 
 ...
 labs(x="", y="Number of responses", title="Effect of funding changes on the next generation of researchers")+
  ...
  geom_text(aes(label=Freq, vjust=-0.5), size=3)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,750))+colScale
