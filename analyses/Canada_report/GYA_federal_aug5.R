...
p<-ggplot(dat1, aes(Year, value, col=Type, group=1)) + 
  geom_line(size=1,col=1) + #scale_x_continuous(breaks=NULL) + 
  labs(x="", y=ylabel) + 
...
dat2$sector<-ifelse(grepl("Social", dat2$type), "Social sciences", "Natural sciences")
...
g1<-ggplot(dat2, aes(Year, value, col=sector, group=1)) + 
  geom_line() + #scale_x_continuous(breaks=NULL) + 
  labs(x="", y="") + 
  theme(legend.title=element_blank(),
...
g1<-ggplot(dat3[!dat3$Type=="Percent of Total R&D in the Natural Sciences and Engineering funded by the Federal Government",],
			...
			scale_fill_manual(values=c(brewer.pal(8, name="Dark2")))
...
g2<-ggplot(dat7[dat7$type=="Personnel",],
			aes(Year, value, fill=type, group=1)) + geom_line() +  scale_y_continuous(labels=comma) +
			labs(x="", y=ylabel) +
			guides(fill=FALSE) + 
