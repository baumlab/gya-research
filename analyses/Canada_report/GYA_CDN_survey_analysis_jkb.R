## Started 16 September 2016 by JKB
## Script to conduct statistical analyses of GYA survey - Canadian reponses only 
...
require(colorRamps); require(plotrix); require(plyr); 
...
#--------------------#--------------------#--------------------
names(research.change)
head(research.change)
dim(research.change)

cdn.res.change <- research.change[research.change$Country_work=="Canada" | (!(research.change$Country_work=="Canada") & 
                                                                  research.change$Country_work=="" & research.change$Country=="Canada"),]
dim(cdn.res.change)  #1283 12
head(cdn.res.change)

#### Overall 
cdn.res.change<-cdn.res.change[!cdn.res.change$changed_10yrs=="",]
cdn.res.change<-cdn.res.change[!cdn.res.change$changed_10yrs=="Can't comment (new researcher)",]
dim(cdn.res.change)  #1149 12
cdn.res.change<-droplevels(cdn.res.change)
table(cdn.res.change$changed_10yrs)
## No Yes 
## 635 514 
## 55.3% said no (when no comment removed)
## See plot in first_survey_responses.pdf page 11

### Gender
#cdn.res.change<-cdn.res.change[!cdn.res.change$gender=="Other",]
cdn.res.change<-cdn.res.change[!cdn.res.change$gender=="",]
dim(cdn.res.change)  #1138 12
cdn.res.change<-cdn.res.change[!cdn.res.change$changed_10yrs=="",]
dim(cdn.res.change)  #1138 12
cdn.res.change<-droplevels(cdn.res.change)
#head(canada)
...
change <-data.frame(table(cdn.res.change$changed_10yrs))
change
sum.change<-data.frame(table(cdn.res.change$changed_10yrs, cdn.res.change$gender))
sum.change
...
#*******************************************************************
#*******************NOT Significant P = 0.3657 i.e. No sig difference in responses (yes vs. no) between men and women ***************************
#*******************************************************************
